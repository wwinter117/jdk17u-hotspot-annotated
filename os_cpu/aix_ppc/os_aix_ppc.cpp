        ...
        if (thread->thread_state() == _thread_in_Java) {
            ...
            if (sig == SIGILL && nativeInstruction_at(pc)->is_sigill_zombie_not_entrant()) {
                ...
                goto run_stub;
                       ...
                       cb->is_compiled()) {
                if (TraceTraps) {
                    tty->print_cr("trap: safepoint_poll at "
                    INTPTR_FORMAT
                    " (%s)", p2i(pc),
                ...
                goto run_stub;
                       ...
                       cb->is_compiled()) {
                if (TraceTraps) {
                    tty->print_cr("trap: safepoint_poll at return at "
                    INTPTR_FORMAT
                    " (nmethod)", p2i(pc));
                }
                ...
                if (TraceTraps) {
                    tty->print_cr("trap: ic_miss_check at "
                    INTPTR_FORMAT
                    " (SIGTRAP)", pc);
                }
                ...
                if (TraceTraps) {
                    tty->print_cr("trap: null_check at "
                    INTPTR_FORMAT
                    " (SIGTRAP)", pc);
                }
                ...
                const char *msg = NULL;
                switch (stop_type) {
                    case MacroAssembler::stop_stop              :
                        msg = "stop";
                        break;
                    case MacroAssembler::stop_untested          :
                        msg = "untested";
                        break;
                    case MacroAssembler::stop_unimplemented     :
                        msg = "unimplemented";
                        break;
                    case MacroAssembler::stop_shouldnotreachhere:
                        msg = "shouldnotreachhere";
                        break;
                    default:
                        msg = "unknown";
                        break;
...
void os::print_context(outputStream *st, const void *context) {
    ...
    st->print("pc ="
    INTPTR_FORMAT
    "  ", uc->uc_mcontext.jmp_context.iar);
    st->print("lr ="
    INTPTR_FORMAT
    "  ", uc->uc_mcontext.jmp_context.lr);
    st->print("ctr="
    INTPTR_FORMAT
    "  ", uc->uc_mcontext.jmp_context.ctr);
    st->cr();
    for (int i = 0; i < 32; i++) {
        st->print("r%-2d="
        INTPTR_FORMAT
        "  ", i, uc->uc_mcontext.jmp_context.gpr[i]);
        if (i % 3 == 2) st->cr();
    }
    st->cr();
    st->cr();
...
void os::print_tos_pc(outputStream *st, const void *context) {
    ...
    st->print_cr("Decoded instructions: (pc="
    PTR_FORMAT
    ")", pc);
...
void os::print_register_info(outputStream *st, const void *context) {
    ...
    st->print("pc =");
    print_location(st, (intptr_t) uc->uc_mcontext.jmp_context.iar);
    st->print("lr =");
    print_location(st, (intptr_t) uc->uc_mcontext.jmp_context.lr);
    st->print("sp =");
    print_location(st, (intptr_t) os::Aix::ucontext_get_sp(uc));
    for (int i = 0; i < 32; i++) {
        st->print("r%-2d=", i);
