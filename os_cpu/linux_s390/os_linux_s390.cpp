...
bool PosixSignals::pd_hotspot_signal_handler(int sig, siginfo_t *info,
    ...
    if (info != NULL && uc != NULL && thread != NULL) {
        ...
        if ((unsigned long) (pc - (address) info->si_addr) <= (unsigned long) Assembler::instr_maxlen()) {
            ...
            if (TraceTraps) {
                tty->print_cr("trap_pc at "
                INTPTR_FORMAT, p2i(trap_pc));
            }
        }
        ...
        if (thread->thread_state() == _thread_in_Java) {
            ...
            if (sig == SIGILL && nativeInstruction_at(pc)->is_sigill_zombie_not_entrant()) {
                ...
                stub = SharedRuntime::continuation_for_implicit_exception(thread, trap_pc,
...
size_t os::Posix::_compiler_thread_min_stack_allowed = (52
DEBUG_ONLY(+ 32)) *
K;
size_t os::Posix::_java_thread_min_stack_allowed = (32
DEBUG_ONLY(+ 8)) *
K;
...
void os::print_context(outputStream *st, const void *context) {
    ...
    st->print_cr("        ip = "
    INTPTR_FORMAT
    " ", uc->uc_mcontext.psw.addr);
    st->print_cr(" proc mask = "
    INTPTR_FORMAT
    " ", uc->uc_mcontext.psw.mask);
    ...
    for (int i = 0; i < 16; i += 2) {
        st->print("  fr%-2d = "
        INTPTR_FORMAT
        "  ", i, (int64_t)(uc->uc_mcontext.fpregs.fprs[i].d));
        st->print("  fr%-2d = "
        INTPTR_FORMAT
        "  |", i + 1, (int64_t)(uc->uc_mcontext.fpregs.fprs[i + 1].d));
...
void os::print_register_info(outputStream *st, const void *context) {
    ...
    st->print("pc =");
    print_location(st, (intptr_t) uc->uc_mcontext.psw.addr);
    for (int i = 0; i < 16; i++) {
        st->print("r%-2d=", i);
