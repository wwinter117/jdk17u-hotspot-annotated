...
inline T Atomic::PlatformCmpxchg<1>::operator()(T volatile *dest __attribute__((unused)),
    ...
    uint64_t
            w_cv = ((uint64_t)(unsigned char)
    compare_value) << shift;  // widen to 64-bit 0x00000000..CC..
    uint64_t
            w_ev = ((uint64_t)(unsigned char)
    exchange_value) << shift; // widen to 64-bit 0x00000000..EE..
...
inline T Atomic::PlatformCmpxchg<4>::operator()(T volatile *dest __attribute__((unused)),
    ...
    __asm__ __volatile__ (
            ...
            : /*%3*/"r" ((int64_t)(int32_t)
    compare_value), /*%4*/"r"(exchange_value)
    : "memory" );
