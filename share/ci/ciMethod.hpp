...
public:
    ...
    vmIntrinsicID intrinsic_id() const {
        check_is_loaded();
        return _intrinsic_id;
    }

    bool has_exception_handlers() const {
        check_is_loaded();
    ...
    int interpreter_invocation_count() const {
        check_is_loaded();
    ...
    ciMethod *
    resolve_invoke(ciKlass *caller, ciKlass *exact_receiver, bool check_access = true, bool allow_abstract = false);
    ...
    bool is_default_method() const {
        return !is_abstract() && !is_private() &&
               holder()->is_interface();
    }

    bool is_overpass() const {
        check_is_loaded();
        return _is_overpass;
    }
