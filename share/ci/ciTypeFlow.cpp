...
bool ciTypeFlow::StateVector::apply_one_bytecode(ciBytecodeStream *str) {
    ...
    switch (str->cur_bc()) {
        ...
        case Bytecodes::_aload:
            load_local_object(str->get_index());
            break;
        ...
        case Bytecodes::_astore:
            store_local_object(str->get_index());
            break;
        case Bytecodes::_astore_0:
            store_local_object(0);
            break;
        case Bytecodes::_astore_1:
            store_local_object(1);
            break;
        case Bytecodes::_astore_2:
            store_local_object(2);
            break;
        case Bytecodes::_astore_3:
            store_local_object(3);
            break;
        ...
        case Bytecodes::_dload:
            load_local_double(str->get_index());
            break;
        ...
        case Bytecodes::_dstore:
            store_local_double(str->get_index());
            break;
        case Bytecodes::_dstore_0:
            store_local_double(0);
            break;
        case Bytecodes::_dstore_1:
            store_local_double(1);
            break;
        case Bytecodes::_dstore_2:
            store_local_double(2);
            break;
        case Bytecodes::_dstore_3:
            store_local_double(3);
            break;
        ...
        case Bytecodes::_fload:
            load_local_float(str->get_index());
            break;
        ...
        case Bytecodes::_fstore:
            store_local_float(str->get_index());
            break;
        ...
        case Bytecodes::_lstore:
            store_local_long(str->get_index());
            break;
...
void ciTypeFlow::Block::print_on(outputStream *st) const {
    ...
    st->print("  ");
    print_value_on(st);
    st->print(" Stored locals: ");
    def_locals()->print_on(st, outer()->method()->max_locals());
    tty->cr();
...
ciTypeFlow::Block *ciTypeFlow::block_at(int bci, ciTypeFlow::JsrSet *jsrs, CreateOption option) {
    ...
    assert(block == NULL ? (option == no_create) : block->is_backedge_copy() == (option == create_backedge_copy),
           "create option consistent with result");
    ...
    for (int j = 0; j < block_ct; j++) {
        ...
        for (int e = 0; e <= 1; e++) {
            ...
            for (int k = 0; k < l->length(); k++) {
                ...
                if (!s->has_post_order()) {
                    if (CITraceTypeFlow) {
                        tty->print("Removing dead %s successor of #%d: ", (e ? "exceptional" : "normal"),
                                   block->pre_order());
