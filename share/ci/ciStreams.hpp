...
public:
    ...
    int get_constant_u2(bool is_wide = false) const {
        return bytecode().get_constant_u2(instruction_size() - 2, cur_bc_raw(), is_wide);
    }
...
Bytecode::Bytecode(const ciBytecodeStream *stream, address bcp) : _bcp(bcp != NULL ? bcp : stream->cur_bcp()),
                                                                  _code(Bytecodes::code_at(NULL, addr_at(0))) {}
