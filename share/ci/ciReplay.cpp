...
public:
    ...
    void process_compile(TRAPS) {
        ...
        CompiledMethod *nm = (entry_bci != InvocationEntryBci) ? method->lookup_osr_nmethod_for(entry_bci, comp_level,
                                                                                                true) : method->code();
        if (nm != NULL) {
            nm->make_not_entrant();
        }
    ...
    if (rec == NULL) {
        ...
        if (rec->_data_length != 0) {
            ...
            for (int i = 0; i < rec->_classes_length; i++) {
                ...
                intptr_t status = 0;
                if (k != NULL) {
                    status = ciTypeEntries::with_status(env->get_metadata(k)->as_klass(),
...
bool ciReplay::should_inline(void *data, ciMethod *method, int bci, int inline_depth) {
    if (data != NULL) {
        GrowableArray < ciInlineRecord * > *records = (GrowableArray < ciInlineRecord * > *)
        data;
        VM_ENTRY_MARK;
