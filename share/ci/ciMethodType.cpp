...
ciType *ciMethodType::rtype() const {
    GUARDED_VM_ENTRY(
            oop
    rtype = java_lang_invoke_MethodType::rtype(get_oop());
...
int ciMethodType::ptype_count() const {
    GUARDED_VM_ENTRY(
    return java_lang_invoke_MethodType::ptype_count(get_oop());)
}

int ciMethodType::ptype_slot_count() const {
    GUARDED_VM_ENTRY(
    return java_lang_invoke_MethodType::ptype_slot_count(get_oop());)
}

ciType *ciMethodType::ptype_at(int index) const {
    GUARDED_VM_ENTRY(
            oop
    ptype = java_lang_invoke_MethodType::ptype(get_oop(), index);
