...
public:
    ...
    ArgumentMap raw_pop() {
        guarantee(_stack_height > 0, "stack underflow");
        return _stack[--_stack_height];
    }
    ...
    void raw_push(ArgumentMap i) {
        guarantee(_stack_height < _max_stack, "stack overflow");
        _stack[_stack_height++] = i;
    }
...
void BCEscapeAnalyzer::set_returned(ArgumentMap vars) {
    ...
    _return_allocated =
            _return_allocated && vars.contains_allocated() && !(vars.contains_unknown() || vars.contains_vars());
}
...
void BCEscapeAnalyzer::invoke(StateInfo &state, Bytecodes::Code code, ciMethod *target, ciKlass *holder) {
    ...
    bool directly_recursive = (method() == target) &&
                              (code != Bytecodes::_invokevirtual || target->is_final_method() ||
                               state._stack[arg_base].is_empty());
    ...
    if (skip_callee) {
        TRACE_BCEA(3,
                   tty->print_cr("[EA] skipping method %s::%s", holder->name()->as_utf8(), target->name()->as_utf8()));
...
void BCEscapeAnalyzer::compute_escape_info() {
    ...
    if (iid == vmIntrinsics::_none &&
        (method()->is_abstract() || method()->is_native() || !method()->holder()->is_initialized()
...
BCEscapeAnalyzer::BCEscapeAnalyzer(ciMethod *method, BCEscapeAnalyzer *parent)
        : _arena(CURRENT_ENV->arena()), _conservative(method == NULL || !EstimateArgEscape), _method(method),
          _methodData(method ? method->method_data() : NULL), _arg_size(method ? method->arg_size() : 0),
          _arg_local(_arena), _arg_stack(_arena), _arg_returned(_arena), _return_local(false), _return_allocated(false),
          _allocated_escapes(false), _unknown_modified(false), _dependencies(_arena, 4, 0, NULL), _parent(parent),
