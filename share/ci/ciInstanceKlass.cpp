...
bool ciInstanceKlass::is_box_cache_valid() const {
    ...
    if (box_type != T_OBJECT) {
        switch (box_type) {
            case T_INT:
                return is_klass_initialized(java_lang_Integer_IntegerCache::symbol());
            case T_CHAR:
                return is_klass_initialized(java_lang_Character_CharacterCache::symbol());
            case T_SHORT:
                return is_klass_initialized(java_lang_Short_ShortCache::symbol());
            case T_BYTE:
                return is_klass_initialized(java_lang_Byte_ByteCache::symbol());
            case T_LONG:
                return is_klass_initialized(java_lang_Long_LongCache::symbol());
            case T_BOOLEAN:
            case T_FLOAT:
            case T_DOUBLE:
                return true;
...
void ciInstanceKlass::print_impl(outputStream *st) {
    ...
    GUARDED_VM_ENTRY(st->print(" loader="
    INTPTR_FORMAT, p2i(loader()));)
    if (is_loaded()) {
...
void ciInstanceKlass::compute_injected_fields() {
    ...
    assert(_has_injected_fields == -1 || _has_injected_fields == has_injected_fields,
           "broken concurrent initialization");
...
public:
    ...
    void do_field(fieldDescriptor *fd) {
        if (fd->is_final() && !fd->has_initial_value()) {
            ...
            _out->print("staticfield %s %s %s ", _holder, fd->name()->as_quoted_ascii(),
                        fd->signature()->as_quoted_ascii());
            switch (fd->field_type()) {
                case T_BYTE:
                    _out->print_cr("%d", mirror->byte_field(fd->offset()));
                    break;
                case T_BOOLEAN:
                    _out->print_cr("%d", mirror->bool_field(fd->offset()));
                    break;
                case T_SHORT:
                    _out->print_cr("%d", mirror->short_field(fd->offset()));
                    break;
                case T_CHAR:
                    _out->print_cr("%d", mirror->char_field(fd->offset()));
                    break;
                case T_INT:
                    _out->print_cr("%d", mirror->int_field(fd->offset()));
                    break;
                case T_LONG:
                    _out->print_cr(INT64_FORMAT, (int64_t)(mirror->long_field(fd->offset())));
                    break;
                case T_FLOAT: {
