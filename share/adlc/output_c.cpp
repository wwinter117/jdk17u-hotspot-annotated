...
static int pipeline_res_cycles_initializer(
    ...
    for (i = 0; i < pipeline->_rescount; i++) {
        templen += snprintf_checked(&resource_cycles[templen], resource_cycles_size - templen, "  %*d%c // %s\n",
                                    cyclelen, res_cycles[i], (i < pipeline->_rescount - 1) ? ',' : ' ',
                                    pipeline->_reslist.name(i));
    }
...
static int pipeline_res_mask_initializer(
    ...
    for (pipeclass->_resUsage.reset();
        ...
        for (j = cyclemasksize - 1; j >= 0; j--) {
            formatlen =
                    snprintf_checked(&resource_mask[templen], resource_mask_size - templen, "0x%08x%s", res_mask[j],
                                     j > 0 ? ", " : "");
            templen += formatlen;
        }
            ...
            switch (left_interface_type) {
                case Form::register_interface: {
                    ...
                    if (left_op_index != 0) {
                        ...
                        snprintf_checked(left_reg_index, sizeof(left_reg_index), ",inst%u_idx%u", (unsigned) left_index,
                                         (unsigned) left_op_index);
                    } else {
                    ...
                    if (right_index != -1) {
                        ...
                        if (right_op_index != 0) {
                            // Must have index into operands
                            snprintf_checked(right_reg_index, sizeof(right_reg_index), ",inst%u_idx%u",
                                             (unsigned) right_index, (unsigned) right_op_index);
                        } else {
                ...
                case Form::memory_interface: {
                    ...
                    fprintf(fp, "/* %d.%s$$base */ inst%d->_opnds[%d]->base(ra_,inst%d,inst%d_idx%d)) &&\n",
                            right_index, right_op, right_index, right_op_index, right_index, right_index,
                            right_op_index);
                    // index
...
static void
generate_peepreplace(FILE *fp, FormDict &globals, PeepMatch *pmatch, PeepConstraint *pconstraint, PeepReplace *preplace,
                     int max_position) {
    ...
    if (preplace != NULL) {
        ...
        for (preplace->next_operand(inst_num, op_name);
            ...
            if (opnds_index == 0) {
                ...
                fprintf(fp,
                        "        ra_->set_pair(root->_idx, ra_->get_reg_second(inst%d), ra_->get_reg_first(inst%d));\n",
                        inst_num, inst_num);
                ...
                if (_operand == NULL) {
                    ...
                    if (disp != NULL && (*disp == '$')) {
                        ...
                        if (op->_matrule && op->_matrule->is_base_constant(_AD.globalNames())) {
                            ...
                            assert(op->ideal_to_sReg_type(type) != Form::none,
                                   "StackSlots access displacements using 'sRegs'");
                            _may_reloc = false;
                        } else {
...
void ArchDesc::define_postalloc_expand(FILE *fp, InstructForm &inst) {
    ...
    for (; (type != NULL) && (name != NULL);
        ...
        if (strcmp(arg_name, "constanttablebase") == 0) {
            ib += snprintf_checked(ib, remaining_buflen(idxbuf, ib),
...
void ArchDesc::defineEmit(FILE *fp, InstructForm &inst) {
    ...
    if (inst.is_mach_constant() && inst.is_ideal_jump()) {
        fprintf(fp,
                "  ra_->C->output()->constant_table().fill_jump_table(cbuf, (MachConstantNode*) this, _index2label);\n");
    }
...
void ArchDesc::defineClasses(FILE *fp) {
    ...
    if (_pipeline != NULL) {
        for (; (machnode = (MachNodeForm *) _machnodes.iter()) != NULL;) {
            fprintf(_CPP_PIPELINE_file._fp,
                    "const Pipeline * %sNode::pipeline() const { return (&pipeline_class_%03d); }\n",
    ...
    if (_pipeline != NULL) {
        for (; (instr = (InstructForm *) _instructions.iter()) != NULL;) {
            if (instr->_ins_pipe && _pipeline->_classlist.search(instr->_ins_pipe)) {
                fprintf(_CPP_PIPELINE_file._fp, "\n");
                fprintf(_CPP_PIPELINE_file._fp,
                        "const Pipeline * %*sNode::pipeline_class() { return (&pipeline_class_%03d); }\n",
                        max_ident_len, instr->_ident,
                        ((class PipeClassForm *) _pipeline->_classdict[instr->_ins_pipe])->_num);
                fprintf(_CPP_PIPELINE_file._fp,
                        "const Pipeline * %*sNode::pipeline() const { return (&pipeline_class_%03d); }\n",
                        max_ident_len, instr->_ident,
                        ((class PipeClassForm *) _pipeline->_classdict[instr->_ins_pipe])->_num);
...
public:
    ...
    void closing() {
        fprintf(_cpp, "  0 // no trailing comma\n");
        OutputMap::closing();
    }
...
public:
    ...
    void closing() {
        fprintf(_cpp, "  \"invalid rule name\" // no trailing comma\n");
        OutputMap::closing();
    }
...
public:
    ...
    void closing() {
        fprintf(_cpp, "  false // no trailing comma\n");
        OutputMap::closing();
    }
