...
private:
    ...
    ProductionState(const ProductionState &) : _production(cmpstr, hashstr, Form::arena) {
...
void ArchDesc::expand_opclass(FILE *fp, const char *indent, const Expr *cost,
    ...
    if (op && op->_classes.count() > 0) {
        if (debug_output) {
            fprintf(fp, "// expand operand classes for operand: %s \n", (char *) op->_ident);
        } // %%%%% Explanation
...
public:
    ...
    static void set_found(int index, bool val) {
        check_index(index);
        _found[index] = val;
    }
    ...
    static const char *type(int index) {
        check_index(index);
        return _type[index];
    }

    static const char *var(int index) {
        check_index(index);
        return _var[index];
    }

    static const char *pred(int index) {
        check_index(index);
        return _pred[index];
    }
...
bool         dfa_shared_preds::_found[dfa_shared_preds::count] = {false, false,
                                                                  false               IA32_ONLY(COMMA false)};
const char *dfa_shared_preds::_type[dfa_shared_preds::count] = {"int", "jlong",
                                                                "intptr_t"          IA32_ONLY(COMMA "bool")};
const char *dfa_shared_preds::_var[dfa_shared_preds::count] = {"_n_get_int__", "_n_get_long__",
                                                               "_n_get_intptr_t__" IA32_ONLY(COMMA "Compile__current____select_24_bit_instr__")};
const char *dfa_shared_preds::_pred[dfa_shared_preds::count] = {"n->get_int()", "n->get_long()",
                                                                "n->get_intptr_t()" IA32_ONLY(COMMA "Compile::current()->select_24_bit_instr()")};

void
ArchDesc::gen_dfa_state_body(FILE *fp, Dict &minimize, ProductionState &status, Dict &operands_chained_from, int i) {
...
static void dumpexpr(const void *expr) {
    fflush(stdout);
    ((Expr *) expr)->print();
}
...
static void print_production(const void *production) {
    fflush(stdout);
