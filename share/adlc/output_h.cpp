    ...
    if (_register) {
        ...
        while ((reg_def = _register->iter_RegDefs()) != NULL) {
            if (strcmp(reg_def->_callconv, "SOE") == 0 ||
                strcmp(reg_def->_callconv, "AS") == 0)
                ++saved_on_entry;
            if (strcmp(reg_def->_c_conv, "SOE") == 0 ||
                strcmp(reg_def->_c_conv, "AS") == 0)
                ++c_saved_on_entry;
        }
        fprintf(fp_hpp, "\n");
...
static void defineConstructor(FILE *fp, const char *name, uint num_consts,
    ...
    if ((comp = lst.iter()) == NULL) {
        ...
        switch (constant_type) {
            ...
            case Form::idealN : {
                fprintf(fp, "const TypeNarrowOop *c%d", i);
                break;
            }
            case Form::idealNKlass : {
                fprintf(fp, "const TypeNarrowKlass *c%d", i);
                break;
            }
            case Form::idealP : {
            ...
            default: assert(!is_ideal_bool, "Non-constant operand lacks component list.");
                break;
        }
    } // end if NULL
    else {
    ...
    if (call_type != Form::invalid_type) {
        switch (call_type) {
            ...
            case Form::JAVA_NATIVE:
                ...
                break;
            default: assert(0, "ShouldNotReachHere");
        }
        fprintf(fp, "  st->cr();\n");
...
void ArchDesc::declare_pipe_classes(FILE *fp_hpp) {
    ...
    if (_pipeline->_maxcycleused <= 32) {
        ...
        if (_pipeline->_maxcycleused <= 32)
            ...
            fprintf(fp_hpp,
                    "  Pipeline_Use_Cycle_Mask(uint mask1, uint mask2) : _mask((((uint64_t)mask1) << 32) | mask2) {}\n\n");
        ...
        if ((oper->_matrule) && (oper->_matrule->_lChild == NULL) &&
            ...
            if (data_type != Form::none) {
                ...
                switch (data_type) {
                    ...
                    default: assert(false, "No support for this type of stackSlot");
                }
...
/*    else if ( instr->_matrule && instr->_matrule->_rChild &&
      ...
      fprintf(fp, " };\n");
            ...
            fprintf(fp, "  const Type            *bottom_type() const { return TypeRawPtr::BOTTOM; } // Box?\n");
                   ...
                   !strcmp(instr->_matrule->_rChild->_opType, "CMoveP")) {
            int offset = 1;
            ...
            fprintf(fp,
                   ...
                   !strcmp(instr->_matrule->_rChild->_opType, "CMoveN")) {
            int offset = 1;
            // Special special hack to see if the Cmp? has been incorporated in the conditional move
            MatchNode *rl = instr->_matrule->_rChild->_lChild;
            if (rl && !strcmp(rl->_opType, "Binary")) {
...
public:
    ...
    void map(char *name) {
        if (name) fprintf(_cpp, "  %s_rule", name);
        else fprintf(_cpp, "  0");
    }
