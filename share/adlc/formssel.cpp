        ...
        if (form->interface_type(globals) == Form::memory_interface) {
            if (comp->isa(Component::DEF)) {
                ...
                if (last_memory_DEF != NULL) {
                    assert(0 == strcmp(last_memory_DEF, comp->_name),
                           "every memory DEF is followed by a USE of the same name");
...
bool InstructForm::cisc_spills_to(ArchDesc &AD, InstructForm *instr) {
    ...
    cisc_spill_operand = _matrule->matchrule_cisc_spill_match(globals, AD.get_registers(), instr->_matrule, op_name,
                                                              reg_type);
...
bool InstructForm::check_branch_variant(ArchDesc &AD, InstructForm *short_branch) {
    if (_matrule != NULL &&
        ...
        if (different) {
            globalAD->syntax_err(short_branch->_linenum,
                                 "Instruction %s and its short form %s have different parameters\n", _ident,
                                 short_branch->_ident);
        }
...
bool Opcode::print_opcode(FILE *fp, Opcode::opcode_type desired_opcode) {
    ...
    switch (desired_opcode) {
        ...
        case TERTIARY:
            ...
            break;
        default: assert(false, "ShouldNotReachHere();");
            break;
    }
...
void OperandForm::format_constant(FILE *fp, uint const_index, uint const_type) {
    switch (const_type) {
        case Form::idealI:
            ...
            break;
        case Form::idealP:
            fprintf(fp, "  if (_c%d) _c%d->dump_on(st);\n", const_index, const_index);
            break;
...
void OperandForm::access_constant(FILE *fp, FormDict &globals,
    ...
    switch (dtype) {
        case idealI:
            ...
            break;
        case idealP:
            fprintf(fp, "_c%d->get_con()", const_index);
            break;
        case idealL:
            fprintf(fp, "_c%d", const_index);
            break;
...
int MatchNode::needs_ideal_memory_edge(FormDict &globals) const {
    static const char *needs_ideal_memory_list[] = {
            ...
            "StoreVector", "LoadVector", "LoadVectorGather", "StoreVectorScatter", "LoadVectorMasked",
            "StoreVectorMasked",
            "LoadRange", "LoadKlass", "LoadNKlass", "LoadL_unaligned", "LoadD_unaligned",
            "LoadPLocked",
            "StorePConditional", "StoreIConditional", "StoreLConditional",
            "CompareAndSwapB", "CompareAndSwapS", "CompareAndSwapI", "CompareAndSwapL", "CompareAndSwapP",
            "CompareAndSwapN",
            "WeakCompareAndSwapB", "WeakCompareAndSwapS", "WeakCompareAndSwapI", "WeakCompareAndSwapL",
            "WeakCompareAndSwapP", "WeakCompareAndSwapN",
            "CompareAndExchangeB", "CompareAndExchangeS", "CompareAndExchangeI", "CompareAndExchangeL",
            "CompareAndExchangeP", "CompareAndExchangeN",
#if INCLUDE_SHENANDOAHGC
...
int MatchNode::cisc_spill_match(FormDict &globals, RegisterForm *registers, MatchNode *mRule2, const char *&operand,
                                const char *&reg_type) {
    int cisc_spillable = Maybe_cisc_spillable;
