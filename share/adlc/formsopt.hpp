...
public:
    ...
    template<typename T>
    T *addRegClass(const char *className);
...
public:
    ...
    PipeClassOperandForm(const char *stage, unsigned iswrite, unsigned more_instrs)
            : _stage(stage), _iswrite(iswrite), _more_instrs(more_instrs) {};
...
public:
    ...
    // Constructor
            : _resource(resource), _stage(stage), _cycles(cycles) {};
...
public:
    ...
    void setFixedLatency(int fixed_latency) {
        _has_fixed_latency = 1;
