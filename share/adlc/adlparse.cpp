    ...
    do {
        ...
        if (!strcmp(ident, "predicate")) oper->_predicate = pred_parse();
            ...
            // Check identifier to see if it is the name of an attribute
                 ...
                 (attr->_atype == OP_ATTR))
            oper->_attribs = attr_parse(ident);
        else {
            parse_err(SYNERR,
                      "expected one of - constraint, predicate, match, encode, format, construct, or the name of a defined operand attribute at %s\n",
                      ident);
        }
        skipws();
    } while (_curchar != '%');
    ...
    if (_curchar == '%' && *(_ptr + 1) == '{') {
        ...
        while (_curchar != '%' && *(_ptr + 1) != '}') {
            ...
            if (strcmp(token, "reg_def") == 0) { reg_def_parse(); }
                ...
                parse_err(SYNERR, "bad token %s inside register block.\n", token);
                break;
            }
            skipws();
        }
...
void ADLParser::skipws_common(bool do_preproc) {
    ...
    while (_curline != NULL) {                // Check for end of file
        if (*_ptr == '\n') {                   // keep proper track of new lines
            ...
            _ptr = ++next;
            next++;               // increment _ptr past comment end
