...
void RangeCheckEliminator::Visitor::do_LogicOp(LogicOp *lo) {
    if (lo->type()->as_IntType() && lo->op() == Bytecodes::_iand &&
        (lo->x()->as_Constant() || lo->y()->as_Constant())) {
        int constant = 0;
...
void RangeCheckEliminator::Visitor::do_ArithmeticOp(ArithmeticOp *ao) {
    ...
    if (ao->op() == Bytecodes::_irem) {
        ...
        if (((x->as_Constant() || y->as_Constant()) && (ao->op() == Bytecodes::_iadd)) ||
            (y->as_Constant() && ao->op() == Bytecodes::_isub)) {
...
// Update bound
void RangeCheckEliminator::update_bound(IntegerStack &pushed, Value v, Instruction::Condition cond, Value value,
                                        int constant) {
...
void
RangeCheckEliminator::add_access_indexed_info(InstructionList &indices, int idx, Value instruction, AccessIndexed *ai) {
...
void
RangeCheckEliminator::in_block_motion(BlockBegin *block, AccessIndexedList &accessIndexed, InstructionList &arrays) {
    ...
    for (int i = 0; i < arrays.length(); i++) {
        ...
        for (int j = 0; j < accessIndexed.length(); j++) {
            ...
            if (c != NULL) {
                ...
                while (ao != NULL && (ao->x()->as_Constant() || ao->y()->as_Constant()) &&
                       (ao->op() == Bytecodes::_iadd || ao->op() == Bytecodes::_isub)) {
                    c = ao->y()->as_Constant();
        ...
        if (_optimistic) {
            for (int i = 0; i < indices.length(); i++) {
                ...
                if (info->_list->length() > 2 && range_cond) {
                    ...
                    int bci = NOT_PRODUCT(first->printable_bci())
                    PRODUCT_ONLY(-1);
                    insert_position = predicate(upper_compare, Instruction::aeq, length_instr, state, insert_position,
                                                bci);
                    insert_position = predicate_cmp_with_const(lower_compare, Instruction::leq, -1, state,
                                                               insert_position);
...
bool RangeCheckEliminator::is_ok_for_deoptimization(Instruction *insert_position, Instruction *array_instr,
                                                    Instruction *length_instr, Instruction *lower_instr, int lower,
                                                    Instruction *upper_instr, int upper) {
    bool upper_check = true;
    assert(lower_instr || lower >= 0, "If no lower_instr present, lower must be greater 0");
    assert(!lower_instr || lower_instr->dominator_depth() <= insert_position->dominator_depth(),
           "Dominator depth must be smaller");
    assert(!upper_instr || upper_instr->dominator_depth() <= insert_position->dominator_depth(),
           "Dominator depth must be smaller");
    ...
    assert(!length_instr || length_instr->dominator_depth() <= insert_position->dominator_depth(),
           "Dominator depth must be smaller");
...
Instruction *
RangeCheckEliminator::predicate(Instruction *left, Instruction::Condition cond, Instruction *right, ValueStack *state,
                                Instruction *insert_position, int bci) {
...
Instruction *
RangeCheckEliminator::predicate_cmp_with_const(Instruction *instr, Instruction::Condition cond, int constant,
                                               ValueStack *state, Instruction *insert_position, int bci) {
...
Instruction *
RangeCheckEliminator::predicate_add(Instruction *left, int left_const, Instruction::Condition cond, Instruction *right,
                                    ValueStack *state, Instruction *insert_position, int bci) {
...
Instruction *
RangeCheckEliminator::predicate_add_cmp_with_const(Instruction *left, int left_const, Instruction::Condition cond,
                                                   int constant, ValueStack *state, Instruction *insert_position,
                                                   int bci) {
    Constant *const_instr = new Constant(new IntConstant(constant));
    insert_position = insert_after(insert_position, const_instr, bci);
...
void
RangeCheckEliminator::insert_deoptimization(ValueStack *state, Instruction *insert_position, Instruction *array_instr,
                                            Instruction *length_instr, Instruction *lower_instr, int lower,
                                            Instruction *upper_instr, int upper, AccessIndexed *ai) {
    assert(is_ok_for_deoptimization(insert_position, array_instr, length_instr, lower_instr, lower, upper_instr, upper),
           "should have been tested before");
    bool upper_check = !(upper_instr && upper_instr->as_ArrayLength() &&
                         upper_instr->as_ArrayLength()->array() == array_instr);

    int bci = NOT_PRODUCT(ai->printable_bci())
    PRODUCT_ONLY(-1);
    if (lower_instr) {
        ...
        if (lower == 0) {
            ...
            insert_position = predicate_add_cmp_with_const(lower_instr, lower, Instruction::lss, 0, state,
                                                           insert_position, bci);
        } else {
            assert(lower < 0, "");
            // Add 1
            ...
            insert_position = predicate_cmp_with_const(lower_instr, Instruction::leq, lower, state, insert_position,
                                                       bci);
        }
    }
    ...
    if (!upper_instr) {
        ...
        if (upper == 0) {
            ...
            // Compare for geq array.length
            insert_position = predicate_add(upper_instr, upper, Instruction::geq, length_instr, state, insert_position,
                                            bci);
        } else {
            assert(upper > 0, "");
            upper = -upper;
            // Compare for geq array.length
            insert_position = predicate_add(length_instr, upper, Instruction::leq, upper_instr, state, insert_position,
                                            bci);
        }
    }
}
...
void RangeCheckEliminator::add_if_condition(IntegerStack &pushed, Value x, Value y, Instruction::Condition condition) {
    ...
    if (c != NULL) {
        ...
        instr_value = NULL;
    } else if (ao != NULL && (!ao->x()->as_Constant() || !ao->y()->as_Constant()) &&
               ((ao->op() == Bytecodes::_isub && ao->y()->as_Constant()) || ao->op() == Bytecodes::_iadd)) {
...
void RangeCheckEliminator::process_access_indexed(BlockBegin *loop_header, BlockBegin *block, AccessIndexed *ai) {
    ...
    TRACE_RANGE_CHECK_ELIMINATION(
            tty->print_cr("Access indexed: index=%d length=%d", ai->index()->id(),
    ...
    if (ai->check_flag(Instruction::NeedsRangeCheckFlag)) {
        ...
        if (in_array_bound(index_bound, ai->array()) ||
            (index_bound && array_bound && index_bound->is_smaller(array_bound) && !index_bound->lower_instr() &&
             index_bound->lower() >= 0)) {
            TRACE_RANGE_CHECK_ELIMINATION(
                    tty->fill_to(block->dominator_depth() * 2);
                    tty->print_cr("Bounds check for instruction %d in block B%d can be fully eliminated!", ai->id(),
                                  ai->block()->block_id())
            );
            ...
            if (!loop_invariant(loop_header, array_instr)) {
                TRACE_RANGE_CHECK_ELIMINATION(
                        tty->fill_to(block->dominator_depth() * 2);
                        tty->print_cr("Array %d is not loop invariant to header B%d", ai->array()->id(),
            ...
            TRACE_RANGE_CHECK_ELIMINATION(
                    tty->fill_to(block->dominator_depth() * 2);
                    tty->print_cr("LOOP INVARIANT access indexed %d found in block B%d!", ai->id(),
                                  ai->block()->block_id())
            );
            ...
            TRACE_RANGE_CHECK_ELIMINATION(
                    tty->fill_to(block->dominator_depth() * 2);
                    tty->print_cr("Inserting deopt at bci %d in block B%d!", state->bci(),
                                  insert_position->block()->block_id())
            );

            if (!is_ok_for_deoptimization(insert_position, array_instr, length_instr, lower_instr, index_bound->lower(),
                                          upper_instr, index_bound->upper())) {
                TRACE_RANGE_CHECK_ELIMINATION(
                        tty->fill_to(block->dominator_depth() * 2);
            ...
            insert_deoptimization(state, insert_position, array_instr, length_instr, lower_instr, index_bound->lower(),
                                  upper_instr, index_bound->upper(), ai);
...
void RangeCheckEliminator::calc_bounds(BlockBegin *block, BlockBegin *loop_header) {
    ...
    assert(!loop_header || loop_header->is_set(BlockBegin::linear_scan_loop_header_flag),
           "Loop header has to be real !");
    ...
    for (int i = 0; i < block->dominates()->length(); i++) {
        ...
        if (!next->is_set(BlockBegin::donot_eliminate_range_checks)) {
            ...
            if (block->is_set(BlockBegin::linear_scan_loop_header_flag) &&
                (block->loop_index() == next->loop_index() || next->loop_depth() > block->loop_depth())) {
    ...
    for (int i = 0; i < _ir->linear_scan_order()->length(); i++) {
        ...
        if (bound_stack && bound_stack->length() > 0) {
            ...
            if ((bound->has_lower() || bound->has_upper()) &&
                (bound->lower_instr() != phi || bound->upper_instr() != phi || bound->lower() != 0 ||
                 bound->upper() != 0)) {
        ...
        while (!instr->as_BlockEnd()) {
            if (instr->id() < _bounds.length()) {
                ...
                if (bound_stack && bound_stack->length() > 0) {
                    Bound *bound = bound_stack->top();
                    if ((bound->has_lower() || bound->has_upper()) &&
                        (bound->lower_instr() != instr || bound->upper_instr() != instr || bound->lower() != 0 ||
                         bound->upper() != 0)) {
                        TRACE_RANGE_CHECK_ELIMINATION(tty->fill_to(2 * block->dominator_depth());
...
RangeCheckEliminator::Bound::Bound(int lower, Value lower_instr, int upper, Value upper_instr) {
    assert(!lower_instr || !lower_instr->as_Constant() || !lower_instr->type()->as_IntConstant(),
           "Must not be constant!");
    assert(!upper_instr || !upper_instr->as_Constant() || !upper_instr->type()->as_IntConstant(),
           "Must not be constant!");
    this->_lower = lower;
    this->_upper = upper;
...
RangeCheckEliminator::Bound::Bound(Instruction::Condition cond, Value v, int constant) {
    assert(!v || (v->type() && (v->type()->as_IntType() || v->type()->as_ObjectType())),
           "Type must be array or integer!");
