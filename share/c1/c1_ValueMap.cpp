...
ValueMap::ValueMap()
        : _nesting(0), _entries(ValueMapInitialSize, ValueMapInitialSize, NULL), _killed_values(), _entry_count(0) {
    NOT_PRODUCT(reset_statistics());
}


ValueMap::ValueMap(ValueMap *old)
        : _nesting(old->_nesting + 1), _entries(old->_entries.length(), old->_entries.length(), NULL), _killed_values(),
...
Value ValueMap::find_insert(Value x) {
    ...
    if (hash != 0) {
        ...
        for (ValueMapEntry *entry = entry_at(entry_index(hash, size())); entry != NULL; entry = entry->next()) {
            if (entry->hash() == hash) {
                ...
                if (!is_killed(f) && f->is_equal(x)) {
                    ...
                    TRACE_VALUE_NUMBERING(tty->print_cr(
                            "Value Numbering: %s %c%d equal to %c%d  (size %d, entries %d, nesting-diff %d)", x->name(),
                            x->type()->tchar(), x->id(), f->type()->tchar(), f->id(), size(), entry_count(),
        ...
        TRACE_VALUE_NUMBERING(
                tty->print_cr("Value Numbering: insert %s %c%d  (size %d, entries %d, nesting %d)", x->name(),
                              x->type()->tchar(), x->id(), size(), entry_count(), nesting()));
    }
...
void ValueMap::print() {
    ...
    for (int i = 0; i < size(); i++) {
        if (entry_at(i) != NULL) {
            ...
            for (ValueMapEntry *entry = entry_at(i); entry != NULL; entry = entry->next()) {
                Value value = entry->value();
                tty->print("%s %c%d (%s%d) -> ", value->name(), value->type()->tchar(), value->id(),
                           is_killed(value) ? "x" : "", entry->nesting());
                entries++;
            }
            tty->print_cr("NULL");
        }
...
void ValueMap::print_statistics() {
    ...
    tty->print_cr("finds:%3d  hits:%3d   kills:%3d  hit rate: %1.4f", _number_of_finds, _number_of_hits,
                  _number_of_kills, hit_rate);
}
...
private:
    ...
    void kill_array(ValueType *type) {
        ...
        assert(basic_type >= 0 && basic_type < T_VOID, "Invalid type");
        _has_indexed_store[basic_type] = true;
    }

public:
    ShortLoopOptimizer(GlobalValueNumbering *gvn)
            : _gvn(gvn), _loop_blocks(ValueMapMaxLoopSize), _too_complicated_loop(false) {
...
public:
    LoopInvariantCodeMotion(ShortLoopOptimizer *slo, GlobalValueNumbering *gvn, BlockBegin *loop_header,
                            BlockList *loop_blocks);
};

LoopInvariantCodeMotion::LoopInvariantCodeMotion(ShortLoopOptimizer *slo, GlobalValueNumbering *gvn,
                                                 BlockBegin *loop_header, BlockList *loop_blocks)
...
void LoopInvariantCodeMotion::process_block(BlockBegin *block) {
    ...
    while (cur != NULL) {
        ...
        if (cur->as_Constant() != NULL) {
            ...
            cur_invariant = !lf->needs_patching() && !lf->field()->is_volatile() &&
                            !_short_loop_optimizer->has_field_store(lf->field()->type()->basic_type()) &&
                            is_invariant(lf->obj()) && _insert_is_pred;
            ...
            cur_invariant =
                    !_short_loop_optimizer->has_indexed_store(as_BasicType(cur->type())) && is_invariant(li->array()) &&
                    is_invariant(li->index()) && _insert_is_pred;
...
GlobalValueNumbering::GlobalValueNumbering(IR *ir)
        : _compilation(ir->compilation()), _current_map(NULL),
    ...
    assert(start_block == ir->start() && start_block->number_of_preds() == 0 && start_block->dominator() == NULL,
           "must be start block");
    assert(start_block->next()->as_Base() != NULL && start_block->next()->next() == NULL,
           "start block must not have instructions");
...
void GlobalValueNumbering::substitute(Instruction *instr) {
    ...
    if (subst != instr) {
        ...
        TRACE_VALUE_NUMBERING(tty->print_cr("substitution for %c%d set to %c%d", instr->type()->tchar(), instr->id(),
                                            subst->type()->tchar(), subst->id()));
