...
LIR_Opr LIR_OprFact::value_type(ValueType *type) {
    ...
    switch (tag) {
        ...
        case addressTag:
            return LIR_OprFact::addressConst(type->as_AddressConstant()->value());
        case intTag    :
            return LIR_OprFact::intConst(type->as_IntConstant()->value());
        case floatTag  :
            return LIR_OprFact::floatConst(type->as_FloatConstant()->value());
        case longTag   :
            return LIR_OprFact::longConst(type->as_LongConstant()->value());
        case doubleTag :
            return LIR_OprFact::doubleConst(type->as_DoubleConstant()->value());
        default:
            ShouldNotReachHere();
...
LIR_Address::Scale LIR_Address::scale(BasicType type) {
    ...
    switch (elem_size) {
        ...
        case 2:
            return LIR_Address::times_2;
        case 4:
            return LIR_Address::times_4;
        case 8:
            return LIR_Address::times_8;
    }
    ShouldNotReachHere();
    return LIR_Address::times_1;
}
...
#else
        : LIR_Op(lir_cond_float_branch, LIR_OprFact::illegalOpr, (CodeEmitInfo *) NULL), _cond(cond)
#endif
        , _label(block->label()), _block(block), _ublock(ublock), _stub(NULL) {
}
...
void LIR_OpBranch::negate_cond() {
    switch (cond()) {
        case lir_cond_equal:
            set_cond(lir_cond_notEqual);
            break;
        case lir_cond_notEqual:
            set_cond(lir_cond_equal);
            break;
        case lir_cond_less:
            set_cond(lir_cond_greaterEqual);
            break;
        case lir_cond_lessEqual:
            set_cond(lir_cond_greater);
            break;
        case lir_cond_greaterEqual:
            set_cond(lir_cond_less);
            break;
        case lir_cond_greater:
            set_cond(lir_cond_lessEqual);
            break;
        default:
            ShouldNotReachHere();
    }
...
LIR_OpTypeCheck::LIR_OpTypeCheck(LIR_Code code, LIR_Opr result, LIR_Opr object, ciKlass *klass,
        ...
        : LIR_Op(code, result, NULL), _object(object), _array(LIR_OprFact::illegalOpr), _klass(klass), _tmp1(tmp1),
          _tmp2(tmp2), _tmp3(tmp3), _fast_check(fast_check), _info_for_patch(info_for_patch),
          _info_for_exception(info_for_exception), _stub(stub), _profiled_method(NULL), _profiled_bci(-1),
...
LIR_OpTypeCheck::LIR_OpTypeCheck(LIR_Code code, LIR_Opr object, LIR_Opr array, LIR_Opr tmp1, LIR_Opr tmp2, LIR_Opr tmp3,
                                 CodeEmitInfo *info_for_exception)
        : LIR_Op(code, LIR_OprFact::illegalOpr, NULL), _object(object), _array(array), _klass(NULL), _tmp1(tmp1),
          _tmp2(tmp2), _tmp3(tmp3), _fast_check(false), _info_for_patch(NULL), _info_for_exception(info_for_exception),
          _stub(NULL), _profiled_method(NULL), _profiled_bci(-1), _should_profile(false) {
...
LIR_OpArrayCopy::LIR_OpArrayCopy(LIR_Opr src, LIR_Opr src_pos, LIR_Opr dst, LIR_Opr dst_pos, LIR_Opr length,
        ...
        : LIR_Op(lir_arraycopy, LIR_OprFact::illegalOpr, info), _src(src), _src_pos(src_pos), _dst(dst),
          _dst_pos(dst_pos), _length(length), _tmp(tmp), _expected_type(expected_type), _flags(flags) {
    _stub = new ArrayCopyStub(this);
}

LIR_OpUpdateCRC32::LIR_OpUpdateCRC32(LIR_Opr crc, LIR_Opr val, LIR_Opr res)
        : LIR_Op(lir_updatecrc32, res, NULL), _crc(crc), _val(val) {
}
...
// LIR_OpAllocObj
        case lir_alloc_object: {
            ...
            if (opAllocObj->_opr->is_valid()) {
                do_input(opAllocObj->_opr);
...
// LIR_Op2
        ...
        case lir_unwind: {
            ...
            assert(op1->_opr->is_valid(), "exception oop");
            do_input(op1->_opr);
...
// LIR_OpArrayCopy
        case lir_arraycopy: {
            ...
            assert(opArrayCopy->_src->is_valid(), "used");
            do_input(opArrayCopy->_src);
            do_temp(opArrayCopy->_src);
            assert(opArrayCopy->_src_pos->is_valid(), "used");
            do_input(opArrayCopy->_src_pos);
            do_temp(opArrayCopy->_src_pos);
            assert(opArrayCopy->_dst->is_valid(), "used");
            do_input(opArrayCopy->_dst);
            do_temp(opArrayCopy->_dst);
            assert(opArrayCopy->_dst_pos->is_valid(), "used");
            do_input(opArrayCopy->_dst_pos);
            do_temp(opArrayCopy->_dst_pos);
            assert(opArrayCopy->_length->is_valid(), "used");
            do_input(opArrayCopy->_length);
            do_temp(opArrayCopy->_length);
            assert(opArrayCopy->_tmp->is_valid(), "used");
...
// LIR_OpUpdateCRC32
        case lir_updatecrc32: {
            ...
            assert(opUp->_crc->is_valid(), "used");
            do_input(opUp->_crc);
            do_temp(opUp->_crc);
            assert(opUp->_val->is_valid(), "used");
            do_input(opUp->_val);
            do_temp(opUp->_val);
            assert(opUp->_result->is_valid(), "used");
...
// LIR_OpLock
        ...
        case lir_unlock: {
            ...
            assert(opLock->_lock->is_valid(), "used");
            do_temp(opLock->_lock);
            assert(opLock->_hdr->is_valid(), "used");
            do_temp(opLock->_hdr);
            assert(opLock->_obj->is_valid(), "used");
            do_temp(opLock->_obj);
...
// LIR_OpProfileCall:
        case lir_profile_call: {
            ...
            assert(opProfileCall->_mdo->is_valid(), "used");
            do_temp(opProfileCall->_mdo);
            assert(opProfileCall->_tmp1->is_valid(), "used");
...
// LIR_OpProfileType:
        case lir_profile_type: {
            ...
            do_input(opProfileType->_mdp);
            do_temp(opProfileType->_mdp);
...
LIR_List::LIR_List(Compilation *compilation, BlockBegin *block)
        : _operations(8), _compilation(compilation)
#ifndef PRODUCT
        , _block(block)
#endif
#ifdef ASSERT
...
void LIR_List::volatile_load_unsafe_reg(LIR_Opr base, LIR_Opr offset, LIR_Opr dst, BasicType type, CodeEmitInfo *info,
...
void LIR_List::store_mem_int(jint v, LIR_Opr base, int offset_in_bytes, BasicType type, CodeEmitInfo *info,
                             LIR_PatchCode patch_code) {
    append(new LIR_Op1(
            lir_move,
...
void LIR_List::store_mem_oop(jobject o, LIR_Opr base, int offset_in_bytes, BasicType type, CodeEmitInfo *info,
                             LIR_PatchCode patch_code) {
    append(new LIR_Op1(
            lir_move,
...
void LIR_List::volatile_store_unsafe_reg(LIR_Opr src, LIR_Opr base, LIR_Opr offset, BasicType type, CodeEmitInfo *info,
                                         LIR_PatchCode patch_code) {
    append(new LIR_Op1(
            lir_move,
            src,
...
void LIR_List::allocate_array(LIR_Opr dst, LIR_Opr len, LIR_Opr t1, LIR_Opr t2, LIR_Opr t3, LIR_Opr t4, BasicType type,
...
void
LIR_List::lock_object(LIR_Opr hdr, LIR_Opr obj, LIR_Opr lock, LIR_Opr scratch, CodeStub *stub, CodeEmitInfo *info) {
...
void LIR_List::instanceof(LIR_Opr result, LIR_Opr object, ciKlass *klass, LIR_Opr tmp1, LIR_Opr tmp2, LIR_Opr tmp3,
                          bool fast_check, CodeEmitInfo *info_for_patch, ciMethod *profiled_method, int profiled_bci) {
    LIR_OpTypeCheck *c = new LIR_OpTypeCheck(lir_instanceof, result, object, klass, tmp1, tmp2, tmp3, fast_check, NULL,
                                             info_for_patch, NULL);
    if (profiled_method != NULL) {
        c->set_profiled_method(profiled_method);
        c->set_profiled_bci(profiled_bci);
...
void LIR_Const::print_value_on(outputStream *out) const {
    switch (type()) {
        case T_ADDRESS:
            out->print("address:%d", as_jint());
            break;
        case T_INT:
            out->print("int:%d", as_jint());
            break;
        case T_LONG:
            out->print("lng:"
            JLONG_FORMAT, as_jlong());
            break;
        case T_FLOAT:
            out->print("flt:%f", as_jfloat());
            break;
        case T_DOUBLE:
            out->print("dbl:%f", as_jdouble());
            break;
        case T_OBJECT:
            out->print("obj:"
            INTPTR_FORMAT, p2i(as_jobject()));
            break;
        case T_METADATA:
            out->print("metadata:"
            INTPTR_FORMAT, p2i(as_metadata()));
            break;
        default:
            out->print("%3d:0x"
            UINT64_FORMAT_X, type(), (uint64_t) as_jlong());
            break;
    }
}
...
void print_LIR(BlockList *blocks) {
    ...
    for (i = 0; i < blocks->length(); i++) {
        ...
        print_block(bb);
        tty->print("__id_Instruction___________________________________________");
        tty->cr();
...
const char *LIR_Op::name() const {
    ...
    switch (code()) {
        ...
        case lir_on_spin_wait:
            s = "on_spin_wait";
            break;
        case lir_backwardbranch_target:
            s = "backbranch";
            break;
        case lir_std_entry:
            s = "std_entry";
...
void LIR_Op1::print_patch_code(outputStream *out, LIR_PatchCode code) {
    switch (code) {
        ...
        case lir_patch_low:
            ...
            break;
        case lir_patch_high:
            out->print("[patch_high]");
            break;
        case lir_patch_normal:
            out->print("[patch_normal]");
            break;
        default:
            ShouldNotReachHere();
    }
...
void LIR_OpAllocObj::print_instr(outputStream *out) const {
    ...
    tmp4()->print(out);
    out->print(" ");
    out->print("[hdr:%d]", header_size());
    out->print(" ");
    out->print("[obj:%d]", object_size());
    out->print(" ");
    out->print("[lbl:"
    INTPTR_FORMAT
    "]", p2i(stub()->entry()));
...
void LIR_Op2::print_instr(outputStream *out) const {
    ...
    in_opr2()->print(out);
    out->print(" ");
    if (tmp1_opr()->is_valid()) {
        tmp1_opr()->print(out);
        out->print(" ");
    }
    if (tmp2_opr()->is_valid()) {
        tmp2_opr()->print(out);
        out->print(" ");
    }
    if (tmp3_opr()->is_valid()) {
        tmp3_opr()->print(out);
        out->print(" ");
    }
    if (tmp4_opr()->is_valid()) {
        tmp4_opr()->print(out);
        out->print(" ");
    }
    if (tmp5_opr()->is_valid()) {
        tmp5_opr()->print(out);
        out->print(" ");
    }
    result_opr()->print(out);
...
void LIR_OpProfileType::print_instr(outputStream *out) const {
    ...
    out->print(" current = ");
    ciTypeEntries::print_ciklass(out, current_klass());
    out->print(" ");
    mdp()->print(out);
    out->print(" ");
    obj()->print(out);
