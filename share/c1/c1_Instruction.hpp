...
public:
    ...
    Instruction(ValueType *type, ValueStack *state_before = NULL, bool type_is_constant = false)
            : _id(Compilation::current()->get_next_id()),
              ...
              _printable_bci(-99),
#endif
              _use_count(0), _pin_state(0), _type(type), _next(NULL), _subst(NULL), _operand(LIR_OprFact::illegalOpr),
              _flags(0), _state_before(state_before), _exception_handlers(NULL), _block(NULL) {
    ...
    int printable_bci() const {
        assert(has_printable_bci(), "_printable_bci should have been set");
    ...
    void set_exception_state(ValueStack *s) {
        check_state(s);
        _exception_state = s;
    }

    void set_state_before(ValueStack *s) {
        check_state(s);
        _state_before = s;
    }
...
public:
    ...
    Phi(ValueType *type, BlockBegin *b, int index)
            : Instruction(type->base()), _pf_flags(0), _index(index) {
        _block = b;
...
public:
    ...
    Local(ciType *declared, ValueType *type, int index, bool receiver)
            : Instruction(type), _java_index(index), _is_receiver(receiver), _declared_type(declared) {
...
public:
    ...
    enum CompareResult {
        not_comparable = -1, cond_false, cond_true
    };
...
public:
    ...
    AccessField(Value obj, int offset, ciField *field, bool is_static,
            ...
            : Instruction(as_ValueType(field->type()->basic_type()), state_before), _obj(obj), _offset(offset),
              _field(field), _explicit_null_check(NULL) {
...
public:
    ...
    StoreField(Value obj, int offset, ciField *field, Value value, bool is_static,
               ValueStack *state_before, bool needs_patching)
            : AccessField(obj, offset, field, is_static, state_before, needs_patching), _value(value) {
...
public:
    ...
    AccessArray(ValueType *type, Value array, ValueStack *state_before)
            : Instruction(type, state_before), _array(array) {
...
public:
    ...
    ArrayLength(Value array, ValueStack *state_before)
            : AccessArray(intType, array, state_before), _explicit_null_check(NULL) {}
...
public:
    ...
    AccessIndexed(Value array, Value index, Value length, BasicType elt_type, ValueStack *state_before, bool mismatched)
            : AccessArray(as_ValueType(elt_type), array, state_before), _index(index), _length(length),
              _elt_type(elt_type), _mismatched(mismatched) {
    ...
    // generic
    virtual void input_values_do(ValueVisitor *f) {
        AccessArray::input_values_do(f);
        f->visit(&_index);
        if (_length != NULL) f->visit(&_length);
    }
};
...
public:
    ...
    LoadIndexed(Value array, Value index, Value length, BasicType elt_type, ValueStack *state_before,
            ...
            : AccessIndexed(array, index, length, elt_type, state_before, mismatched), _explicit_null_check(NULL) {}
...
public:
    ...
    StoreIndexed(Value array, Value index, Value length, BasicType elt_type, Value value, ValueStack *state_before,
            ...
            : AccessIndexed(array, index, length, elt_type, state_before, mismatched), _value(value),
              _profiled_method(NULL), _profiled_bci(0), _check_boolean(check_boolean) {
...
public:
    ...
    Op2(ValueType *type, Bytecodes::Code op, Value x, Value y, ValueStack *state_before = NULL)
            : Instruction(type, state_before), _op(op), _x(x), _y(y) {
        ASSERT_VALUES
    }
...
public:
    ...
    IfOp(Value x, Condition cond, Value y, Value tval, Value fval)
            : Op2(tval->type()->meet(fval->type()), (Bytecodes::Code) cond, x, y), _tval(tval), _fval(fval) {
        ASSERT_VALUES
...
public:
    ...
    NullCheck(Value obj, ValueStack *state_before)
            : Instruction(obj->type()->base(), state_before), _obj(obj) {
        ASSERT_VALUES
        set_can_trap(true);
...
public:
    ...
    StateSplit(ValueType *type, ValueStack *state_before = NULL)
            : Instruction(type, state_before), _state(NULL) {
...
public:
    ...
    NewArray(Value length, ValueStack *state_before)
            : StateSplit(objectType, state_before), _length(length) {
...
public:
    // creation
    NewObjectArray(ciKlass *klass, Value length, ValueStack *state_before) : NewArray(length, state_before),
                                                                             _klass(klass) {}
...
public:
    // creation
    NewMultiArray(ciKlass *klass, Values *dims, ValueStack *state_before) : NewArray(NULL, state_before), _klass(klass),
                                                                            _dims(dims) {
        ASSERT_VALUES
    }
...
public:
    ...
    AccessMonitor(Value obj, int monitor_no, ValueStack *state_before = NULL)
            : StateSplit(illegalType, state_before), _obj(obj), _monitor_no(monitor_no) {
        set_needs_null_check(true);
        ASSERT_VALUES
    }
...
public:
    ...
    Intrinsic(ValueType *type,
              ...
              bool cantrap = true)
            : StateSplit(type, state_before), _id(id), _args(args), _recv(NULL) {
...
public:
    ...
    // creation
    BlockBegin(int bci)
            : StateSplit(illegalType), _block_id(Compilation::current()->get_next_block_id()), _bci(bci),
              _depth_first_number(-1), _linear_scan_number(-1), _dominator_depth(-1), _loop_depth(0), _loop_index(-1),
              _flags(0), _total_preds(0), _stores_to_locals(), _successors(2), _predecessors(2), _dominates(2),
              _dominator(NULL), _end(NULL), _exception_handlers(1), _exception_states(NULL), _exception_handler_pco(-1),
              _lir(NULL), _live_in(), _live_out(), _live_gen(), _live_kill(), _fpu_register_usage(),
              _fpu_stack_state(NULL), _first_lir_instruction_id(-1), _last_lir_instruction_id(-1) {
        _block = this;
#ifndef PRODUCT
    ...
    enum Flag {
        ...
        parser_loop_header_flag =
        1 << 7,  // set by parser to identify blocks where phi functions can not be created on demand
    ...
    void merge(ValueStack *state) {
        bool b = try_merge(state);
...
public:
    ...
    BlockEnd(ValueType *type, ValueStack *state_before, bool is_safepoint)
            : StateSplit(type, state_before), _sux(NULL) {
...
public:
    ...
    Goto(BlockBegin *sux, ValueStack *state_before, bool is_safepoint = false)
            : BlockEnd(illegalType, state_before, is_safepoint), _profiled_method(NULL), _profiled_bci(0),
              _direction(none) {
    ...
    Goto(BlockBegin *sux, bool is_safepoint) : BlockEnd(illegalType, NULL, is_safepoint), _profiled_method(NULL),
                                               _profiled_bci(0), _direction(none) {
        BlockList *s = new BlockList(1);
        s->append(sux);
        set_sux(s);
    }
...
public:
    // creation
    // unordered_is_true is valid for float/double compares only
    RangeCheckPredicate(Value x, Condition cond, bool unordered_is_true, Value y, ValueStack *state) : StateSplit(
            illegalType), _x(x), _cond(cond), _y(y) {
        ASSERT_VALUES
...
public:
    // creation
    // unordered_is_true is valid for float/double compares only
    If(Value x, Condition cond, bool unordered_is_true, Value y, BlockBegin *tsux, BlockBegin *fsux,
       ValueStack *state_before, bool is_safepoint)
            : BlockEnd(illegalType, state_before, is_safepoint), _x(x), _cond(cond), _y(y), _profiled_method(NULL),
              _profiled_bci(0), _swapped(false) {
        ASSERT_VALUES
        set_flag(UnorderedIsTrueFlag, unordered_is_true);
        assert(x->type()->tag() == y->type()->tag(), "types must match");
...
public:
    ...
    Switch(Value tag, BlockList *sux, ValueStack *state_before, bool is_safepoint)
            : BlockEnd(illegalType, state_before, is_safepoint), _tag(tag) {
        ASSERT_VALUES
        set_sux(sux);
    }
...
public:
    ...
    LookupSwitch(Value tag, BlockList *sux, intArray *keys, ValueStack *state_before, bool is_safepoint)
            : Switch(tag, sux, state_before, is_safepoint), _keys(keys) {
...
public:
    // creation
    Throw(Value exception, ValueStack *state_before) : BlockEnd(illegalType, state_before, true),
...
protected:
    ...
    UnsafeOp(BasicType basic_type, bool is_put)
            : Instruction(is_put ? voidType : as_ValueType(basic_type)), _basic_type(basic_type) {
...
protected:
    UnsafeRawOp(BasicType basic_type, Value addr, bool is_put)
            : UnsafeOp(basic_type, is_put), _base(addr), _index(NULL), _log2_scale(0) {
    ...
    UnsafeRawOp(BasicType basic_type, Value base, Value index, int log2_scale, bool is_put)
            : UnsafeOp(basic_type, is_put), _base(base), _index(index), _log2_scale(log2_scale) {
    }
...
public:
    UnsafePutObject(BasicType basic_type, Value object, Value offset, Value value, bool is_volatile)
            : UnsafeObjectOp(basic_type, object, offset, true, is_volatile), _value(value) {
        ASSERT_VALUES
    }
...
public:
    UnsafeGetAndSetObject(BasicType basic_type, Value object, Value offset, Value value, bool is_add)
            : UnsafeObjectOp(basic_type, object, offset, false, false), _value(value), _is_add(is_add) {
        ASSERT_VALUES
    }
...
public:
    ProfileCall(ciMethod *method, int bci, ciMethod *callee, Value recv, ciKlass *known_holder, Values *obj_args,
                bool inlined)
            : Instruction(voidType), _method(method), _bci_of_invoke(bci), _callee(callee), _recv(recv),
              _known_holder(known_holder), _obj_args(obj_args), _inlined(inlined) {
...
public:
    ProfileReturnType(ciMethod *method, int bci, ciMethod *callee, Value ret)
            : Instruction(voidType), _method(method), _callee(callee), _bci_of_invoke(bci), _ret(ret) {
        set_needs_null_check(true);
...
public:
    RuntimeCall(ValueType *type, const char *entry_name, address entry, Values *args, bool pass_thread = true)
            : Instruction(type), _entry_name(entry_name), _entry(entry), _args(args), _pass_thread(pass_thread) {
        ASSERT_VALUES
        pin();
    }
...
public:
    ProfileInvoke(ciMethod *inlinee, ValueStack *state)
            : Instruction(voidType), _inlinee(inlinee), _state(state) {
...
public:
    MemBar(LIR_Code code)
            : Instruction(voidType), _code(code) {
        pin();
    }
...
inline BlockBegin *BlockBegin::sux_at(int i) const {
    assert(_end == NULL || _end->sux_at(i) == _successors.at(i), "mismatch");
    return _successors.at(i);
}

inline void BlockBegin::add_successor(BlockBegin *sux) {
    assert(_end == NULL, "Would create mismatch with successors of BlockEnd");
