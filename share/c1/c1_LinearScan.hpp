...
private:
    ...
    bool is_block_begin(int op_id) {
        return op_id == 0 || block_of_op_with_id(op_id) != block_of_op_with_id(op_id - 1);
    }
    ...
    void create_unhandled_lists(Interval **list1, Interval **list2, bool (is_list1)(const Interval *i),
    ...
    void
    resolve_exception_edge(XHandler *handler, int throwing_op_id, int reg_num, Phi *phi, MoveResolver &move_resolver);
    ...
    IRScopeDebugInfo *
    compute_debug_info_for_scope(int op_id, IRScope *cur_scope, ValueStack *cur_state, ValueStack *innermost_state);
...
public:
    ...
    int to() {
        if (_cached_to == -1) _cached_to = calc_to();
    ...
    void assign_reg(int reg) {
        _assigned_reg = reg;
        _assigned_regHi = LinearScan::any_reg;
    }

    void assign_reg(int reg, int regHi) {
        _assigned_reg = reg;
        _assigned_regHi = regHi;
    }
    ...
    bool always_in_memory() const {
        return split_parent()->_spill_state == storeAtDefinition || split_parent()->_spill_state == startInMemory;
    }
    ...
    int first_usage(
            IntervalUseKind min_use_kind) const;           // id of the first operation requiring this interval in a register
...
protected:
    ...
    Interval **unhandled_first_addr(IntervalKind kind) {
        check_bounds(kind);
        return &_unhandled_first[kind];
    }

    Interval **active_first_addr(IntervalKind kind) {
        check_bounds(kind);
        return &_active_first[kind];
    }

    Interval **inactive_first_addr(IntervalKind kind) {
        check_bounds(kind);
        return &_inactive_first[kind];
    }
...
public:
    ...
    Interval *unhandled_first(IntervalKind kind) {
        check_bounds(kind);
        return _unhandled_first[kind];
    }

    Interval *active_first(IntervalKind kind) {
        check_bounds(kind);
        return _active_first[kind];
    }

    Interval *inactive_first(IntervalKind kind) {
        check_bounds(kind);
        return _inactive_first[kind];
    }
