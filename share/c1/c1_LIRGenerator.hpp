...
public:
    ResolveNode(LIR_Opr operand)
            : _operand(operand), _assigned(false), _visited(false), _start_node(false) {};
...
public:
    LIR_Opr call_runtime(BasicTypeArray *signature, LIRItemList *args, address entry, ValueType *result_type,
                         CodeEmitInfo *info);

    LIR_Opr call_runtime(BasicTypeArray *signature, LIR_OprList *args, address entry, ValueType *result_type,
                         CodeEmitInfo *info);
...
public:
    ...
    void array_store_check(LIR_Opr value, LIR_Opr array, CodeEmitInfo *store_check_info, ciMethod *profiled_method,
                           int profiled_bci);
    ...
    void arithmetic_op(Bytecodes::Code code, LIR_Opr result, LIR_Opr left, LIR_Opr right, LIR_Opr tmp,
    ...
    void
    arithmetic_op_long(Bytecodes::Code code, LIR_Opr result, LIR_Opr left, LIR_Opr right, CodeEmitInfo *info = NULL);

    void arithmetic_op_fpu(Bytecodes::Code code, LIR_Opr result, LIR_Opr left, LIR_Opr right,
    ...
    void monitor_enter(LIR_Opr object, LIR_Opr lock, LIR_Opr hdr, LIR_Opr scratch, int monitor_no,
    ...
    void new_instance(LIR_Opr dst, ciInstanceKlass *klass, bool is_unresolved, LIR_Opr scratch1, LIR_Opr scratch2,
    ...
    bool can_inline_as_constant(Value i S390_ONLY(COMMA int bits = 20)
    ...
    void increment_invocation_counter(CodeEmitInfo *info) {
        if (compilation()->count_invocations()) {
            increment_event_counter(info, LIR_OprFact::intConst(InvocationCounter::count_increment), InvocationEntryBci,
                                    false);
        }
    }
    ...
    void increment_backedge_counter_conditionally(LIR_Condition cond, LIR_Opr left, LIR_Opr right, CodeEmitInfo *info,
                                                  int left_bci, int right_bci, int bci);
    ...
    static LIR_Condition lir_cond(If::Condition cond) {
        ...
        switch (cond) {
            ...
            case If::lss:
                ...
                break;
            case If::leq:
                l = lir_cond_lessEqual;
                break;
            case If::geq:
                l = lir_cond_greaterEqual;
                break;
            case If::gtr:
                l = lir_cond_greater;
                break;
            case If::aeq:
                l = lir_cond_aboveEqual;
                break;
            case If::beq:
                l = lir_cond_belowEqual;
                break;
            default:
                fatal("You must pass valid If::Condition");
        };
        return l;
    }
...
public:
    ...
    LIRGenerator(Compilation *compilation, ciMethod *method)
            : _compilation(compilation), _method(method), _virtual_register_number(LIR_OprDesc::vreg_base),
              _vreg_flags(num_vreg_flags), _barrier_set(BarrierSet::barrier_set()->barrier_set_c1()) {
    }
