...
address Runtime1::arraycopy_count_address(BasicType type) {
    switch (type) {
        case T_BOOLEAN:
        case T_BYTE:
            return (address) & _byte_arraycopy_stub_cnt;
        case T_CHAR:
        case T_SHORT:
            return (address) & _short_arraycopy_stub_cnt;
        case T_FLOAT:
        case T_INT:
            return (address) & _int_arraycopy_stub_cnt;
        case T_DOUBLE:
        case T_LONG:
            return (address) & _long_arraycopy_stub_cnt;
        case T_ARRAY:
        case T_OBJECT:
            return (address) & _oop_arraycopy_stub_cnt;
        default:
            ShouldNotReachHere();
            return NULL;
    }
...
CodeBlob *Runtime1::generate_blob(BufferBlob *buffer_blob, int stub_id, const char *name, bool expect_oop_map,
                                  StubAssemblerCodeGenClosure *cl) {
    ResourceMark rm;
...
JRT_ENTRY(void, Runtime1::new_instance(JavaThread
...
JRT_ENTRY(void, Runtime1::new_type_array(JavaThread
...
JRT_ENTRY(void, Runtime1::new_object_array(JavaThread

* current,
Klass *array_klass, jint
length))
...
JRT_ENTRY(void, Runtime1::new_multi_array(JavaThread
...
JRT_ENTRY(void, Runtime1::unimplemented_entry(JavaThread
...
JRT_ENTRY(void, Runtime1::throw_array_store_exception(JavaThread
...
static nmethod
*
counter_overflow_helper(JavaThread
* current,
int branch_bci, Method
* m) {
nmethod *osr_nm = NULL;
...
JRT_BLOCK_ENTRY(address, Runtime1::counter_overflow(JavaThread * current, int
bci,
Method *method
))
nmethod *osr_nm;
...
RegisterMap map(current, false);
frame fr = current->last_frame().sender(&map);
Deoptimization::deoptimize_frame(current, fr
.
...
JRT_ENTRY_NO_ASYNC(static address, exception_handler_for_pc_helper(JavaThread
...
==
Runtime1::blob_for(Runtime1::handle_exception_from_callee_id)
) {
...
"NULL exceptions should be handled by throw_exception");
// Check that exception is a subclass of Throwable
assert(exception
->

is_a(vmClasses::Throwable_klass()),
...
// tracing
if (
log_is_enabled(Info, exceptions
)) {
ResourceMark rm;
stringStream tempst;
assert(nm
...
!= NULL, "Unexpected NULL method()");
tempst.print("C1 compiled method <%s>\n"
" at PC" INTPTR_FORMAT " for thread " INTPTR_FORMAT,
nm->method()->print_value_string(),
p2i(pc), p2i(current)
);
...
Deoptimization::deoptimize_frame(current, caller_frame
.

id()

);
assert(caller_is_deopted(current),
"Must be deoptimized");
...
current->
set_is_method_handle_return(nm
->
is_method_handle_return(pc)
);
return
fast_continuation;
}
...
nm->
add_handler_for_exception_and_pc(exception, pc, continuation
);
}
}
...
ResourceMark rm;
log_info(exceptions)("Thread "
PTR_FORMAT " continuing at PC " PTR_FORMAT
...
address
Runtime1::exception_handler_for_pc(JavaThread
* current) {
...
if (nm !=
NULL &&caller_is_deopted(current)
) {
...
JRT_ENTRY(void, Runtime1::throw_range_check_exception(JavaThread

* current,
int index, arrayOopDesc
* a))
NOT_PRODUCT(_throw_range_check_exception_count
++;)
const int len = 35;
assert(len<strlen("Index %d out of bounds for length %d"),
"Must allocate more space for message.");
char message[2 * jintAsStringSize + len];
os::snprintf_checked(message,
sizeof(message), "Index %d out of bounds for length %d", index, a->
...
JRT_ENTRY(void, Runtime1::throw_index_exception(JavaThread
...
JRT_ENTRY(void, Runtime1::throw_div0_exception(JavaThread

* current))
NOT_PRODUCT(_throw_div0_exception_count
++;)
SharedRuntime::throw_and_post_jvmti_exception(current, vmSymbols::java_lang_ArithmeticException(),
"/ by zero");

JRT_END


JRT_ENTRY(void, Runtime1::throw_null_pointer_exception(JavaThread
...
JRT_ENTRY(void, Runtime1::throw_class_cast_exception(JavaThread
...
JRT_ENTRY(void, Runtime1::throw_incompatible_class_change_error(JavaThread
...
JRT_BLOCK_ENTRY(void, Runtime1::monitorenter(JavaThread

* current,
oopDesc *obj, BasicObjectLock
* lock))
...
"must match");
SharedRuntime::monitor_enter_helper(obj, lock
->
...
"last_Java_sp must be set");
oop obj = lock->obj();
assert(oopDesc::is_oop(obj),
"must be NULL or an object");
SharedRuntime::monitor_exit_helper(obj, lock
->
...
JRT_ENTRY(void, Runtime1::deoptimize(JavaThread
...
assert(nm
==
CodeCache::find_nmethod(caller_frame
.
...
resolve_field_return_klass(const methodHandle &caller, int bci, TRAPS) {
    ...
    LinkResolver::resolve_field_access(result, constants, field_access.index(), caller, Bytecodes::java_code(code),
                                       CHECK_NULL);
...
JRT_ENTRY(void, Runtime1::patch_code(JavaThread
...
LinkResolver::resolve_field_access(result, constants, field_access
.

index(), caller_method, Bytecodes::java_code(code), CHECK
...
if (TracePatching) {
ttyLocker ttyl;
tty->print_cr(" Patching %s at bci %d at address " INTPTR_FORMAT "  (%s)",
Bytecodes::name(code), bci,
        p2i(instr_pc), (stub_id
== Runtime1::access_field_patching_id) ? "field" : "klass");
nmethod *caller_code = CodeCache::find_nmethod(caller_frame.pc());
assert(caller_code
!= NULL, "nmethod not found");
...
Disassembler::decode(copy_buff, copy_buff
+ *byte_count, tty);
}
...
== 4 && (patch_field_type == T_DOUBLE || patch_field_type == T_LONG)), "illegal offset for type");
assert(patch_field_offset
>= 0, "illegal offset");
n_move->
add_offset_in_bytes(patch_field_offset);
} else if (load_klass_or_mirror_patch_id) {
...
assert(do_patch
== true, "initialization must be complete at this point");
} else {
...
n_copy->
set_data(cast_from_oop<intx>(mirror())
);
}
...
"illegal init value");
n_copy->
set_data(cast_from_oop<intx>(appendix())
);
...
nmethod *nm = CodeCache::find_nmethod(instr_pc);
assert(nm
!= NULL, "invalid nmethod_pc");

// The old patch site is now a move instruction so update
// the reloc info so that it will get updated during
// future GCs.
RelocIterator iter(nm, (address)
instr_pc, (address)(instr_pc + 1));
relocInfo::change_reloc_info_for_address(&iter, (address)
instr_pc,
...
nmethod *nm = CodeCache::find_nmethod(caller_frame.pc());
guarantee(nm
!= NULL, "only nmethods can contain non-perm oops");
...
JRT_LEAF(int, Runtime1::is_instance_of(oopDesc
...
assert(mirror
!= NULL, "should null-check on mirror before calling");
...
JRT_ENTRY(void, Runtime1::predicate_failed_trap(JavaThread
...
assert((PENDING_EXCEPTION
->

is_a (vmClasses::OutOfMemoryError_klass())
...
if (mdo != NULL) {
mdo->
inc_trap_count(Deoptimization::Reason_none);
}
...
inlinee->
print_short_name(&ss1);
m->
print_short_name(&ss2);
tty->print_cr("Predicate failed trap in method %s at bci %d inlined in %s at pc " INTPTR_FORMAT, ss1.
