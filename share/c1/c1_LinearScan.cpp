...
LinearScan::LinearScan(IR *ir, LIRGenerator *gen, FrameMap *frame_map)
        : _compilation(ir->compilation()), _ir(ir), _gen(gen), _frame_map(frame_map),
          _cached_blocks(*ir->linear_scan_order()), _num_virtual_regs(gen->max_virtual_register_number()),
          _has_fpu_registers(false), _num_calls(-1), _max_spills(0), _unused_spill_slot(-1),
          _intervals(0)   // initialized later with correct length
        , _new_intervals_from_allocation(NULL), _sorted_intervals(NULL), _needs_full_resort(false),
          _lir_ops(0)     // initialized later with correct length
        , _block_of_op(0) // initialized later with correct length
        , _has_info(0), _has_call(0), _interval_in_loop(0)  // initialized later with correct length
    ...
    switch (interval->spill_state()) {
        ...
        case oneDefinitionFound:
            ...
            if (def_pos < interval->spill_definition_pos() - 2) {
                ...
                assert(block_of_op_with_id(def_pos) == block_of_op_with_id(interval->spill_definition_pos()),
                       "block must be equal");
            }
            break;
        ...
        for (int j = 1; j < num_inst; j++) {
            ...
            if (op_id == -1) {
                ...
                if (interval->assigned_reg() >= LinearScan::nof_regs && interval->always_in_memory()) {
                    ...
                    TRACE_LINEAR_SCAN(4, tty->print_cr("eliminating move from interval %d to %d",
                                                       op1->in_opr()->vreg_number(), op1->result_opr()->vreg_number()));
                ...
                assert(interval == Interval::end() ||
                       (interval->is_split_parent() && interval->spill_state() == storeAtDefinition),
                       "invalid interval");
                ...
                while (interval != Interval::end() && interval->spill_definition_pos() == op_id) {
                    ...
                    TRACE_LINEAR_SCAN(4, tty->print_cr(
                            "inserting move after definition of interval %d to stack slot %d at op_id %d",
                            interval->reg_num(), interval->canonical_spill_slot() - LinearScan::nof_regs, op_id));
...
void LinearScan::set_live_gen_kill(Value value, LIR_Op *op, BitMap &live_gen, BitMap &live_kill) {
    ...
    assert(!value->type()->is_illegal(),
           "if this local is used by the interpreter it shouldn't be of indeterminate type");
    assert(con == NULL || opr->is_virtual() || opr->is_constant() || opr->is_illegal(),
           "asumption: Constant instructions have only constant operands");
    ...
    if ((con == NULL || con->is_pinned()) && opr->is_register()) {
        ...
        if (!live_kill.at(reg)) {
            ...
            TRACE_LINEAR_SCAN(4, tty->print_cr("  Setting live_gen for value %c%d, LIR op_id %d, register number %d",
    ...
    for (int i = 0; i < num_blocks; i++) {
        ...
        for (int j = 1; j < num_inst; j++) {
            ...
            for (k = 0; k < n; k++) {
                ...
                if (opr->is_virtual_register()) {
                    assert(reg_num(opr) == opr->vreg_number() && !is_valid_reg_num(reg_numHi(opr)),
                           "invalid optimization below");
                    reg = opr->vreg_number();
                    if (!live_kill.at(reg)) {
                        live_gen.set_bit(reg);
                        TRACE_LINEAR_SCAN(4, tty->print_cr("  Setting live_gen for register %d at instruction %d", reg,
                                                           op->id()));
                    }
...
void LinearScan::add_use(LIR_Opr opr, int from, int to, IntervalUseKind use_kind) {
    TRACE_LINEAR_SCAN(2,
                      tty->print(" use "); opr->print(tty); tty->print_cr(" from %d to %d (%d)", from, to, use_kind));
    assert(opr->is_register(), "should not be called otherwise");
...
void LinearScan::handle_method_arguments(LIR_Op *op) {
    ...
    if (op->code() == lir_move) {
      ...
      TRACE_LINEAR_SCAN(4, tty->print_cr("found move from stack slot %d to vreg %d", o->is_single_stack() ? o->single_stack_ix() : o->double_stack_ix(), reg_num(move->result_opr())));
            ...
            int stack_slot = LinearScan::nof_regs +
                             (move->in_opr()->is_single_stack() ? move->in_opr()->single_stack_ix()
        ...
        case lir_convert: {
            ...
            if (move_to->is_register() && move_from->is_register()) {
                ...
                if (from != NULL && to != NULL) {
                    ...
                    TRACE_LINEAR_SCAN(4, tty->print_cr("operation at op_id %d: added hint from interval %d to %d",
...
void LinearScan::build_intervals() {
    ...
    for (i = block_count() - 1; i >= 0; i--) {
        ...
        for (int number = (int) live.get_next_one_offset(0, size);
             number < size; number = (int) live.get_next_one_offset(number + 1, size)) {
...
void LinearScan::create_unhandled_lists(Interval **list1, Interval **list2, bool (is_list1)(const Interval *i),
                                        bool (is_list2)(const Interval *i)) {
...
void LinearScan::sort_intervals_after_allocation() {
    ...
    while (old_idx + new_idx < old_len + new_len) {
        if (new_idx >= new_len ||
            (old_idx < old_len && old_list->at(old_idx)->from() <= new_list->at(new_idx)->from())) {
...
Interval *LinearScan::interval_at_block_end(BlockBegin *block, int reg_num) {
    ...
    return split_child_at_op_id(interval_at(reg_num), block->last_lir_instruction_id() + 1,
...
void LinearScan::resolve_collect_mappings(BlockBegin *from_block, BlockBegin *to_block, MoveResolver &move_resolver) {
    ...
    for (int r = (int) live_at_edge.get_next_one_offset(0, size);
         r < size; r = (int) live_at_edge.get_next_one_offset(r + 1, size)) {
        ...
        if (from_interval != to_interval && (from_interval->assigned_reg() != to_interval->assigned_reg() ||
                                             from_interval->assigned_regHi() != to_interval->assigned_regHi())) {
...
void LinearScan::resolve_data_flow() {
    ...
    for (i = 0; i < num_blocks; i++) {
        ...
        if (block->number_of_preds() == 1 && block->number_of_sux() == 1 &&
            block->number_of_exception_handlers() == 0) {
            LIR_OpList *instructions = block->lir()->instructions_list();
            ...
            assert(instructions->last()->as_OpBranch()->cond() == lir_cond_always,
            ...
            if (instructions->length() == 2) {
                ...
                if (!block_completed.at(pred->linear_scan_number()) && !block_completed.at(sux->linear_scan_number())) {
                    TRACE_LINEAR_SCAN(3, tty->print_cr("**** optimizing empty block B%d (pred: B%d, sux: B%d)",
        ...
        if (!block_completed.at(i)) {
            ...
            for (int s = 0; s < num_sux; s++) {
                ...
                if (!already_resolved.at(to_block->linear_scan_number())) {
                    TRACE_LINEAR_SCAN(3,
                                      tty->print_cr("**** processing edge between B%d and B%d", from_block->block_id(),
                                                    to_block->block_id()));
...
void LinearScan::resolve_exception_entry(BlockBegin *block, MoveResolver &move_resolver) {
    ...
    for (int r = (int) block->live_in().get_next_one_offset(0, size);
         r < size; r = (int) block->live_in().get_next_one_offset(r + 1, size)) {
...
void LinearScan::resolve_exception_edge(XHandler *handler, int throwing_op_id, int reg_num, Phi *phi,
                                        MoveResolver &move_resolver) {
    if (interval_at(reg_num) == NULL) {
        // if a phi function is never used, no interval is created -> ignore this
        return;
    }
    ...
    if (phi != NULL) {
        ...
        if (from_interval != to_interval) {
            ...
            if (!from_interval->always_in_memory() ||
                from_interval->canonical_spill_slot() != to_interval->assigned_reg()) {
                move_resolver.add_mapping(from_interval, to_interval);
            }
        }
    }
...
void LinearScan::resolve_exception_edge(XHandler *handler, int throwing_op_id, MoveResolver &move_resolver) {
    TRACE_LINEAR_SCAN(4, tty->print_cr("resolving exception handler B%d: throwing_op_id=%d",
...
#else
                assert(assigned_reg >= pd_first_fpu_reg && assigned_reg <= pd_last_fpu_reg, "no fpu register");
                assert(interval->assigned_regHi() == any_reg,
...
LIR_Opr LinearScan::color_lir_opr(LIR_Opr opr, int op_id, LIR_OpVisitState::OprMode mode) {
  ...
  if (res->is_fpu_register()) {
    ...
    assert(!gen()->is_vreg_flag_set(opr->vreg_number(), LIRGenerator::callee_saved) ||
           !FrameMap::is_caller_save_register(res), "bad allocation");
...
OopMap *LinearScan::compute_oop_map(IntervalWalker *iw, LIR_Op *op, CodeEmitInfo *info, bool is_call_site) {
    ...
    for (Interval *interval = iw->active_first(fixedKind); interval != Interval::end(); interval = interval->next()) {
        ...
        assert(interval->current_from() <= op->id() && op->id() <= interval->current_to(),
               "interval should not be active otherwise");
        ...
        if (op->is_patching() || op->id() < interval->current_to()) {
            ...
            assert(!is_call_site || assigned_reg >= nof_regs || !is_caller_save(assigned_reg),
            ...
            if (interval->always_in_memory() &&
                ...
                assert(interval->assigned_reg() < LinearScan::nof_regs,
                       "interval is on stack, so stack slot is registered twice");
...
// use).
ConstantOopWriteValue *LinearScan::_oop_null_scope_value = new(ResourceObj::C_HEAP, mtCompiler) ConstantOopWriteValue(
        NULL);
...
int LinearScan::append_scope_value_for_constant(LIR_Opr opr, GrowableArray<ScopeValue *> *scope_values) {
    ...
    switch (t) {
        ...
        case T_FLOAT: {
            ...
            switch (value) {
                case -1:
                    scope_values->append(_int_m1_scope_value);
                    break;
                case 0:
                    scope_values->append(_int_0_scope_value);
                    break;
                case 1:
                    scope_values->append(_int_1_scope_value);
                    break;
                case 2:
                    scope_values->append(_int_2_scope_value);
                    break;
                default:
                    scope_values->append(new ConstantIntValue(c->as_jint_bits()));
                    break;
            }
            return 1;
        }
...
int LinearScan::append_scope_value_for_operand(LIR_Opr opr, GrowableArray<ScopeValue *> *scope_values) {
    if (opr->is_single_stack()) {
        ...
        Location::Type
                int_loc_type = NOT_LP64(Location::normal)
        LP64_ONLY(Location::int_in_long);
        ...
        // check if cached value is correct
        DEBUG_ONLY(assert_equal(sv, new LocationValue(
                Location::new_reg_loc(is_oop ? Location::oop : int_loc_type, frame_map()->regname(opr)))));
...
int LinearScan::append_scope_value(int op_id, Value value, GrowableArray<ScopeValue *> *scope_values) {
    if (value != NULL) {
        ...
        assert(con == NULL || opr->is_virtual() || opr->is_constant() || opr->is_illegal(),
               "asumption: Constant instructions have only constant operands (or illegal if constant is optimized away)");
        assert(con != NULL || opr->is_virtual(), "asumption: non-Constant instructions have only virtual operands");
        ...
        if (opr->is_virtual()) {
            ...
            assert(!has_call(op_id) || opr->is_stack() || !is_caller_save(reg_num(opr)),
                   "can not have caller-save register operands at calls");
...
IRScopeDebugInfo *LinearScan::compute_debug_info_for_scope(int op_id, IRScope *cur_scope, ValueStack *cur_state,
                                                           ValueStack *innermost_state) {
...
void LinearScan::compute_debug_info(CodeEmitInfo *info, int op_id) {
    ...
    if (info->_scope_debug_info == NULL) {
        ...
        info->_scope_debug_info = compute_debug_info_for_scope(op_id, innermost_scope, innermost_state,
                                                               innermost_state);
    } else {
        // debug information already set. Check that it is correct from the current point of view
        DEBUG_ONLY(assert_equal(info->_scope_debug_info,
                                compute_debug_info_for_scope(op_id, innermost_scope, innermost_state,
                                                             innermost_state)));
    }
}
...
void LinearScan::print_intervals(const char *label) {
    if (TraceLinearScanLevel >= 1) {
        ...
        for (i = 0; i < block_count(); i++) {
            BlockBegin *block = block_at(i);
            tty->print("B%d [%d, %d, %d, %d] ", block->block_id(), block->first_lir_instruction_id(),
                       block->last_lir_instruction_id(), block->loop_index(), block->loop_depth());
        }
        tty->cr();
        tty->cr();
    }
...
void MoveResolver::insert_move(Interval *from_interval, Interval *to_interval) {
    ...
    TRACE_LINEAR_SCAN(4, tty->print_cr("MoveResolver: inserted move from register %d (%d, %d) to %d (%d, %d)",
...
void MoveResolver::insert_move(LIR_Opr from_opr, Interval *to_interval) {
    ...
    TRACE_LINEAR_SCAN(4, tty->print("MoveResolver: inserted move from constant "); from_opr->print(); tty->print_cr(
            "  to %d (%d, %d)", to_interval->reg_num(), to_interval->assigned_reg(), to_interval->assigned_regHi()));
}
...
void MoveResolver::resolve_mappings() {
    TRACE_LINEAR_SCAN(4, tty->print_cr("MoveResolver: resolving mappings for Block B%d, index %d",
                                       _insert_list->block() != NULL ? _insert_list->block()->block_id() : -1,
                                       _insert_idx));
...
void MoveResolver::set_insert_position(LIR_List *insert_list, int insert_idx) {
    TRACE_LINEAR_SCAN(4, tty->print_cr("MoveResolver: setting insert position to Block B%d, index %d",
                                       insert_list->block() != NULL ? insert_list->block()->block_id() : -1,
                                       insert_idx));
    assert(_insert_list == NULL && _insert_idx == -1,
           "use move_insert_position instead of set_insert_position when data already set");
...
void MoveResolver::move_insert_position(LIR_List *insert_list, int insert_idx) {
    TRACE_LINEAR_SCAN(4, tty->print_cr("MoveResolver: moving insert position to Block B%d, index %d",
                                       insert_list->block() != NULL ? insert_list->block()->block_id() : -1,
                                       insert_idx));
...
void MoveResolver::add_mapping(Interval *from_interval, Interval *to_interval) {
    TRACE_LINEAR_SCAN(4, tty->print_cr("MoveResolver: adding mapping from %d (%d, %d) to %d (%d, %d)",
                                       from_interval->reg_num(), from_interval->assigned_reg(),
                                       from_interval->assigned_regHi(), to_interval->reg_num(),
                                       to_interval->assigned_reg(), to_interval->assigned_regHi()));
...
void MoveResolver::add_mapping(LIR_Opr from_opr, Interval *to_interval) {
    TRACE_LINEAR_SCAN(4, tty->print("MoveResolver: adding mapping from "); from_opr->print(); tty->print_cr(
            " to %d (%d, %d)", to_interval->reg_num(), to_interval->assigned_reg(), to_interval->assigned_regHi()));
...
void Interval::print_on(outputStream *out, bool is_cfg_printer) const {
    const char *SpillState2Name[] = {"no definition", "no spill store", "one spill store", "store at definition",
                                     "start in memory", "no optimization"};
    ...
    out->print("%d %d ", split_parent()->reg_num(),
               (register_hint(false) != NULL ? register_hint(false)->reg_num() : -1));
    ...
    assert(_use_pos_and_kinds.length() % 2 == 0, "must be");
    for (int i = _use_pos_and_kinds.length() - 2; i >= 0; i -= 2) {
        assert(_use_pos_and_kinds.at(i + 1) >= firstValidKind && _use_pos_and_kinds.at(i + 1) <= lastValidKind,
               "invalid use kind");
...
IntervalWalker::IntervalWalker(LinearScan *allocator, Interval *unhandled_fixed_first, Interval *unhandled_any_first)
        : _compilation(allocator->compilation()), _allocator(allocator) {
...
void IntervalWalker::append_to_unhandled(Interval **list, Interval *interval) {
    ...
    while (cur->from() < interval->from() ||
           (cur->from() == interval->from() && cur->first_usage(noUse) < interval->first_usage(noUse))) {
        prev = cur;
        cur = cur->next();
    }
    if (prev == NULL) {
...
void IntervalWalker::next_interval() {
    ...
    if (any != Interval::end()) {
        ...
        assert(any == Interval::end() || fixed == Interval::end() || any->from() != fixed->from() || kind == fixedKind,
...
void IntervalWalker::walk_to(int lir_op_id) {
    ...
    while (current() != NULL) {
        ...
        TRACE_LINEAR_SCAN(2, if (_current_position < id) {
            tty->cr();
            tty->print_cr("walk_to(%d) **************************************************************", id);
        })
...
LinearScanWalker::LinearScanWalker(LinearScan *allocator, Interval *unhandled_fixed_first,
                                   Interval *unhandled_any_first)
        : IntervalWalker(allocator, unhandled_fixed_first, unhandled_any_first), _move_resolver(allocator) {
    for (int i = 0; i < LinearScan::nof_regs; i++) {
...
void LinearScanWalker::insert_move(int op_id, Interval *src_it, Interval *dst_it) {
    ...
    assert(op_id > 0 && allocator()->block_of_op_with_id(op_id - 2) == op_block,
           "cannot insert move at block boundary");
...
int LinearScanWalker::find_optimal_split_pos(BlockBegin *min_block, BlockBegin *max_block, int max_split_pos) {
    ...
    assert(optimal_split_pos > allocator()->max_lir_op_id() || allocator()->is_block_begin(optimal_split_pos),
           "algorithm must move split pos to block boundary");

    return optimal_split_pos;
}


int LinearScanWalker::find_optimal_split_pos(Interval *it, int min_split_pos, int max_split_pos,
                                             bool do_loop_optimization) {
    ...
    if (min_split_pos == max_split_pos) {
        ...
        if (min_block == max_block) {
            ...
            TRACE_LINEAR_SCAN(4, tty->print_cr(
                    "      cannot move split pos to block boundary because min_pos and max_pos are in same block"));
            optimal_split_pos = max_split_pos;

        } else if (it->has_hole_between(max_split_pos - 1, max_split_pos) &&
                   !allocator()->is_block_begin(max_split_pos)) {
            ...
            TRACE_LINEAR_SCAN(4, tty->print_cr(
                    "      interval has hole just before max_split_pos, so splitting at max_split_pos"));
            optimal_split_pos = max_split_pos;

        } else {
            // seach optimal block boundary between min_split_pos and max_split_pos
            TRACE_LINEAR_SCAN(4, tty->print_cr(
                    "      moving split pos to optimal block boundary between block B%d and B%d", min_block->block_id(),
                    max_block->block_id()));
            ...
            if (do_loop_optimization) {
                ...
                if (loop_end_pos < max_split_pos) {
                    ...
                    TRACE_LINEAR_SCAN(4, tty->print_cr(
                            "      interval is used in loop that ends in block B%d, so trying to move max_block back from B%d to B%d",
                            loop_block->block_id(), max_block->block_id(), loop_block->block_id()));
                    assert(loop_block != min_block,
                           "loop_block and min_block must be different because block boundary is needed between");

                    optimal_split_pos = find_optimal_split_pos(min_block, loop_block,
                                                               loop_block->last_lir_instruction_id() + 2);
...
void LinearScanWalker::split_before_usage(Interval *it, int min_split_pos, int max_split_pos) {
    ...
    if (optimal_split_pos == it->to() && it->next_usage(mustHaveRegister, min_split_pos) == max_jint) {
        ...
        TRACE_LINEAR_SCAN(4, tty->print_cr(
                "      no split necessary because optimal split position is at end of interval"));
        return;
    }

    // must calculate this before the actual split is performed and before split position is moved to odd op_id
    bool move_necessary = !allocator()->is_block_begin(optimal_split_pos) &&
                          !it->has_hole_between(optimal_split_pos - 1, optimal_split_pos);
    ...
    assert(allocator()->is_block_begin(optimal_split_pos) || (optimal_split_pos % 2 == 1),
           "split pos must be odd when not on block boundary");
    assert(!allocator()->is_block_begin(optimal_split_pos) || (optimal_split_pos % 2 == 0),
    ...
    TRACE_LINEAR_SCAN(2, tty->print_cr("      split interval in two parts (insert_move_when_activated: %d)",
                                       move_necessary));
...
void LinearScanWalker::split_for_spilling(Interval *it) {
    ...
    if (min_split_pos == it->from()) {
        ...
        TRACE_LINEAR_SCAN(2, tty->print_cr(
                "      spilling entire interval because split pos is at beginning of interval"));
        assert(it->first_usage(shouldHaveRegister) > current_position(),
               "interval must not have use position before current_position");
        ...
        while (parent != NULL && parent->is_split_child()) {
            ...
            if (parent->assigned_reg() < LinearScan::nof_regs) {
                if (parent->first_usage(shouldHaveRegister) == max_jint) {
                    ...
                    TRACE_LINEAR_SCAN(4, tty->print_cr(
                            "      kicking out interval %d out of its register because it is never used",
                            parent->reg_num()));
        ...
        if (!allocator()->is_block_begin(optimal_split_pos)) {
            TRACE_LINEAR_SCAN(4, tty->print_cr("      inserting move from interval %d to %d", it->reg_num(),
                                               spilled_part->reg_num()));
...
void LinearScanWalker::split_and_spill_interval(Interval *it) {
    ...
    if (it->state() == inactiveState) {
        ...
        assert(it->next_usage(mustHaveRegister, current_pos) == max_jint,
               "the remaining part is spilled to stack and therefore has no register");
        split_for_spilling(it);
    }
}

int
LinearScanWalker::find_free_reg(int reg_needed_until, int interval_to, int hint_reg, int ignore_reg, bool *need_split) {
    ...
    for (int i = _first_reg; i <= _last_reg; i++) {
        if (i == ignore_reg) {
            ...
            if (min_full_reg == any_reg || i == hint_reg ||
                (_use_pos[i] < _use_pos[min_full_reg] && min_full_reg != hint_reg)) {
                min_full_reg = i;
            }
        } else if (_use_pos[i] > reg_needed_until) {
            // this register is at least free until reg_needed_until
            if (max_partial_reg == any_reg || i == hint_reg ||
                (_use_pos[i] > _use_pos[max_partial_reg] && max_partial_reg != hint_reg)) {
                max_partial_reg = i;
            }
        }
    }
...
bool LinearScanWalker::alloc_free_reg(Interval *cur) {
    ...
    assert(unhandled_first(fixedKind) == Interval::end(),
...
void LinearScanWalker::alloc_locked_reg(Interval *cur) {
    ...
    if (reg == any_reg || (_num_phys_regs == 2 && regHi == any_reg) || use_pos <= cur->first_usage(mustHaveRegister)) {
        ...
        TRACE_LINEAR_SCAN(4, tty->print_cr("able to spill current interval. first_usage(register): %d, use_pos: %d",
                                           cur->first_usage(mustHaveRegister), use_pos));

        if (cur->first_usage(mustHaveRegister) <= cur->from() + 1) {
            assert(false,
                   "cannot spill interval that is used in first instruction (possible reason: no register found)");
...
bool LinearScanWalker::is_move(LIR_Op *op, Interval *from, Interval *to) {
    ...
    return in->is_virtual() && res->is_virtual() && in->vreg_number() == from->reg_num() &&
           res->vreg_number() == to->reg_num();
}
...
void LinearScanWalker::combine_spilled_intervals(Interval *cur) {
    ...
    if (!is_move(allocator()->lir_op_with_id(begin_pos), register_hint, cur) ||
        !is_move(allocator()->lir_op_with_id(end_pos), cur, register_hint)) {
    ...
    assert(cur->first_usage(mustHaveRegister) == begin_pos,
           "must have use position at begin of interval because of move");
    assert(end_hint->first_usage(mustHaveRegister) == end_pos,
           "must have use position at begin of interval because of move");
...
bool LinearScanWalker::activate_current() {
    ...
    TRACE_LINEAR_SCAN(4, tty->print_cr("      split_parent: %d, insert_move_when_activated: %d",
    ...
    if (cur->assigned_reg() >= LinearScan::nof_regs) {
        ...
        TRACE_LINEAR_SCAN(4, tty->print_cr(
                "      interval has spill slot assigned (method parameter) -> split it before first use"));
    ...
    if (cur->insert_move_when_activated()) {
        ...
        TRACE_LINEAR_SCAN(4, tty->print_cr(
                "Inserting move from interval %d to %d because insert_move_when_activated is set",
...
bool EdgeMoveOptimizer::operations_different(LIR_Op *op1, LIR_Op *op2) {
    ...
    if (op1->code() == lir_move && op2->code() == lir_move) {
        ...
        if (move1->info() == move2->info() && move1->in_opr() == move2->in_opr() &&
            move1->result_opr() == move2->result_opr()) {
...
void EdgeMoveOptimizer::optimize_moves_at_block_end(BlockBegin *block) {
    ...
    for (i = 0; i < num_preds; i++) {
        ...
        assert(pred_instructions->last()->as_OpBranch()->cond() == lir_cond_always,
               "block must end with unconditional branch");
    ...
    while (true) {
        ...
        TRACE_LINEAR_SCAN(4, tty->print("found instruction that is equal in all %d predecessors: ",
                                        num_preds); op->print());
...
void EdgeMoveOptimizer::optimize_moves_at_block_begin(BlockBegin *block) {
    ...
    assert(cur_instructions->last()->as_OpBranch()->cond() == lir_cond_always,
           "block must end with unconditional branch");
    ...
    while (true) {
        ...
        TRACE_LINEAR_SCAN(4, tty->print("----- found instruction that is equal in all %d successors: ",
                                        num_sux); op->print());
...
void ControlFlowOptimizer::reorder_short_loop(BlockList *code, BlockBegin *header_block, int header_idx) {
    ...
    if (i == code->length() || code->at(i)->loop_depth() < header_block->loop_depth()) {
        ...
        if (end_block->number_of_sux() == 1 && end_block->sux_at(0) == header_block) {
            ...
            assert(code->at(end_idx)->is_set(BlockBegin::backward_branch_target_flag),
                   "must be backward branch target");
...
void ControlFlowOptimizer::substitute_branch_target(BlockBegin *block, BlockBegin *target_from, BlockBegin *target_to) {
    TRACE_LINEAR_SCAN(3, tty->print_cr("Deleting empty block: substituting from B%d to B%d inside B%d",
                                       target_from->block_id(), target_to->block_id(), block->block_id()));
        ...
        if (last_op->code() == lir_branch) {
            ...
            if (last_branch->info() == NULL) {
                if (last_branch->block() == code->at(i + 1)) {
                    ...
                    TRACE_LINEAR_SCAN(3, tty->print_cr("Deleting unconditional branch at end of block B%d",
                                                       block->block_id()));
                  ...
                  prev_cmove = (LIR_Op4*)prev_op;
                            ...
                            if (prev_branch->block() == code->at(i + 1) && prev_branch->info() == NULL) {
                                ...
                                TRACE_LINEAR_SCAN(3, tty->print_cr(
                                        "Negating conditional branch and deleting unconditional branch at end of block B%d",
                                        block->block_id()));
        ...
        if (cur_instructions->length() == 2 && cur_last_op->code() == lir_return) {
            ...
            for (int j = block->number_of_preds() - 1; j >= 0; j--) {
                ...
                if (pred_last_op->code() == lir_branch) {
                    ...
                    if (pred_last_branch->block() == block && pred_last_branch->cond() == lir_cond_always &&
                        pred_last_branch->info() == NULL) {
...
const char *LinearScanStatistic::counter_name(int counter_idx) {
    switch (counter_idx) {
        ...
        case counter_fpu_method:
            return "methods using fpu";
        case counter_loop_method:
            return "methods with loops";
        case counter_exception_method:
            return "methods with xhandler";

        case counter_loop:
            return "loops";
        case counter_block:
            return "blocks";
        case counter_loop_block:
            return "blocks inside loop";
        case counter_exception_block:
            return "exception handler entries";
        case counter_interval:
            return "intervals";
        case counter_fixed_interval:
            return "fixed intervals";
        case counter_range:
            return "ranges";
        case counter_fixed_range:
            return "fixed ranges";
        case counter_use_pos:
            return "use positions";
        case counter_fixed_use_pos:
            return "fixed use positions";
        case counter_spill_slots:
            return "spill slots";

            // counter for classes of lir instructions
        case counter_instruction:
        ...
        case counter_cmp:
            return "compare";
        case counter_cond_branch:
            return "conditional branches";
        case counter_uncond_branch:
            return "unconditional branches";
        case counter_stub_branch:
            return "branches to stub";
        case counter_alu:
            return "artithmetic + logic";
        case counter_alloc:
            return "allocations";
        case counter_sync:
        ...
        case counter_unwind:
            return "unwind";
        case counter_typecheck:
            return "type+null-checks";
        case counter_fpu_stack:
            return "fpu-stack";
        case counter_misc_inst:
            return "other instructions";
        case counter_other_inst:
            return "misc. instructions";

            // counter for different types of moves
        case counter_move_total:
            return "total moves";
        case counter_move_reg_reg:
            return "register->register";
        case counter_move_reg_stack:
            return "register->stack";
        case counter_move_stack_reg:
            return "stack->register";
        case counter_move_stack_stack:
            return "stack->stack";
        case counter_move_reg_mem:
            return "register->memory";
        case counter_move_mem_reg:
            return "memory->register";
        case counter_move_const_any:
            return "constant->any";
...
LinearScanStatistic::Counter LinearScanStatistic::base_counter(int counter_idx) {
    if (counter_idx == counter_fpu_method || counter_idx == counter_loop_method ||
        counter_idx == counter_exception_method) {
...
void LinearScanStatistic::collect(LinearScan *allocator) {
    ...
    for (i = 0; i < allocator->ir()->code()->length(); i++) {
        ...
        for (int j = 0; j < instructions->length(); j++) {
            ...
            switch (op->code()) {
                case lir_label:
                    inc_counter(counter_label);
                    break;
                case lir_std_entry:
                case lir_osr_entry:
                    inc_counter(counter_entry);
                    break;
                case lir_return:
                    inc_counter(counter_return);
                    break;

                case lir_rtcall:
                case lir_static_call:
                case lir_optvirtual_call:
                    inc_counter(counter_call);
                    break;
                ...
                case lir_shr:
                case lir_ushr:
                    inc_counter(counter_alu);
                    break;

                case lir_alloc_object:
                case lir_alloc_array:
                    inc_counter(counter_alloc);
                    break;

                case lir_monaddr:
                case lir_lock:
                case lir_unlock:
                    inc_counter(counter_sync);
                    break;

                case lir_throw:
                    inc_counter(counter_throw);
                    break;

                case lir_unwind:
                    inc_counter(counter_unwind);
                    break;
                ...
                case lir_store_check:
                    inc_counter(counter_typecheck);
                    break;

                case lir_fpop_raw:
                case lir_fxch:
                case lir_fld:
                    inc_counter(counter_fpu_stack);
                    break;
                ...
                default:
                    inc_counter(counter_other_inst);
                    break;
            }
        }
    }
...
const char *LinearScanTimers::timer_name(int idx) {
    switch (idx) {
        case timer_do_nothing:
            return "Nothing (Time Check)";
        case timer_number_instructions:
            return "Number Instructions";
        case timer_compute_local_live_sets:
            return "Local Live Sets";
        case timer_compute_global_live_sets:
            return "Global Live Sets";
        case timer_build_intervals:
            return "Build Intervals";
        case timer_sort_intervals_before:
            return "Sort Intervals Before";
        case timer_allocate_registers:
            return "Allocate Registers";
        case timer_resolve_data_flow:
            return "Resolve Data Flow";
        case timer_sort_intervals_after:
            return "Sort Intervals After";
        case timer_eliminate_spill_moves:
            return "Spill optimization";
        case timer_assign_reg_num:
            return "Assign Reg Num";
        case timer_allocate_fpu_stack:
            return "Allocate FPU Stack";
        case timer_optimize_lir:
            return "Optimize LIR";
        default:
            ShouldNotReachHere();
            return "";
...
void LinearScanTimers::end_method(LinearScan *allocator) {
    if (TimeEachLinearScan) {
        ...
        if (total >= 0.0005) {
            ...
            tty->print("@");
            allocator->compilation()->method()->print_name();
...
void LinearScanTimers::print(double total_time) {
    if (TimeLinearScan) {
        ...
        for (int i = 0; i < number_of_timers; i++) {
            ...
            tty->print_cr("    %25s: %6.3f s (%4.1f%%)  corrected: %6.3f s (%4.1f%%)", timer_name(i), t,
                          (t / total_time) * 100.0, t - c, (t - c) / (total_time - 2 * number_of_timers * c) * 100);
        }
    }
}
