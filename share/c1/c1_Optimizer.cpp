...
void CE_Eliminator::block_do(BlockBegin *block) {
    ...
    for_each_phi_fun(t_block, phi,
    return; );
    for_each_phi_fun(f_block, phi,
    return; );
                        ...
                        if (tval && fval) {
                            ...
                            if (prev != NULL) {
                                ...
                                if (tblock != fblock && !if_->is_safepoint()) {
                                    ...
                                    _merge_count++;
                                    if (PrintBlockElimination) {
                                        tty->print_cr("%d. replaced If and IfOp at end of B%d with single If",
                                                      _merge_count, block->block_id());
                                    }
...
public:
    ...
    NullCheckEliminator(Optimizer *opt)
            : _opt(opt), _work_list(new BlockList()), _set(new ValueSet()),
    ...
    Value last_explicit_null_check_obj() {
...
void NullCheckVisitor::do_ProfileCall(ProfileCall *x) {
    nce()->clear_last_explicit_null_check();
...
void NullCheckEliminator::handle_Intrinsic(Intrinsic *x) {
    ...
    if (set_contains(recv)) {
        ...
        if (PrintNullCheckElimination) {
            tty->print_cr("Eliminated Intrinsic %d's null check for value %d", vmIntrinsics::as_int(x->id()),
                          recv->id());
        }
        x->set_needs_null_check(false);
    } else {
        set_put(recv);
        if (PrintNullCheckElimination) {
            tty->print_cr("Intrinsic %d of value %d proves value to be non-null", vmIntrinsics::as_int(x->id()),
                          recv->id());
        }
        // Ensure previous passes do not cause wrong state
        x->set_needs_null_check(true);
