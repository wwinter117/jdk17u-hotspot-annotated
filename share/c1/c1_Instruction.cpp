...
void Instruction::update_exception_state(ValueStack *state) {
    if (state != NULL &&
        (state->kind() == ValueStack::EmptyExceptionState || state->kind() == ValueStack::ExceptionState)) {
        assert(state->kind() == ValueStack::EmptyExceptionState ||
               Compilation::current()->env()->should_retain_local_variables(), "unexpected state kind");
...
Invoke::Invoke(Bytecodes::Code code, ValueType *result_type, Value recv, Values *args,
        ...
        : StateSplit(result_type, state_before), _code(code), _recv(recv), _args(args), _target(target) {
...
Constant::CompareResult Constant::compare(Instruction::Condition cond, Value right) const {
    ...
    switch (lt->tag()) {
        case intTag: {
            ...
            switch (cond) {
                case If::eql:
                    return x == y ? cond_true : cond_false;
                case If::neq:
                    return x != y ? cond_true : cond_false;
                case If::lss:
                    return x < y ? cond_true : cond_false;
        ...
        case objectTag: {
            ...
            if (xvalue->is_loaded() && yvalue->is_loaded()) {
                switch (cond) {
                    case If::eql:
                        return xvalue == yvalue ? cond_true : cond_false;
                    case If::neq:
                        return xvalue != yvalue ? cond_true : cond_false;
                    default     :
                        break;
...
void BlockBegin::iterate_preorder(boolArray &mark, BlockClosure *closure) {
    if (!mark.at(block_id())) {
        ...
        {
            for (int i = number_of_exception_handlers() - 1; i >= 0; i--)
                exception_handler_at(i)->iterate_preorder(mark, closure);
        }
...
void BlockBegin::iterate_postorder(boolArray &mark, BlockClosure *closure) {
    if (!mark.at(block_id())) {
        ...
        {
            for (int i = number_of_exception_handlers() - 1; i >= 0; i--)
                exception_handler_at(i)->iterate_postorder(mark, closure);
        }
    ...
    if (existing_state == NULL) {
        ...
        if (is_set(BlockBegin::parser_loop_header_flag)) {
            ...
            for_each_stack_value(new_state, index, new_value)
            {
                ...
                TRACE_PHI(tty->print_cr("creating phi-function %c%d for stack %d",
                                        new_state->stack_at(index)->type()->tchar(), new_state->stack_at(index)->id(),
                                        index));
            }
            ...
            for_each_local_value(new_state, index, new_value)
            {
                bool requires_phi = requires_phi_function.at(index) ||
                                    (new_value->type()->is_double_word() && requires_phi_function.at(index + 1));
                if (requires_phi || !SelectivePhiFunctions) {
                    new_state->setup_phi_for_local(this, index);
                    TRACE_PHI(tty->print_cr("creating phi-function %c%d for local %d",
                                            new_state->local_at(index)->type()->tchar(),
                                            new_state->local_at(index)->id(), index));
                }
            }
        }
        ...
        if (is_set(BlockBegin::was_visited_flag)) {
            ...
            for_each_stack_value(existing_state, index, existing_value)
            {
                ...
                if (new_value != existing_value && (existing_phi == NULL || existing_phi->block() != this)) {
                    ...
                    TRACE_PHI(tty->print_cr("creating phi-function %c%d for stack %d",
                                            existing_state->stack_at(index)->type()->tchar(),
                                            existing_state->stack_at(index)->id(), index));
                }
            }
            ...
            for_each_local_value(existing_state, index, existing_value)
            {
                ...
                if (new_value == NULL || new_value->type()->tag() != existing_value->type()->tag()) {
                    ...
                    TRACE_PHI(tty->print_cr("creating phi-function %c%d for local %d",
                                            existing_state->local_at(index)->type()->tchar(),
                                            existing_state->local_at(index)->id(), index));
                }
            }
        }
...
void RangeCheckPredicate::check_state() {
    assert(state()->kind() != ValueStack::EmptyExceptionState && state()->kind() != ValueStack::ExceptionState,
           "will deopt with empty state");
}
