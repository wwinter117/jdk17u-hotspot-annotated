...
public:
    NewInstanceStub(LIR_Opr klass_reg, LIR_Opr result, ciInstanceKlass *klass, CodeEmitInfo *info,
...
public:
    ...
    PatchingStub(MacroAssembler *masm, PatchID id, int index = -1) :
            _id(id), _info(NULL), _index(index) {
    ...
    void install(MacroAssembler *masm, LIR_PatchCode patch_code, Register obj, CodeEmitInfo *info) {
        ...
        if (_id == PatchingStub::access_field_id) {
            ...
            switch (patch_code) {
                case lir_patch_low:
                    field_offset = lo_word_offset_in_bytes;
                    break;
                case lir_patch_high:
                    field_offset = hi_word_offset_in_bytes;
                    break;
...
public:
    ArrayStoreExceptionStub(LIR_Opr obj, CodeEmitInfo *info) : SimpleExceptionStub(
            Runtime1::throw_array_store_exception_id, obj, info) {}
