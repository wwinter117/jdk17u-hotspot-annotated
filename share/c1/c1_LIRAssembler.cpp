...
LIR_Assembler::LIR_Assembler(Compilation *c) :
        _masm(c->masm()), _bs(BarrierSet::barrier_set()), _compilation(c), _frame_map(c->frame_map()),
        _current_block(NULL), _pending_non_safepoint(NULL), _pending_non_safepoint_offset(0) {
    ...
    for (int i = 0; i < info_list->length(); i++) {
        ...
        for (int j = 0; j < handlers->length(); j++) {
            ...
            assert(handler->entry_code() == NULL ||
                   ...
                   handler->entry_code()->instructions_list()->last()->code() == lir_delay_slot,
                   "last operation must be branch");
...
void LIR_Assembler::move_op(LIR_Opr src, LIR_Opr dest, BasicType type, LIR_PatchCode patch_code, CodeEmitInfo *info,
                            bool pop_fpu_stack, bool unaligned, bool wide) {
    if (src->is_register()) {
        if (dest->is_register()) {
