...
public:
    ...
    XHandler(ciExceptionHandler *desc)
            : _desc(desc), _entry_block(NULL), _entry_code(NULL), _entry_pco(-1), _phi_operand(-1), _scope_count(-1)
#ifdef ASSERT
    , _lir_op_id(-1)
#endif
    {}
    ...
    XHandler(XHandler *other)
            : _desc(other->_desc), _entry_block(other->_entry_block), _entry_code(other->_entry_code),
              _entry_pco(other->_entry_pco), _phi_operand(other->_phi_operand), _scope_count(other->_scope_count)
#ifdef ASSERT
...
public:
    // creation
    IRScope(Compilation *compilation, IRScope *caller, int caller_bci, ciMethod *method, int osr_bci,
...
public:
    IRScopeDebugInfo(IRScope *scope,
            ...
            _scope(scope), _bci(bci), _locals(locals), _expressions(expressions), _monitors(monitors),
            _caller(caller) {}
