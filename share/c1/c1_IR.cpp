...
BlockBegin *IRScope::build_graph(Compilation *compilation, int osr_bci) {
    ...
    NOT_PRODUCT(
    if (PrintValueNumbering && Verbose) gm.print_stats());
    if (compilation->bailed_out()) return NULL;
    return gm.start();
}


IRScope::IRScope(Compilation *compilation, IRScope *caller, int caller_bci, ciMethod *method, int osr_bci,
                 bool create_graph)
        : _compilation(compilation), _callees(2), _requires_phi_function(method->max_locals()) {
    _caller = caller;
...
CodeEmitInfo::CodeEmitInfo(ValueStack *stack, XHandlers *exception_handlers, bool deoptimize_on_exception)
        : _scope_debug_info(NULL), _scope(stack->scope()), _exception_handlers(exception_handlers), _oop_map(NULL),
          _stack(stack), _is_method_handle_invoke(false), _deoptimize_on_exception(deoptimize_on_exception),
...
CodeEmitInfo::CodeEmitInfo(CodeEmitInfo *info, ValueStack *stack)
        : _scope_debug_info(NULL), _scope(info->_scope), _exception_handlers(NULL), _oop_map(NULL),
          _stack(stack == NULL ? info->_stack : stack), _is_method_handle_invoke(info->_is_method_handle_invoke),
          _deoptimize_on_exception(info->_deoptimize_on_exception), _force_reexecute(info->_force_reexecute) {
...
#ifndef PRODUCT
            if (PrintCFG || PrintCFG1) {
                tty->print_cr("CFG after CEE");
                print(true);
            }
            if (PrintIR || PrintIR1) {
                tty->print_cr("IR after CEE");
                print(false);
            }
#endif
        }
...
#ifndef PRODUCT
            if (PrintCFG || PrintCFG1) {
                tty->print_cr("CFG after block elimination");
                print(true);
            }
            if (PrintIR || PrintIR1) {
                tty->print_cr("IR after block elimination");
                print(false);
            }
#endif
        }
...
#ifndef PRODUCT
        if (PrintCFG || PrintCFG1) {
            tty->print_cr("CFG after null check elimination");
            print(true);
        }
        if (PrintIR || PrintIR1) {
            tty->print_cr("IR after null check elimination");
            print(false);
        }
#endif
    }
...
private:
    ...
    void inc_forward_branches(BlockBegin *b) {
        _forward_branches.at_put(b->block_id(), _forward_branches.at(b->block_id()) + 1);
    }

    int dec_forward_branches(BlockBegin *b) {
        _forward_branches.at_put(b->block_id(), _forward_branches.at(b->block_id()) - 1);
        return _forward_branches.at(b->block_id());
    }
...
void ComputeLinearScanOrder::count_edges(BlockBegin *cur, BlockBegin *parent) {
    TRACE_LINEAR_SCAN(3, tty->print_cr("Enter count_edges for block B%d coming from B%d", cur->block_id(),
                                       parent != NULL ? parent->block_id() : -1));
...
void ComputeLinearScanOrder::mark_loops() {
    ...
    for (int i = _loop_end_blocks.length() - 1; i >= 0; i--) {
        ...
        TRACE_LINEAR_SCAN(3, tty->print_cr("Processing loop from B%d to B%d (loop %d):", loop_start->block_id(),
                                           loop_end->block_id(), loop_idx));
...
void ComputeLinearScanOrder::compute_dominator_impl(BlockBegin *cur, BlockBegin *parent) {
    ...
    if (cur->dominator() == NULL) {
        TRACE_LINEAR_SCAN(4, tty->print_cr("DOM: initializing dominator of B%d to B%d", cur->block_id(),
                                           parent->block_id()));
        cur->set_dominator(parent);

    } else if (!(cur->is_set(BlockBegin::linear_scan_loop_header_flag) &&
                 parent->is_set(BlockBegin::linear_scan_loop_end_flag))) {
        TRACE_LINEAR_SCAN(4, tty->print_cr("DOM: computing dominator of B%d: common dominator of B%d and B%d is B%d",
                                           cur->block_id(), parent->block_id(), cur->dominator()->block_id(),
                                           common_dominator(cur->dominator(), parent)->block_id()));
...
void ComputeLinearScanOrder::sort_into_work_list(BlockBegin *cur) {
    ...
    TRACE_LINEAR_SCAN(3, for (int i = 0; i < _work_list.length(); i++)
        tty->print_cr("%8d B%2d  weight:%6x", i, _work_list.at(i)->block_id(), _work_list.at(i)->linear_scan_number()));
...
void ComputeLinearScanOrder::append_block(BlockBegin *cur) {
    TRACE_LINEAR_SCAN(3, tty->print_cr("appending block B%d (weight 0x%6x) to linear-scan order", cur->block_id(),
...
void ComputeLinearScanOrder::compute_order(BlockBegin *start_block) {
    ...
    if (osr_entry != NULL) {
        ...
        assert(osr_entry->sux_at(0)->number_of_preds() >= 2,
               "sucessor of osr entry must have two predecessors (otherwise it is not present in normal control flow");
...
bool ComputeLinearScanOrder::compute_dominators_iter() {
    ...
    for (int i = 1; i < num_blocks; i++) {
        ...
        if (dominator != block->dominator()) {
            TRACE_LINEAR_SCAN(4, tty->print_cr("DOM: updating dominator of B%d from B%d to B%d", block->block_id(),
                                               block->dominator()->block_id(), dominator->block_id()));
...
void ComputeLinearScanOrder::compute_dominators() {
    TRACE_LINEAR_SCAN(3, tty->print_cr("----- computing dominators (iterative computation reqired: %d)",
