...
void InstructionPrinter::print_object(Value obj) {
    ...
    if (type->as_ObjectConstant() != NULL) {
        ...
        if (value->is_null_object()) {
            ...
            output()->print("<unloaded object "
            INTPTR_FORMAT
            ">", p2i(value));
        } else {
            output()->print("<object "
            INTPTR_FORMAT
            " klass=", p2i(value->constant_encoding()));
        ...
        if (value->is_loaded()) {
            ...
            output()->print("<unloaded instance "
            INTPTR_FORMAT
            ">", p2i(value));
        }
        ...
        output()->print("<array "
        INTPTR_FORMAT
        ">", p2i(type->as_ArrayConstant()->value()->constant_encoding()));
...
void InstructionPrinter::print_head() {
    const char filler = '_';
    fill_to(bci_pos, filler);
    output()->print("bci");
    fill_to(use_pos, filler);
    output()->print("use");
    fill_to(temp_pos, filler);
    output()->print("tid");
    fill_to(instr_pos, filler);
    output()->print("instr");
...
void InstructionPrinter::print_line(Instruction *instr) {
    ...
    if (instr->has_printable_bci()) {
        fill_to(bci_pos);
        output()->print("%d", instr->printable_bci());
    }
    fill_to(use_pos);
    output()->print("%d", instr->use_count());
    fill_to(temp_pos);
    print_temp(instr);
    fill_to(instr_pos);
    print_instr(instr);
...
void InstructionPrinter::do_Constant(Constant *x) {
    ...
    switch (t->tag()) {
        case intTag    :
            ...
            break;
        case longTag   :
            output()->print(JLONG_FORMAT, t->as_LongConstant()->value());
            output()->print("L");
            break;
        case floatTag  :
...
void InstructionPrinter::do_MemBar(MemBar *x) {
    ...
    switch (code) {
        case lir_membar_acquire   :
            output()->print("membar_acquire");
            break;
        case lir_membar_release   :
            output()->print("membar_release");
            break;
        case lir_membar           :
            output()->print("membar");
            break;
        case lir_membar_loadload  :
            output()->print("membar_loadload");
            break;
        case lir_membar_storestore:
            output()->print("membar_storestore");
            break;
        case lir_membar_loadstore :
            output()->print("membar_loadstore");
            break;
        case lir_membar_storeload :
            output()->print("membar_storeload");
            break;
        default                   :
            ShouldNotReachHere();
            break;
