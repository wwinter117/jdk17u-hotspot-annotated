...
private:
    ...
    class PrintBlockClosure : public BlockClosure {
        void block_do(BlockBegin *block) {
            if (block != NULL)
                Compilation::current()->cfg_printer_output()->print_block(block);
        }
    };
...
public:
    ...
    void set_print_flags(bool do_print_HIR, bool do_print_LIR) {
        _do_print_HIR = do_print_HIR;
        _do_print_LIR = do_print_LIR;
    }
