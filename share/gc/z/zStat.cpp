...
void ZStatPhaseCycle::register_end(const Ticks &start, const Ticks &end) const {
    ...
    log_info(gc)("Garbage Collection (%s) "
    ZSIZE_FMT "->"
    ZSIZE_FMT,
            GCCause::to_string(ZCollectedHeap::heap()->gc_cause()),
...
void ZStatPhasePause::register_start(const Ticks &start) const {
    ...
    LogTarget(Debug, gc, phases, start)
    log;
    log_start(log);
}
...
void ZStatMark::print() {
    ...
    log_info(gc, marking)("Mark Stack Usage: "
    SIZE_FORMAT
    "M", _mark_stack_usage / M);
}
...
void ZStatRelocation::print() {
    ...
    log_info(gc, reloc)("Forwarding Usage: "
    SIZE_FORMAT
    "M", _forwarding_usage / M);
}
