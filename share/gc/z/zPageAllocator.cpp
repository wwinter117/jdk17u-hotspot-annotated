...
ZPageAllocator::ZPageAllocator(ZWorkers *workers,
    ...
    log_info_p(gc, init)("Min Capacity: "
    SIZE_FORMAT
    "M", min_capacity / M);
    log_info_p(gc, init)("Initial Capacity: "
    SIZE_FORMAT
    "M", initial_capacity / M);
    log_info_p(gc, init)("Max Capacity: "
    SIZE_FORMAT
    "M", max_capacity / M);
    if (ZPageSizeMedium > 0) {
        log_info_p(gc, init)("Medium Page Size: "
        SIZE_FORMAT
        "M", ZPageSizeMedium / M);
    } else {
    ...
    if (!prime_cache(workers, initial_capacity)) {
        log_error_p(gc)("Failed to allocate initial Java heap ("
        SIZE_FORMAT
        "M)", initial_capacity / M);
        return;
    }
...
ZPage *ZPageAllocator::alloc_page_create(ZPageAllocation *allocation) {
    ...
    if (flushed > 0) {
        ...
        log_debug(gc, heap)("Page Cache Flushed: "
        SIZE_FORMAT
        "M", flushed / M);
    }
