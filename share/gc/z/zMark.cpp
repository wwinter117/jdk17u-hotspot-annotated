...
void ZMark::push_partial_array(uintptr_t addr, size_t size, bool finalizable) {
    ...
    log_develop_trace(gc, marking)("Array push partial: "
    PTR_FORMAT
    " ("
    SIZE_FORMAT
    "), stripe: "
...
void ZMark::follow_small_array(uintptr_t addr, size_t size, bool finalizable) {
    ...
    log_develop_trace(gc, marking)("Array follow small: "
    PTR_FORMAT
    " ("
    SIZE_FORMAT
    ")", addr, size);
...
void ZMark::follow_large_array(uintptr_t addr, size_t size, bool finalizable) {
    ...
    log_develop_trace(gc, marking)("Array follow large: "
    PTR_FORMAT
    "-"
    PTR_FORMAT
    " ("
...
public:
    ...
    ~ZMarkTimeout() {
        ...
        log_debug(gc, marking)("Mark With Timeout (%s): %s, "
        UINT64_FORMAT
        " oops, %.3fms",
...
void ZMark::work_with_timeout(ZMarkCache *cache, ZMarkStripe *stripe, ZMarkThreadLocalStacks *stacks,
