...
size_t ZMarkStackSpace::expand_space() {
    ...
    if (new_size > ZMarkStackSpaceLimit) {
        ...
        fatal("Mark stack space exhausted. Use -XX:ZMarkStackSpaceLimit=<size> to increase the "
              "maximum number of bytes allocated for mark stacks. Current limit is "
        SIZE_FORMAT
        "M.",
                ZMarkStackSpaceLimit / M);
    }

    log_debug(gc, marking)("Expanding mark stack space: "
    SIZE_FORMAT
    "M->"
    SIZE_FORMAT
    "M",
...
size_t ZMarkStackSpace::shrink_space() {
    ...
    if (shrink_size > 0) {
        // Shrink
        log_debug(gc, marking)("Shrinking mark stack space: "
        SIZE_FORMAT
        "M->"
        SIZE_FORMAT
        "M",
