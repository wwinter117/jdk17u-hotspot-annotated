...
bool G1Policy::need_to_start_conc_mark(const char *source, size_t alloc_word_size) {
    ...
    if (marking_request_bytes > marking_initiating_used_threshold) {
        ...
        log_debug(gc, ergo, ihop)("%s occupancy: "
        SIZE_FORMAT
...
double G1Policy::logged_cards_processing_time() const {
    double all_cards_processing_time =
            average_time_ms(G1GCPhaseTimes::ScanHR) + average_time_ms(G1GCPhaseTimes::OptScanHR);
    size_t logged_dirty_cards = phase_times()->sum_thread_work_items(G1GCPhaseTimes::MergeLB,
                                                                     G1GCPhaseTimes::MergeLBDirtyCards);
    size_t scan_heap_roots_cards =
            phase_times()->sum_thread_work_items(G1GCPhaseTimes::ScanHR, G1GCPhaseTimes::ScanHRScannedCards) +
...
void G1Policy::record_collection_pause_end(double pause_time_ms, bool concurrent_operation_is_full_mark) {
    ...
    if (update_stats) {
        size_t const total_log_buffer_cards =
                p->sum_thread_work_items(G1GCPhaseTimes::MergeHCC, G1GCPhaseTimes::MergeHCCDirtyCards) +
        ...
        size_t const total_cards_merged =
                p->sum_thread_work_items(G1GCPhaseTimes::MergeRS, G1GCPhaseTimes::MergeRSDirtyCards) +
        ...
        size_t const total_cards_scanned =
                p->sum_thread_work_items(G1GCPhaseTimes::ScanHR, G1GCPhaseTimes::ScanHRScannedCards) +
        ...
        const size_t from_rs_length_cards = (total_cards_scanned > total_log_buffer_cards) ? total_cards_scanned -
                                                                                             total_log_buffer_cards : 0;
        ...
        if (copied_bytes > 0) {
            double cost_per_byte_ms =
                    (average_time_ms(G1GCPhaseTimes::ObjCopy) + average_time_ms(G1GCPhaseTimes::OptObjCopy)) /
                    copied_bytes;
    ...
    double scan_logged_cards_time_goal_ms =
            _mmu_tracker->max_gc_time() * MILLIUNITS * G1RSetUpdatingPauseTimePercent / 100.0;

    if (scan_logged_cards_time_goal_ms < merge_hcc_time_ms) {
        log_debug(gc, ergo, refine)(
                "Adjust concurrent refinement thresholds (scanning the HCC expected to take longer than Update RS time goal)."
    ...
    log_debug(gc, ergo, refine)(
            "Concurrent refinement times: Logged Cards Scan time goal: %1.2fms Logged Cards Scan time: %1.2fms HCC time: %1.2fms",
...
double G1Policy::predict_base_elapsed_time_ms(size_t pending_cards,
    ...
    size_t effective_scanned_cards = _analytics->predict_scan_card_num(rs_length,
                                                                       collector_state()->in_young_only_phase());
    return
            _analytics->predict_card_merge_time_ms(pending_cards + rs_length,
                                                   collector_state()->in_young_only_phase()) +
...
void G1Policy::calculate_old_collection_set_regions(G1CollectionSetCandidates *candidates,
    ...
    if (num_expensive_regions > 0) {
        log_debug(gc, ergo, cset)(
                "Added %u initial old regions to collection set although the predicted time was too high.",
                num_expensive_regions);
    }
...
bool G1Policy::preventive_collection_required(uint alloc_region_count) {
    ...
    size_t const eden_surv_bytes_pred =
            _eden_surv_rate_group->accum_surv_rate_pred(eden_count) * HeapRegion::GrainBytes;
    size_t const total_young_predicted_surviving_bytes =
            eden_surv_bytes_pred + _predicted_surviving_bytes_from_survivor;
    ...
    if (required_regions > _g1h->num_free_regions() - alloc_region_count) {
        log_debug(gc, ergo, cset)(
                "Preventive GC, insufficient free regions. Predicted need %u. Curr Eden %u (Pred %u). Curr Survivor %u (Pred %u). Curr Old %u (Pred %u) Free %u Alloc %u",
                required_regions,
                eden_count,
