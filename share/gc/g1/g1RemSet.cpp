...
public:
    ...
    bool chunk_needs_scan(uint const region_idx, uint const card_in_region) const {
        size_t const idx =
                ((size_t) region_idx << _log_scan_chunks_per_region) + (card_in_region >> _scan_chunks_shift);
        assert(idx < _num_total_scan_chunks, "Index "
        SIZE_FORMAT
        " out of bounds "
        SIZE_FORMAT,
...
private:
    ...
    public:
        ...
        double worker_cost() const override {
            ...
            return ((double) align_up((size_t) num_regions << HeapRegion::LogCardsPerRegion, chunk_size()) /
                    chunk_size());
        }
...
public:
    G1CardTableScanner(CardValue *start_card, size_t size) :
        ...
        assert(is_aligned(start_card, sizeof(size_t)), "Unaligned start addr "
        PTR_FORMAT, p2i(start_card));
        assert(is_aligned(size, sizeof(size_t)), "Unaligned size "
        SIZE_FORMAT, size);
    }
...
class G1ScanHRForRegionClosure : public HeapRegionClosure {
    ...
    HeapWord *scan_memregion(uint region_idx_for_card, MemRegion mr) {
        ...
        assert(scanned_to >= mr.end(), "Scanned to "
        PTR_FORMAT
        " less than range "
        PTR_FORMAT, p2i(scanned_to), p2i(mr.end()));
    ...
    void scan_heap_roots(HeapRegion *r) {
        ...
        while (claim.has_next()) {
            ...
            while (first_scan_idx != claim.size()) {
                assert(*_ct->byte_for_index(region_card_base_idx + first_scan_idx) <= 0x1, "is %d at region %u idx "
                SIZE_FORMAT, *_ct->byte_for_index(region_card_base_idx + first_scan_idx), region_idx, first_scan_idx);
...
void G1RemSet::scan_collection_set_regions(G1ParScanThreadState *pss,
    ...
    if (scan_phase == G1GCPhaseTimes::OptScanHR) {
        p->record_or_add_thread_work_item(scan_phase, worker_id, cl.opt_refs_scanned(),
                                          G1GCPhaseTimes::ScanHRScannedOptRefs);
        p->record_or_add_thread_work_item(scan_phase, worker_id, cl.opt_refs_memory_used(),
                                          G1GCPhaseTimes::ScanHRUsedMemory);
    }
}
...
public:
    ...
    virtual void work(uint worker_id) {
        ...
        if (_initial_evacuation &&
            ...
            p->record_or_add_thread_work_item(merge_remset_phase, worker_id, cl.merged_sparse(),
                                              G1GCPhaseTimes::MergeRSMergedSparse);
            p->record_or_add_thread_work_item(merge_remset_phase, worker_id, cl.merged_fine(),
                                              G1GCPhaseTimes::MergeRSMergedFine);
            p->record_or_add_thread_work_item(merge_remset_phase, worker_id, cl.merged_coarse(),
                                              G1GCPhaseTimes::MergeRSMergedCoarse);
            p->record_or_add_thread_work_item(merge_remset_phase, worker_id, cl.cards_dirty(),
                                              G1GCPhaseTimes::MergeRSDirtyCards);
        }
        ...
        if (_initial_evacuation && G1HotCardCache::default_use_cache()) {
            ...
            p->record_thread_work_item(G1GCPhaseTimes::MergeHCC, worker_id, cl.cards_dirty(),
                                       G1GCPhaseTimes::MergeHCCDirtyCards);
            p->record_thread_work_item(G1GCPhaseTimes::MergeHCC, worker_id, cl.cards_skipped(),
                                       G1GCPhaseTimes::MergeHCCSkippedCards);
        }
        ...
        if (_initial_evacuation) {
            ...
            p->record_thread_work_item(G1GCPhaseTimes::MergeLB, worker_id, cl.cards_dirty(),
                                       G1GCPhaseTimes::MergeLBDirtyCards);
            p->record_thread_work_item(G1GCPhaseTimes::MergeLB, worker_id, cl.cards_skipped(),
                                       G1GCPhaseTimes::MergeLBSkippedCards);
        }
    }
};
...
void G1RemSet::print_merge_heap_roots_stats() {
    ...
    size_t total_old_region_cards =
            (g1h->num_regions() - (g1h->num_free_regions() - g1h->collection_set()->cur_length())) *
            HeapRegion::CardsPerRegion;

    log_debug(gc, remset)("Visited cards "
    SIZE_FORMAT
    " Total dirty "
    SIZE_FORMAT
    " (%.2lf%%) Total old "
...
void G1RemSet::merge_heap_roots(bool initial_evacuation) {
...

    {
        ...
        log_debug(gc, ergo)("Running %s using %u workers for "
        SIZE_FORMAT
        " regions",
...
class G1RebuildRemSetTask : public AbstractGangTask {
    ...
    class G1RebuildRemSetHeapRegionClosure : public HeapRegionClosure {
        ...
        bool is_humongous_live(oop const humongous_obj, const G1CMBitMap *const bitmap, HeapWord *tams,
                               HeapWord *tars) const {
        ...
        public:
            LiveObjIterator(const G1CMBitMap *const bitmap, const HeapWord *tams, const MemRegion mr,
                            HeapWord *first_oop_into_mr) :
                    _bitmap(bitmap),
                    _tams(tams),
                    _mr(mr),
                ...
                if (is_live(_current)) {
                    ...
                    assert(_current == _mr.end() || is_live(_current),
                           "Current "
                    PTR_FORMAT
                    " should be live (%s) or beyond the end of the MemRegion ("
                    PTR_FORMAT
                    ")",
            ...
            oop next() const {
                ...
                PTR_FORMAT
                " outside %d",
                        p2i(_current), p2i(_tams), _tams > _current, p2i(_mr.start()), p2i(_mr.end()), _mr.contains(
                        result));
                return result;
            }
        ...
        size_t rebuild_rem_set_in_region(const G1CMBitMap *const bitmap,
            ...
            if (hr->is_humongous()) {
                ...
                if (is_humongous_live(humongous_obj, bitmap, top_at_mark_start, top_at_rebuild_start)) {
                    ...
                    return top_at_mark_start != hr->bottom() ? mr.intersection(
                            MemRegion(cast_from_oop<HeapWord *>(humongous_obj), humongous_obj->size())).byte_size() : 0;
                } else {
                    return 0;
                }
            }

            for (LiveObjIterator it(bitmap, top_at_mark_start, mr,
                                    hr->block_start(mr.start())); it.has_next(); it.move_to_next()) {
                oop obj = it.next();
    ...
    public:
        ...
        bool do_heap_region(HeapRegion *hr) {
            ...
            DEBUG_ONLY(HeapWord *
            const top_at_rebuild_start_check = _cm->top_at_rebuild_start(region_idx);)
            ...
            ") == bottom() ("
            PTR_FORMAT
            ") indicates the old region %u is empty (%s)",
            ...
            while (cur < hr->end()) {
                ...
                MemRegion next_chunk = MemRegion(hr->bottom(), top_at_rebuild_start).intersection(
                        MemRegion(cur, chunk_size_in_words));
            ...
            DEBUG_ONLY(HeapWord *
            const top_at_rebuild_start = _cm->top_at_rebuild_start(region_idx);)
            assert(top_at_rebuild_start == NULL ||
                   total_marked_bytes == hr->marked_bytes(),
                   "Marked bytes "
            SIZE_FORMAT
            " for region %u (%s) in [bottom, TAMS) do not match calculated marked bytes "
            SIZE_FORMAT
            " "
            "("
            PTR_FORMAT
