        ...
        if (nm != NULL) {
            ...
            if (!nm->is_alive()) {
                ...
                if (!oop_cl.has_oops_in_region()) {
                    ...
                    "] has nmethod "
                    PTR_FORMAT
                    " in its strong code roots with no pointers into region",
                            p2i(_hr->bottom()), p2i(_hr->end()), p2i(nm));
                    _failures = true;
                    ...
                    ","
                    PTR_FORMAT
                    "] has other failures for nmethod "
                    PTR_FORMAT,
                            p2i(_hr->bottom()), p2i(_hr->end()), p2i(nm));
                    _failures = true;
                }
...
void HeapRegion::verify_strong_code_roots(VerifyOption vo, bool *failures) const {
    ...
    if (is_continues_humongous()) {
        if (strong_code_roots_length > 0) {
            log_error(gc, verify)("region "
            HR_FORMAT
            " is a continuation of a humongous region but has "
            SIZE_FORMAT
            " code root entries",
                    HR_FORMAT_PARAMS(this), strong_code_roots_length);
            *failures = true;
    ...
    *failures = false;
    ...
    while (p < top()) {
        ...
        if (!g1h->is_obj_dead_cond(obj, this, vo)) {
            if (oopDesc::is_oop(obj)) {
                ...
                log_error(gc, verify)(PTR_FORMAT
                " not an oop", p2i(obj));
                *failures = true;
                return;
            }
    ...
    if (is_region_humongous) {
        ...
        if (cast_from_oop<HeapWord *>(obj) > bottom() || cast_from_oop<HeapWord *>(obj) + obj->size() < bottom()) {
            log_error(gc, verify)("this humongous region is not part of its' humongous object "
            PTR_FORMAT, p2i(obj));
            *failures = true;
            return;
        }
    ...
    if (!is_region_humongous && p != top()) {
        log_error(gc, verify)("end of last object "
        PTR_FORMAT
        " "
        "does not match top "
        PTR_FORMAT, p2i(p), p2i(top()));
        *failures = true;
        return;
    }
    ...
    if (p < the_end) {
        ...
        if (b_start_1 != p) {
            log_error(gc, verify)("BOT look up for top: "
            PTR_FORMAT
            " "
            " yielded "
            PTR_FORMAT
        ...
        if (addr_2 < the_end) {
            ...
            if (b_start_2 != p) {
                log_error(gc, verify)("BOT look up for top + 1: "
                PTR_FORMAT
                " "
                " yielded "
                PTR_FORMAT
        ...
        if (addr_3 < the_end) {
            ...
            if (b_start_3 != p) {
                log_error(gc, verify)("BOT look up for top + diff: "
                PTR_FORMAT
                " "
                " yielded "
                PTR_FORMAT
        ...
        if (b_start_4 != p) {
            log_error(gc, verify)("BOT look up for end - 1: "
            PTR_FORMAT
            " "
            " yielded "
            PTR_FORMAT
