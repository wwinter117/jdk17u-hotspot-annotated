...
void
G1NewTracer::report_evacuation_statistics(const G1EvacSummary &young_summary, const G1EvacSummary &old_summary) const {
...
void G1NewTracer::send_basic_ihop_statistics(size_t threshold,
    ...
    if (evt.should_commit()) {
        ...
        evt.set_recentAllocationRate(
                last_allocation_duration != 0.0 ? last_allocation_size / last_allocation_duration : 0.0);
...
void G1NewTracer::send_adaptive_ihop_statistics(size_t threshold,
    ...
    if (evt.should_commit()) {
        ...
        evt.set_thresholdPercentage(
                internal_target_occupancy > 0 ? ((double) threshold / internal_target_occupancy) : 0.0);
