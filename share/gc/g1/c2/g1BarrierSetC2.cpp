...
void G1BarrierSetC2::pre_barrier(GraphKit *kit,
    ...
    assert(in_bytes(SATBMarkQueue::byte_width_of_active()) == 4 || in_bytes(SATBMarkQueue::byte_width_of_active()) == 1,
           "flag width");
    ...
    __ if_then(marking, BoolTest::ne, zero, unlikely);
    {
        ...
        __ if_then(pre_val, BoolTest::ne, kit->null());
        {
            ...
            __ else_();
            {
                ...
                __ make_leaf_call(tf, CAST_FROM_FN_PTR(address, G1BarrierSetRuntime::write_ref_field_pre_entry),
...
void G1BarrierSetC2::g1_mark_card(GraphKit *kit,
    ...
    __ else_();
    {
        __ make_leaf_call(tf, CAST_FROM_FN_PTR(address, G1BarrierSetRuntime::write_ref_field_post_entry),
...
bool G1BarrierSetC2::is_gc_barrier_node(Node *node) const {
    ...
    return strcmp(call->_name, "write_ref_field_pre_entry") == 0 ||
           strcmp(call->_name, "write_ref_field_post_entry") == 0;
}
            ...
            if (c->in(i) != NULL && c->in(i)->is_Region() &&
                ...
                for (uint j = 1; j < r->req(); j++) {
                    if (r->in(j) != NULL && r->in(j)->is_Proj() &&
                        ...
                        r->in(j)->in(0)->as_Call()->entry_point() ==
...
bool
G1BarrierSetC2::escape_add_to_con_graph(ConnectionGraph *conn_graph, PhaseGVN *gvn, Unique_Node_List *delayed_worklist,
                                        Node *n, uint opcode) const {
    if (opcode == Op_StoreP) {
