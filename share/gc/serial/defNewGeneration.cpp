...
void DefNewGeneration::FastKeepAliveClosure::do_oop(narrowOop *p) {
    DefNewGeneration::FastKeepAliveClosure::do_oop_work(p);
}
...
void CLDScanClosure::do_cld(ClassLoaderData *cld) {
    NOT_PRODUCT(ResourceMark
    rm);
    log_develop_trace(gc, scavenge)("CLDScanClosure::do_cld "
    PTR_FORMAT
    ", %s, dirty: %s",
            p2i(cld),
...
HeapWord *DefNewGeneration::allocate_from_space(size_t size) {
    ...
    bool do_alloc = should_try_alloc && (Heap_lock->owned_by_self() || (SafepointSynchronize::is_at_safepoint() &&
    ...
    log_trace(gc, alloc)("DefNewGeneration::allocate_from_space("
    SIZE_FORMAT
    "):  will_fail: %s  heap_lock: %s  free: "
    SIZE_FORMAT
    "%s%s returns %s",
            size,
...
void DefNewGeneration::collect(bool full,
    ...
    GCTraceTime(Trace, gc, phases)
    tm("DefNew", NULL, heap->gc_cause());
...
void DefNewGeneration::gc_epilogue(bool full) {
    DEBUG_ONLY(
    static bool seen_incremental_collection_failed = false;)
