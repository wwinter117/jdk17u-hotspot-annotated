...
void SystemDictionary::post_class_load_event(EventClassLoad *event, const InstanceKlass *k,
...
InstanceKlass *SystemDictionary::load_instance_class_impl(Symbol *class_name, Handle class_loader, TRAPS) {
    ...
    if (class_loader.is_null()) {
        ...
        // find_or_define_instance_class may return a different InstanceKlass
        if (k != NULL) {
            CDS_ONLY(SharedClassLoadingMark
            slm(THREAD, k);)
...
bool SystemDictionary::do_unloading(GCTimer *gc_timer) {
    ...
    {
        GCTraceTime(Debug, gc, phases)
        t("ClassLoaderData", gc_timer);
...
Method *SystemDictionary::find_method_handle_intrinsic(vmIntrinsicID iid,
    ...
    assert(Arguments::is_interpreter_only() || (spe->method()->has_compiled_code() &&
                                                spe->method()->code()->entry_point() ==
                                                spe->method()->from_compiled_entry()),
...
static Method *unpack_method_and_appendix(Handle mname,
    ...
    if (mname.not_null()) {
        ...
        if (m != NULL) {
            ...
            LogTarget(Info, methodhandles)
            lt;
            if (lt.develop_is_enabled()) {
                ResourceMark rm(THREAD);
                LogStream ls(lt);
                ls.print("Linked method="
                INTPTR_FORMAT
                ": ", p2i(m));
                m->print_on(&ls);
                if (appendix != NULL) {
                    ls.print("appendix = ");
                    appendix->print_on(&ls);
                }
                ls.cr();
            }
...
void SystemDictionary::invoke_bootstrap_method(BootstrapInfo &bootstrap_specifier, TRAPS) {
    ...
    if (bootstrap_specifier.caller() == NULL || bootstrap_specifier.type_arg().is_null()) {
        THROW_MSG(vmSymbols::java_lang_InternalError(),
                  "Invalid bootstrap method invocation with no caller or type argument");
    }
