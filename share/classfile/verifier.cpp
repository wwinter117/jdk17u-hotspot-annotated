...
void Verifier::trace_class_resolution(Klass *resolve_class, InstanceKlass *verify_class) {
    ...
    if (source_file != NULL) {
        log_debug(
        class, resolve)("%s %s %s (verification)", verify, resolve, source_file);
    } else {
        log_debug(
        class, resolve)("%s %s (verification)", verify, resolve);
    }
}

// Prints the end-verification message to the appropriate output.
void
Verifier::log_end_verification(outputStream *st, const char *klassName, Symbol *exception_name, oop pending_exception) {
...
bool Verifier::verify(InstanceKlass *klass, bool should_verify_class, TRAPS) {
    ...
    log_info(
    class, init)("Start class verification for: %s", klass->external_name());
    if (klass->major_version() >= STACKMAP_ATTRIBUTE_MAJOR_VERSION) {
        ...
        if (can_failover && !HAS_PENDING_EXCEPTION &&  // Split verifier doesn't set PENDING_EXCEPTION for failure
            ...
            log_info(
            class, init)("Fail over class verification to old verifier for: %s", klass->external_name());
...
void ClassVerifier::verify_class(TRAPS) {
    ...
    if (was_recursively_verified()) {
        ...
        log_info(
        class, init)("Recursive verification detected for: %s",
