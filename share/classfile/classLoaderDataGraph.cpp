...
void ClassLoaderDataGraph::clean_deallocate_lists(bool walk_previous_versions) {
    ...
    log_debug(
    class, loader, data)("clean_deallocate_lists: loaders processed %u %s",
...
bool ClassLoaderDataGraph::is_valid(ClassLoaderData *loader_data) {
    DEBUG_ONLY(
    if (!VMError::is_error_reported()) { assert_locked_or_safepoint(ClassLoaderDataGraph_lock); } )
    if (loader_data != NULL) {
...
bool ClassLoaderDataGraph::do_unloading() {
    ...
    log_debug(
    class, loader, data)("do_unloading: loaders processed %u, loaders removed %u", loaders_processed, loaders_removed);
...
Klass *ClassLoaderDataGraphKlassIteratorAtomic::next_klass() {
    ...
    assert(head == NULL, "head is "
    PTR_FORMAT
    ", expected not null:", p2i(head));
    return NULL;
}
