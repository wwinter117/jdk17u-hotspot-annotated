    ...
    for (int i = 0; i < table_size(); ++i) {
        ...
        while (entry != NULL) {
            ...
            if (pd != NULL) {
                ...
                oops_removed++;
                LogTarget(Debug, protectiondomain, table)
                lt;
                if (lt.is_enabled()) {
                    LogStream ls(lt);
...
void ProtectionDomainCacheTable::print_on(outputStream *st) const {
    ...
    for (int index = 0; index < table_size(); index++) {
        for (ProtectionDomainCacheEntry *probe = bucket(index);
             probe != NULL;
             probe = probe->next()) {
            st->print_cr("%4d: protection_domain: "
            PTR_FORMAT, index, p2i(probe->object_no_keepalive()));
        }
    }
}
...
ProtectionDomainCacheTable::add_entry(int index, unsigned int hash, Handle protection_domain) {
    assert_locked_or_safepoint(SystemDictionary_lock);
