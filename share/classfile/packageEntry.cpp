...
void PackageEntry::set_export_walk_required(ClassLoaderData *m_loader_data) {
    ...
    if (!_must_walk_exports &&
        ...
        if (log_is_enabled(Trace, module)) {
            ...
            log_trace(module)(
                    "PackageEntry::set_export_walk_required(): package %s defined in module %s, exports list must be walked",
                    name()->as_C_string(),
...
void PackageEntry::purge_qualified_exports() {
    ...
    if (_must_walk_exports &&
        ...
        if (log_is_enabled(Trace, module)) {
            ...
            log_trace(module)(
                    "PackageEntry::purge_qualified_exports(): package %s defined in module %s, exports list being walked",
                    name()->as_C_string(),
...
void PackageEntryTable::add_entry(int index, PackageEntry *new_entry) {
    assert(Module_lock->owned_by_self(), "should have the Module_lock");
    Hashtable<Symbol *, mtModule>::add_entry(index, (HashtableEntry < Symbol * , mtModule > *)
    new_entry);
}
    ...
    for (int i = 0; i < table_size(); i++) {
        for (PackageEntry *entry = bucket(i);
            ...
            if (module_name != NULL &&
                ...
                ResourceMark rm;
                vm_exit_during_initialization("A non-"
                JAVA_BASE_NAME
                " package was loaded prior to module system initialization", entry->name()->as_C_string());
            }
        }
    }
}
...
void PackageEntry::print(outputStream *st) {
    ...
    PTR_FORMAT
    " name %s module %s classpath_index "
    INT32_FORMAT
