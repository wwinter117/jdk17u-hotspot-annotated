...
static void compute_offset(int &dest_offset,
    ...
    if (ik == NULL) {
        ResourceMark rm;
        log_error(
        class)("Mismatch JDK version for field: %s type: %s", name_symbol->as_C_string(), signature_symbol->as_C_string());
    ...
    if (!ik->find_local_field(name_symbol, signature_symbol, &fd) || fd.is_static() != is_static) {
        ResourceMark rm;
        log_error(
        class)("Invalid layout of %s field: %s type: %s", ik->external_name(),
        ...
        vm_exit_during_initialization(
                "Invalid layout of well-known class: use -Xlog:class+load=info to see the origin of the problem class");
    }
...
static void compute_offset(int &dest_offset, InstanceKlass *ik,
    ...
    if (name == NULL) {
        ResourceMark rm;
        log_error(
        class)("Name %s should be in the SymbolTable since its class is loaded", name_string);
...
Handle java_lang_String::create_from_str(const char *utf8_str, TRAPS) {
    ...
    if (length > 0) {
        if (!has_multibyte) {
            ...
            ArrayAccess<>::arraycopy_from_native(src, value(h_obj()), typeArrayOopDesc::element_offset<jbyte>(0),
                                                 length);
        } else if (is_latin1) {
...
Handle java_lang_String::create_from_platform_dependent_str(const char *str, TRAPS) {
    ...
    typedef jstring (JNICALL
    *to_java_string_fn_t)(JNIEnv * ,
    const char *);
...
char *java_lang_String::as_platform_dependent_str(Handle java_string, TRAPS) {
    ...
    if (_to_platform_string_fn == NULL) {
        void *lib_handle = os::native_java_library();
        _to_platform_string_fn = CAST_TO_FN_PTR(to_platform_string_fn_t,
                                                os::dll_lookup(lib_handle, "GetStringPlatformChars"));
        if (_to_platform_string_fn == NULL) {
...
const char *java_lang_Thread::thread_status_name(oop java_thread) {
    ...
    switch (status) {
        case JavaThreadStatus::NEW                      :
            return "NEW";
        case JavaThreadStatus::RUNNABLE                 :
            return "RUNNABLE";
        case JavaThreadStatus::SLEEPING                 :
            return "TIMED_WAITING (sleeping)";
        case JavaThreadStatus::IN_OBJECT_WAIT           :
            return "WAITING (on object monitor)";
        case JavaThreadStatus::IN_OBJECT_WAIT_TIMED     :
            return "TIMED_WAITING (on object monitor)";
        case JavaThreadStatus::PARKED                   :
            return "WAITING (parking)";
        case JavaThreadStatus::PARKED_TIMED             :
            return "TIMED_WAITING (parking)";
        case JavaThreadStatus::BLOCKED_ON_MONITOR_ENTER :
            return "BLOCKED (on object monitor)";
        case JavaThreadStatus::TERMINATED               :
            return "TERMINATED";
        default                       :
            return "UNKNOWN";
    };
...
public:
    BacktraceIterator(objArrayHandle result, Thread *thread) {
        init(result, thread);
        assert(_methods.is_null() || _methods->length() == java_lang_Throwable::trace_chunk_size,
               "lengths don't match");
    }
    ...
    if (!version_matches(method, version)) {
        ...
        if (line_number == -2) {
            ...
            if (source_file_name != NULL && (line_number != -1)) {
                ...
                buf_off += os::snprintf_checked(buf + buf_off, buf_size - buf_off, "%s:%d)", source_file_name,
                                                line_number);
            ...
            if (WizardMode && nm != NULL) {
                os::snprintf_checked(buf + buf_off, buf_size - buf_off, "(nmethod "
                INTPTR_FORMAT
                ")", (intptr_t) nm);
            }
        }
...
void java_lang_Throwable::get_stack_trace_elements(Handle throwable,
    ...
    objArrayHandle
    result(THREAD, objArrayOop(backtrace(throwable())));
    BacktraceIterator iter(result, THREAD);
...
Handle java_lang_Throwable::create_initialization_error(JavaThread *current, Handle throwable) {
    ...
    if (init_error->klass()->name() != exception_name) {
        log_info(
        class, init)("Exception thrown while saving initialization exception %s",
    ...
    if (!current->has_pending_exception()) {
        ...
        log_info(
        class, init)("Exception thrown while getting stack trace for initialization exception %s",
                init_error->klass()->external_name());
...
oop java_security_AccessControlContext::create(objArrayHandle context, bool isPrivileged, Handle privileged_context,
                                               TRAPS) {
...
public:
    UnsafeConstantsFixup() {
        ...
        _big_endian = LITTLE_ENDIAN_ONLY(false)
...
int InjectedField::compute_offset() {
    ...
    ResourceMark rm;
    tty->print_cr("Invalid layout of %s at %s/%s%s", ik->external_name(), name()->as_C_string(),
                  signature()->as_C_string(), may_be_java ? " (may_be_java)" : "");
#ifndef PRODUCT
    ik->print();
    tty->print_cr("all fields:");
    for (AllFieldStream fs(ik); !fs.done(); fs.next()) {
        tty->print_cr("  name: %s, sig: %s, flags: %08x", fs.name()->as_C_string(), fs.signature()->as_C_string(),
                      fs.access_flags().as_int());
    }
#endif //PRODUCT
    vm_exit_during_initialization(
            "Invalid layout of well-known class: use -Xlog:class+load=info to see the origin of the problem class");
