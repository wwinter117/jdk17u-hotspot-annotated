...
StackMapFrame *StackMapFrame::frame_in_exception_handler(u1 flags) {
    ...
    StackMapFrame *frame = new StackMapFrame(_offset, flags, _locals_size, 0, _max_locals, _max_stack, _locals, stack,
                                             _verifier);
    return frame;
}
...
bool StackMapFrame::is_assignable_to(
    ...
    if (_max_locals != target->max_locals()) {
        *ctx = ErrorContext::locals_size_mismatch(
                _offset, (StackMapFrame * )
        this, (StackMapFrame *) target);
        return false;
    }
    ...
    if (mismatch_loc != target->locals_size()) {
        *ctx = ErrorContext::bad_type(target->offset(),
                                      TypeOrigin::local(mismatch_loc, (StackMapFrame * )
        this),
    ...
    if (mismatch_loc != _stack_size) {
        *ctx = ErrorContext::bad_type(target->offset(),
                                      TypeOrigin::stack(mismatch_loc, (StackMapFrame * )
        this),
