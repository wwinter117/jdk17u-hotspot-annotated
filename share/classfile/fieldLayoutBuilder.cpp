...
void FieldGroup::add_primitive_field(AllFieldStream fs, BasicType type) {
    ...
    LayoutRawBlock *block = new LayoutRawBlock(fs.index(), LayoutRawBlock::REGULAR, size,
                                               size /* alignment == size for primitive types */, false);
    if (_primitive_fields == NULL) {
        _primitive_fields = new(ResourceObj::RESOURCE_AREA, mtInternal) GrowableArray<LayoutRawBlock *>(
                INITIAL_LIST_SIZE);
    }
...
void FieldGroup::add_oop_field(AllFieldStream fs) {
    ...
    LayoutRawBlock *block = new LayoutRawBlock(fs.index(), LayoutRawBlock::REGULAR, size,
                                               size /* alignment == size for oops */, true);
    if (_oop_fields == NULL) {
...
void FieldLayout::initialize_static_layout() {
    ...
    if (InstanceMirrorKlass::offset_of_static_fields() > 0) {
        insert(first_empty_block(),
               new LayoutRawBlock(LayoutRawBlock::RESERVED, InstanceMirrorKlass::offset_of_static_fields()));
        _blocks->set_offset(0);
    }
}
...
void FieldLayout::initialize_instance_layout(const InstanceKlass *super_klass) {
    if (super_klass == NULL) {
        ...
        _start = _blocks;
        insert(first_empty_block(),
               new LayoutRawBlock(LayoutRawBlock::RESERVED, instanceOopDesc::base_offset_in_bytes()));
    } else {
...
void FieldLayout::fill_holes(const InstanceKlass *super_klass) {
    ...
    LayoutRawBlock::Kind filling_type = super_klass->has_contended_annotations() ? LayoutRawBlock::PADDING
                                                                                 : LayoutRawBlock::EMPTY;
...
FieldLayoutBuilder::FieldLayoutBuilder(const Symbol *classname, const InstanceKlass *super_klass,
...
void FieldLayoutBuilder::epilogue() {
    ...
    if (!_contended_groups.is_empty()) {
        for (int i = 0; i < _contended_groups.length(); i++) {
            FieldGroup *cg = _contended_groups.at(i);
            if (cg->oop_count() > 0) {
                assert(cg->oop_fields() != NULL && cg->oop_fields()->at(0) != NULL,
                       "oop_count > 0 but no oop fields found");
