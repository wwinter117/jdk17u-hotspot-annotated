...
private:
    ...
    enum {
        MODULE_READS_SIZE = 101
    };      // Initial size of list of modules that the module can read.
...
private:
    ...
    static unsigned int compute_hash(Symbol *name) {
        return ((name == NULL) ? 0 : (unsigned int) (name->identity_hash()));
    }
