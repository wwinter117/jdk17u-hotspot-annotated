...
void ClassLoaderData::remove_handle(OopHandle h) {
    ...
    oop *ptr = h.ptr_raw();
    if (ptr != NULL) {
        assert(_handles.owner_of(ptr), "Got unexpected handle "
        PTR_FORMAT, p2i(ptr));
...
void ClassLoaderData::add_to_deallocate_list(Metadata *m) {
    ...
    if (!m->is_shared()) {
        ...
        ResourceMark rm;
        log_debug(
        class, loader, data)("deallocate added for %s", m->print_value_string());
...
void ClassLoaderData::print_value_on(outputStream *out) const {
    if (!is_unloading() && class_loader() != NULL) {
        out->print("loader data: "
        INTPTR_FORMAT
        " for instance ", p2i(this));
        ...
        out->print("loader data: "
        INTPTR_FORMAT
        " of %s", p2i(this), loader_name_and_id());
    }
...
void ClassLoaderData::print_on(outputStream *out) const {
    ...
    if (!_unloading) {
        ...
        INTPTR_FORMAT, p2i(_class_loader.peek()));
    } else {
        out->print_cr(" - class loader        <unloading, oop is bad>");
    }
    out->print_cr(" - metaspace           "
    INTPTR_FORMAT, p2i(_metaspace));
    ...
    switch (_claim) {
        case _claim_none:
            out->print_cr("none");
            break;
        case _claim_finalizable:
            out->print_cr("finalizable");
            break;
        case _claim_strong:
            out->print_cr("strong");
            break;
        case _claim_other:
            out->print_cr("other");
            break;
        case _claim_other | _claim_finalizable:
            out->print_cr("other and finalizable");
            break;
        case _claim_other | _claim_strong:
            out->print_cr("other and strong");
            break;
        default:
            ShouldNotReachHere();
    }
    ...
    ((ClassLoaderData * )
    this)->classes_do(&closure);
    out->print_cr(" }");
    out->print_cr(" - packages            "
    INTPTR_FORMAT, p2i(_packages));
    out->print_cr(" - module              "
    INTPTR_FORMAT, p2i(_modules));
    out->print_cr(" - unnamed module      "
    INTPTR_FORMAT, p2i(_unnamed_module));
    out->print_cr(" - dictionary          "
    INTPTR_FORMAT, p2i(_dictionary));
    if (_jmethod_ids != NULL) {
        out->print(" - jmethod count       ");
        Method::print_jmethod_ids_count(this, out);
        out->print_cr("");
    }
    out->print_cr(" - deallocate list     "
    INTPTR_FORMAT, p2i(_deallocate_list));
    out->print_cr(" - next CLD            "
    INTPTR_FORMAT, p2i(_next));
}
...
void ClassLoaderData::verify() {
    ...
    guarantee(cl != NULL || this == ClassLoaderData::the_null_class_loader_data() || has_class_mirror_holder(),
              "must be");
