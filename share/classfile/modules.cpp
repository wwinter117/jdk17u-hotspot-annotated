...
static PackageEntry *get_locked_package_entry(ModuleEntry *module_entry, const char *package_name, int len) {
    ...
    assert(package_entry == NULL || package_entry->module() == module_entry,
           "Unexpectedly found a package linked to another module");
...
static void define_javabase_module(Handle module_handle, jstring version, jstring location,
    ...
    for (int x = 0; x < num_packages; x++) {
        ...
        if (!verify_package_name(package_name, package_len)) {
            THROW_MSG(vmSymbols::java_lang_IllegalArgumentException(),
                      err_msg("Invalid package name: %s for module: "
            JAVA_BASE_NAME, package_name));
        }
    ...
    assert(ModuleEntryTable::javabase_moduleEntry() != NULL, "No ModuleEntry for "
    JAVA_BASE_NAME);
    ...
    log_info(module, load)(JAVA_BASE_NAME
    " location: %s",
    ...
    if (log_is_enabled(Trace, module)) {
        for (int x = 0; x < pkg_list->length(); x++) {
            log_trace(module)("define_javabase_module(): creation of package %s for module "
            JAVA_BASE_NAME,
        ...
        if (!dupl_modules && existing_pkg == NULL) {
            if (module_table->lookup_only(module_symbol) == NULL) {
                ...
                ModuleEntry *module_entry = module_table->locked_create_entry(module,
                                                                              ...
                                                                              version_symbol, location_symbol,
                                                                              loader_data);
...
jobject Modules::get_module(jclass clazz, TRAPS) {
    assert(ModuleEntryTable::javabase_defined(),
           "Attempt to call get_module before "
    JAVA_BASE_NAME
    " is defined");
...
oop Modules::get_named_module(Handle h_loader, const char *package_name) {
    assert(ModuleEntryTable::javabase_defined(),
           "Attempt to call get_named_module before "
    JAVA_BASE_NAME
    " is defined");
