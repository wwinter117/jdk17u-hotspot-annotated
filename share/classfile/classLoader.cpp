...
DEBUG_ONLY(ClassPathImageEntry
*
ClassPathImageEntry::_singleton = NULL;
)
...
//
ClassFileStream *
ClassPathImageEntry::open_stream_for_loader(JavaThread *current, const char *name, ClassLoaderData *loader_data) {
    jlong size;
    ...
    if (location == 0) {
        ...
        if (pkg_name != NULL) {
            if (!Universe::is_module_initialized()) {
                location = (*JImageFindResource)(jimage_non_null(), JAVA_BASE_NAME, get_jimage_version_string(), name,
                                                 &size);
            } else {
                ...
                if (package_entry != NULL) {
                    ...
                    if (module_name != NULL) {
                        location = (*JImageFindResource)(jimage_non_null(), module_name, get_jimage_version_string(),
                                                         name, &size);
                    }
                }
            }
    ...
    if (location != 0) {
        ...
        // Resource allocated
        assert(this == (ClassPathImageEntry * )
        ClassLoader::get_jrt_entry(), "must be");
...
bool ClassPathImageEntry::is_modules_image() const {
    ...
    assert(this == (ClassPathImageEntry * )
    ClassLoader::get_jrt_entry(), "must be used for jrt entry");
    return true;
}
...
void ClassLoader::setup_patch_mod_entries() {
    ...
    _patch_mod_entries = new(ResourceObj::C_HEAP, mtModule) GrowableArray<ModuleClassPathList *>(num_of_entries,
                                                                                                 mtModule);
    ...
    while (cp_stream.has_next()) {
        ...
        if (set_base_piece) {
            ...
            assert(string_ends_with(path, MODULES_IMAGE_NAME) || string_ends_with(path, JAVA_BASE_NAME),
                   "Incorrect boot loader search path, no java runtime image or "
            JAVA_BASE_NAME
            " exploded build");
            struct stat st;
            if (os::stat(path, &st) == 0) {
...
ClassPathEntry *ClassLoader::create_class_path_entry(JavaThread *current,
    ...
    if ((st->st_mode & S_IFMT) == S_IFREG) {
        ...
        log_info(
        class, path)("opened: %s", path);
        log_info(
        class, load)("opened: %s", path);
    } else {
        // Directory
...

    {
        ...
        if (ModuleEntryTable::javabase_moduleEntry() == NULL) {  // may have been inited by CDS.
            ModuleEntry *jb_module = null_cld_modules->locked_create_entry(Handle(),
                                                                           false, vmSymbols::java_base(), NULL, NULL,
                                                                           null_cld);
            if (jb_module == NULL) {
                vm_exit_during_initialization("Unable to create ModuleEntry for "
                JAVA_BASE_NAME);
            }
