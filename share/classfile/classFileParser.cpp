...
void ClassFileParser::parse_constant_pool_entries(const ClassFileStream *const stream,
    ...
    debug_only(
    const u1 *const old_current = stream->current();)
...
void ClassFileParser::parse_interfaces(const ClassFileStream *const stream,
    ...
    if (itfs_len == 0) {
        ...
        {
            debug_only(NoSafepointVerifier
            nsv;)
            for (index = 0; index < itfs_len; index++) {
...
public:
    enum Location {
        _in_field, _in_method, _in_class
    };
    enum ID {
        _unknown = 0,
                ...
                if (LoadLineNumberTables &&
                    ...
                    if (!lvt_allocated) {
                        ...
                        localvariable_table_start = NEW_RESOURCE_ARRAY_IN_THREAD(
                                THREAD,
                        const unsafe_u2*, INITIAL_MAX_LVT_NUMBER);
                    ...
                    if (lvt_cnt == max_lvt_cnt) {
                        max_lvt_cnt <<= 1;
                        localvariable_table_length = REALLOC_RESOURCE_ARRAY(u2, localvariable_table_length, lvt_cnt,
                                                                            max_lvt_cnt);
                        localvariable_table_start = REALLOC_RESOURCE_ARRAY(
                        const unsafe_u2*, localvariable_table_start, lvt_cnt, max_lvt_cnt);
                    }
                    ...
                    if (lvtt_cnt == max_lvtt_cnt) {
                        max_lvtt_cnt <<= 1;
                        localvariable_type_table_length = REALLOC_RESOURCE_ARRAY(u2, localvariable_type_table_length,
                                                                                 lvtt_cnt, max_lvtt_cnt);
                        localvariable_type_table_start = REALLOC_RESOURCE_ARRAY(
                        const unsafe_u2*, localvariable_type_table_start, lvtt_cnt, max_lvtt_cnt);
                    }
...
static bool
inner_classes_check_loop_through_outer(const Array <u2> *inner_classes, int idx, const ConstantPool *cp, int length) {
    ...
    for (int x = 0; x < components_count; x++) {
        ...
        for (int y = 0; y < attributes_count; y++) {
            ...
            if (attribute_name == vmSymbols::tag_signature()) {
                ...
                if (runtime_visible_annotations != NULL) {
                    classfile_parse_error(
                            "Multiple RuntimeVisibleAnnotations attributes for Record component in class file %s",
                            THREAD);
                    return 0;
                }
                runtime_visible_annotations_length = attribute_length;
                ...
                if (runtime_invisible_annotations_exists) {
                    classfile_parse_error(
                            "Multiple RuntimeInvisibleAnnotations attributes for Record component in class file %s",
                            THREAD);
                    return 0;
                }
                runtime_invisible_annotations_exists = true;
                ...
                if (runtime_visible_type_annotations != NULL) {
                    classfile_parse_error(
                            "Multiple RuntimeVisibleTypeAnnotations attributes for Record component in class file %s",
                            THREAD);
                    return 0;
                }
                runtime_visible_type_annotations_length = attribute_length;
                ...
                if (runtime_invisible_type_annotations_exists) {
                    classfile_parse_error(
                            "Multiple RuntimeInvisibleTypeAnnotations attributes for Record component in class file %s",
                            THREAD);
                    return 0;
                }
                runtime_invisible_type_annotations_exists = true;
                if (PreserveAllAnnotations) {
                    ...
                    assert(runtime_invisible_type_annotations != NULL,
                           "null record component invisible type annotation");
                }
                cfs->skip_u1(attribute_length, CHECK_0);
            ...
            if (tag == vmSymbols::tag_signature()) {
                ...
                if (tag == vmSymbols::tag_nest_members()) {
                    ...
                    if (parsed_nest_host_attribute) {
                        classfile_parse_error("Conflicting NestHost and NestMembers attributes in class file %s",
                                              THREAD);
                        return;
                    }
                    ...
                    if (parsed_nest_members_attribute) {
                        classfile_parse_error("Conflicting NestMembers and NestHost attributes in class file %s",
                                              THREAD);
                        return;
                    }
                    if (_need_verify) {
                        guarantee_property(attribute_length == 2, "Wrong NestHost attribute length in class file %s",
                                           CHECK);
                    }
                    ...
                    if (tag == vmSymbols::tag_record()) {
                        ...
                        if (tag == vmSymbols::tag_permitted_subclasses()) {
                            if (parsed_permitted_subclasses_attribute) {
                                classfile_parse_error("Multiple PermittedSubclasses attributes in class file %s",
                                                      CHECK);
                                return;
                            }
...
static Array<InstanceKlass *> *compute_transitive_interfaces(const InstanceKlass *super,
    ...
    if (max_transitive_size == 0) {
        ...
        ResourceMark rm;
        GrowableArray < InstanceKlass * > *
        const result = new GrowableArray<InstanceKlass *>(max_transitive_size);
        ...
        if (vca_result != Reflection::ACCESS_OK) {
            ...
            if (msg == NULL) {
                ...
                Exceptions::fthrow(
                        ...
                        super->external_name(),
                        (same_module) ? this_klass->joint_in_module_of_loader(super)
                                      : this_klass->class_in_module_of_loader(),
                        (same_module) ? "" : "; ",
        ...
        if (vca_result != Reflection::ACCESS_OK) {
            ...
            if (msg == NULL) {
                ...
                Exceptions::fthrow(
                        ...
                        this_klass->external_name(),
                        k->external_name(),
                        (same_module) ? this_klass->joint_in_module_of_loader(k)
                                      : this_klass->class_in_module_of_loader(),
                        (same_module) ? "" : "; ",
        ...
        switch (signature[0]) {
            ...
            case JVM_SIGNATURE_ARRAY:
                ...
                if (array_dim > 255) {
                    ...
                    classfile_parse_error("Array type descriptor has more than 255 dimensions in class file %s",
                                          THREAD);
                    return NULL;
                }
            ...
            if (CheckIntrinsics) {
                ...
                if (method->intrinsic_id() != vmIntrinsics::_none &&
                    ...
                    tty->print("Compiler intrinsic is defined for method [%s], "
                               ...
                               NOT_DEBUG(" Method will not be inlined.")
                    DEBUG_ONLY(" Exiting.")
                    );
                    tty->cr();
...
void ClassFileParser::fill_instance_klass(InstanceKlass *ik,
    ...
    if (!is_internal()) {
        ...
        if (ik->minor_version() == JAVA_PREVIEW_MINOR_VERSION &&
            ...
            ResourceMark rm;
            log_info(
            class, preview)("Loading class %s that depends on preview features (class file version %d.65535)",
        ...
        if (log_is_enabled(Debug, class, resolve))  {
            ...
            if (local_interfaces != NULL) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    log_debug(
                    class, resolve)("%s %s (interface)", from, to);
                }
            }
        }
    }
