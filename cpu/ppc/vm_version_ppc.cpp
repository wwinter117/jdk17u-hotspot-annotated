...
void VM_Version::initialize() {
    ...
    switch (PowerArchitecturePPC64) {
        case 10:
            if (!VM_Version::has_brw()) break;
        case 9:
            if (!VM_Version::has_darn()) break;
        case 8:
            if (!VM_Version::has_lqarx()) break;
        case 7:
            if (!VM_Version::has_popcntw()) break;
        case 6:
            if (!VM_Version::has_cmpb()) break;
        case 5:
            if (!VM_Version::has_popcntb()) break;
        case 0:
            PowerArchitecturePPC64_ok = true;
            break;
        default:
            break;
    }
    ...
    UINTX_FORMAT
    " on this machine", PowerArchitecturePPC64);
...
void VM_Version::config_dscr() {
    ...
    // Emit code.
    uint64_t(*get_dscr)() = (uint64_t(*)())(void * )
    a->function_entry();
    uint32_t *code = (uint32_t *) a->pc();
    a->mfdscr(R3);
    a->blr();
