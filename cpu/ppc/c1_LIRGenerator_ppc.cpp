...
LIR_Opr LIRGenerator::result_register_for(ValueType *type, bool callee) {
    ...
    switch (type->tag()) {
        case intTag:
            ...
            break;
        case objectTag:
            opr = FrameMap::R3_oop_opr;
            break;
        case longTag:
            opr = FrameMap::R3_long_opr;
            break;
        case floatTag:
            opr = FrameMap::F1_opr;
            break;
        case doubleTag:
            opr = FrameMap::F1_double_opr;
            break;
...
inline bool can_handle_logic_op_as_uimm(ValueType *type, Bytecodes::Code bc) {
    ...
    if ((int_or_long_const & 0xFFFF) == 0 &&
        Assembler::is_uimm((jlong)((julong) int_or_long_const >> 16), 16))
        return true;
    ...
    if (bc == Bytecodes::_land &&
        (is_power_of_2(int_or_long_const + 1) ||
         ...
         (int_or_long_const != min_jlong && is_power_of_2(-int_or_long_const))))
        return true;
