...
#define opp_s_field(x, hi_bit, lo_bit) s_field(x, 31-(lo_bit), 31-(hi_bit))
    ...
    static int ds(int x) {
        assert((x & 0x3) == 0, "unaligned offset");
        return opp_s_field(x, 31, 16);
    }
    ...
    static int l10(int x) {
        assert(x == 0 || x == 1, "must be 0 or 1");
        return opp_u_field(x, 10, 10);
    }
    ...
    static int ra0mem(Register r) {
        assert(r != R0, "cannot use register R0 in memory access");
        return ra(r);
    }

    static int ra0mem(int x) {
        assert(x != 0, "cannot use register 0 in memory access");
        return ra(x);
    }
    ...
    static int thct(int x) {
        assert((x & 8) == 0, "must be valid cache specification");
        return th(x);
    }

    static int thds(int x) {
        assert((x & 8) == 8, "must be valid stream specification");
        return th(x);
    }
...
public:
    ...
    inline int
    add_const_optimized(Register d, Register s, void *a, Register tmp = R0, bool return_simm16_rest = false) {
    ...
    inline int
    sub_const_optimized(Register d, Register s, void *a, Register tmp = R0, bool return_simm16_rest = false) {
