...
bool LIR_Assembler::is_small_constant(LIR_Opr opr) {
    Unimplemented();
    return false; // Currently not used on this platform.
}
...
void LIR_Assembler::arithmetic_idiv(LIR_Code code, LIR_Opr left, LIR_Opr right, LIR_Opr temp, LIR_Opr result,
                                    CodeEmitInfo *info) {
    ...
    if (op->cond() == lir_cond_always) {
        ...
        switch (op->cond()) {
            ...
            case lir_cond_belowEqual:
                assert(op->code() != lir_cond_float_branch, ""); // fallthru
            case lir_cond_lessEqual:
                positive = false;
                cond = Assembler::greater;
                break;
...
int LIR_Assembler::store(LIR_Opr from_reg, Register base, int offset, BasicType type, bool wide, bool unaligned) {
    ...
    if (!Assembler::is_simm16(offset)) {
        ...
        switch (type) {
            ...
            case T_BYTE  :
                __ stb(from_reg->as_register(), offset, base);
                break;
            case T_CHAR  :
            case T_SHORT :
                __ sth(from_reg->as_register(), offset, base);
                break;
            case T_INT   :
...
int LIR_Assembler::store(LIR_Opr from_reg, Register base, Register disp, BasicType type, bool wide) {
    ...
    switch (type) {
        case T_BOOLEAN: // fall through
        case T_BYTE  :
            __ stbx(from_reg->as_register(), base, disp);
            break;
        case T_CHAR  :
        case T_SHORT :
            __ sthx(from_reg->as_register(), base, disp);
            break;
        case T_INT   :
...
int LIR_Assembler::load(Register base, int offset, LIR_Opr to_reg, BasicType type, bool wide, bool unaligned) {
    ...
    if (!Assembler::is_simm16(offset)) {
        ...
        switch (type) {
            case T_BOOLEAN: // fall through
            case T_BYTE  :
                __ lbz(to_reg->as_register(), offset, base);
                __ extsb(to_reg->as_register(), to_reg->as_register());
                break;
            case T_CHAR  :
                __ lhz(to_reg->as_register(), offset, base);
                break;
            case T_SHORT :
                __ lha(to_reg->as_register(), offset, base);
                break;
            case T_INT   :
...
int LIR_Assembler::load(Register base, Register disp, LIR_Opr to_reg, BasicType type, bool wide) {
    ...
    switch (type) {
        case T_BOOLEAN: // fall through
        case T_BYTE  :
            __ lbzx(to_reg->as_register(), base, disp);
            __ extsb(to_reg->as_register(), to_reg->as_register());
            break;
        case T_CHAR  :
            __ lhzx(to_reg->as_register(), base, disp);
            break;
        case T_SHORT :
            __ lhax(to_reg->as_register(), base, disp);
            break;
        case T_INT   :
...
void LIR_Assembler::cmove(LIR_Condition condition, LIR_Opr opr1, LIR_Opr opr2, LIR_Opr result, BasicType type) {
    ...
    if (VM_Version::has_isel() && result->is_cpu_register()) {
        ...
        Register first = o1_is_reg ? (opr1->is_single_cpu() ? opr1->as_register() : opr1->as_register_lo())
                                   : result_reg,
                second = o2_is_reg ? (opr2->is_single_cpu() ? opr2->as_register() : opr2->as_register_lo())
                                   : result_reg;
    ...
    if (right->is_register()) {
        if (dest->is_float_kind()) {
            ...
            if (right->is_single_fpu()) {
                ...
                switch (code) {
                    case lir_add:
                        __ fadds(res, lreg, rreg);
                        break;
                    case lir_sub:
                        __ fsubs(res, lreg, rreg);
                        break;
                    case lir_mul:
                        __ fmuls(res, lreg, rreg);
                        break;
                    case lir_div:
                        __ fdivs(res, lreg, rreg);
                        break;
                    default:
                        ShouldNotReachHere();
                }
                ...
                switch (code) {
                    case lir_add:
                        __ fadd(res, lreg, rreg);
                        break;
                    case lir_sub:
                        __ fsub(res, lreg, rreg);
                        break;
                    case lir_mul:
                        __ fmul(res, lreg, rreg);
                        break;
                    case lir_div:
                        __ fdiv(res, lreg, rreg);
                        break;
                    default:
                        ShouldNotReachHere();
                }
            ...
            switch (code) {
                case lir_add:
                    __ add(dst_lo, op1_lo, op2_lo);
                    break;
                case lir_sub:
                    __ sub(dst_lo, op1_lo, op2_lo);
                    break;
                case lir_mul:
                    __ mulld(dst_lo, op1_lo, op2_lo);
                    break;
                default:
                    ShouldNotReachHere();
            }
            ...
            switch (code) {
                case lir_add:
                    __ add(res, lreg, rreg);
                    break;
                case lir_sub:
                    __ sub(res, lreg, rreg);
                    break;
                case lir_mul:
                    __ mullw(res, lreg, rreg);
                    break;
                default:
                    ShouldNotReachHere();
            }
        ...
        if (dest->is_single_cpu()) {
            ...
            switch (code) {
                case lir_sub:
                    assert(Assembler::is_simm16(-simm16), "cannot encode"); // see do_ArithmeticOp_Int
                    simm16 = -simm16;
                case lir_add:
                    if (res == lreg && simm16 == 0) break;
                    __ addi(res, lreg, simm16);
                    break;
                case lir_mul:
                    if (res == lreg && simm16 == 1) break;
                    __ mulli(res, lreg, simm16);
                    break;
                default:
                    ShouldNotReachHere();
            }
            ...
            switch (code) {
                case lir_sub:
                    assert(Assembler::is_simm16(-con), "cannot encode");  // see do_ArithmeticOp_Long
                    con = -con;
                case lir_add:
                    if (res == lreg && con == 0) break;
                    __ addi(res, lreg, (int) con);
                    break;
                case lir_mul:
                    if (res == lreg && con == 1) break;
                    __ mulli(res, lreg, (int) con);
                    break;
                default:
                    ShouldNotReachHere();
            }
...
void LIR_Assembler::logic_op(LIR_Code code, LIR_Opr left, LIR_Opr right, LIR_Opr dest) {
    if (right->is_constant()) { // see do_LogicOp
        ...
        if (dest->is_single_cpu()) {
            switch (code) {
                case lir_logic_and:
                    __ andr(dest->as_register(), left->as_register(), right->as_register());
                    break;
                case lir_logic_or:
                    __ orr(dest->as_register(), left->as_register(), right->as_register());
                    break;
                case lir_logic_xor:
                    __ xorr(dest->as_register(), left->as_register(), right->as_register());
                    break;
                default:
                    ShouldNotReachHere();
            }
            ...
            switch (code) {
                case lir_logic_and:
                    __ andr(dest->as_register_lo(), l, r);
                    break;
                case lir_logic_or:
                    __ orr(dest->as_register_lo(), l, r);
                    break;
                case lir_logic_xor:
                    __ xorr(dest->as_register_lo(), l, r);
                    break;
                default:
                    ShouldNotReachHere();
            }
...
void LIR_Assembler::emit_arraycopy(LIR_OpArrayCopy *op) {
    ...
    if (op->expected_type() == NULL) {
        assert(src->is_nonvolatile() && src_pos->is_nonvolatile() && dst->is_nonvolatile() &&
               dst_pos->is_nonvolatile() &&
...
void LIR_Assembler::shift_op(LIR_Code code, LIR_Opr left, LIR_Opr count, LIR_Opr dest, LIR_Opr tmp) {
    if (dest->is_single_cpu()) {
        ...
        switch (code) {
            case lir_shl:
                __ slw(dest->as_register(), left->as_register(), tmp->as_register());
                break;
            case lir_shr:
                __ sraw(dest->as_register(), left->as_register(), tmp->as_register());
                break;
            case lir_ushr:
                __ srw(dest->as_register(), left->as_register(), tmp->as_register());
                break;
            default:
                ShouldNotReachHere();
        }
        ...
        switch (code) {
            case lir_shl:
                __ sld(dest->as_register_lo(), left->as_register_lo(), tmp->as_register());
                break;
            case lir_shr:
                __ srad(dest->as_register_lo(), left->as_register_lo(), tmp->as_register());
                break;
            case lir_ushr:
                __ srd(dest->as_register_lo(), left->as_register_lo(), tmp->as_register());
                break;
            default:
                ShouldNotReachHere();
        }
...
void LIR_Assembler::shift_op(LIR_Code code, LIR_Opr left, jint count, LIR_Opr dest) {
    ...
    if (dest->is_single_cpu()) {
        ...
        if (count == 0) { __ mr_if_needed(dest->as_register(), left->as_register()); }
            ...
            switch (code) {
                case lir_shl:
                    __ slwi(dest->as_register(), left->as_register(), count);
                    break;
                case lir_shr:
                    __ srawi(dest->as_register(), left->as_register(), count);
                    break;
                case lir_ushr:
                    __ srwi(dest->as_register(), left->as_register(), count);
                    break;
                default:
                    ShouldNotReachHere();
            }
        ...
        if (count == 0) { __ mr_if_needed(dest->as_pointer_register(), left->as_pointer_register()); }
            ...
            switch (code) {
                case lir_shl:
                    __ sldi(dest->as_pointer_register(), left->as_pointer_register(), count);
                    break;
                case lir_shr:
                    __ sradi(dest->as_pointer_register(), left->as_pointer_register(), count);
                    break;
                case lir_ushr:
                    __ srdi(dest->as_pointer_register(), left->as_pointer_register(), count);
                    break;
                default:
                    ShouldNotReachHere();
            }
        ...
        if (C1OptimizeVirtualCallProfiling && known_klass != NULL) {
            ...
            for (i = 0; i < VirtualCallData::row_limit(); i++) {
                ...
                if (known_klass->equals(receiver)) {
                    __ ld(tmp1,
                          md->byte_offset_of_slot(data, VirtualCallData::receiver_count_offset(i)) - mdo_offset_bias,
                          mdo);
                    __ addi(tmp1, tmp1, DataLayout::counter_increment);
                    __ std(tmp1,
                           md->byte_offset_of_slot(data, VirtualCallData::receiver_count_offset(i)) - mdo_offset_bias,
            ...
            for (i = 0; i < VirtualCallData::row_limit(); i++) {
                ...
                if (receiver == NULL) {
                    ...
                    __ std(tmp1, md->byte_offset_of_slot(data, VirtualCallData::receiver_offset(i)) - mdo_offset_bias,
                           mdo);
