...
OopMap *RegisterSaver::push_frame_reg_args_and_save_live_registers(MacroAssembler *masm,
    ...
    *out_frame_size_in_bytes = frame_size_in_bytes;
    ...
    switch (return_pc_location) {
        case return_pc_is_lr:
            __ mflr(R31);
            break;
        case return_pc_is_pre_saved:
            assert(return_pc_adjustment == 0, "unsupported");
            break;
        case return_pc_is_thread_saved_exception_pc:
            __ ld(R31, thread_(saved_exception_pc));
            break;
        default:
            ShouldNotReachHere();
    }
        ...
        if (r_1->is_Register()) {
            ...
            if (!r_2->is_valid()) {
                ...
                if (sig_bt[i] == T_LONG || sig_bt[i] == T_DOUBLE) {
                    DEBUG_ONLY(__ li(tmp, 0);
                    __ std(tmp, st_off, R1_SP); )
                    st_off -= wordSize;
                }
                __ std(r, st_off, R1_SP);
                st_off -= wordSize;
...
nmethod *SharedRuntime::generate_native_wrapper(MacroAssembler *masm,
    ...
    if (method->is_method_handle_intrinsic()) {
        ...
        intptr_t
                start = (intptr_t)
        __ pc();
        int vep_offset = ((intptr_t)
        __ pc()) -start;
        gen_special_dispatch(masm,
                             method,
                             in_sig_bt,
                             in_regs);
        int frame_complete = ((intptr_t)
        __ pc()) -start;  // not complete, period
        __ flush();
    ...
    // --------------------------------------------------------------------------
    assert(start_pc == (intptr_t)
    __ pc(), "uep must be at start");
