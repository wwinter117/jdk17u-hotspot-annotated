...
#error "unknown OS"
    ...
    __ mr(R5_ARG3, intSlot);
    __ b(loop_start);
    __ mr(R6_ARG4, intSlot);
    __ b(loop_start);
    __ mr(R7_ARG5, intSlot);
    __ b(loop_start);
    __ mr(R8_ARG6, intSlot);
    __ b(loop_start);
    __ mr(R9_ARG7, intSlot);
    __ b(loop_start);
    __ mr(R10_ARG8, intSlot);
    __ b(loop_start);

    __ bind(move_float_arg); // each case must consist of 2 instructions (otherwise adapt LogSizeOfTwoInstructions)
    __ fmr(F1_ARG1, floatSlot);
    __ b(loop_start);
    __ fmr(F2_ARG2, floatSlot);
    __ b(loop_start);
    __ fmr(F3_ARG3, floatSlot);
    __ b(loop_start);
    __ fmr(F4_ARG4, floatSlot);
    __ b(loop_start);
    __ fmr(F5_ARG5, floatSlot);
    __ b(loop_start);
    __ fmr(F6_ARG6, floatSlot);
    __ b(loop_start);
    __ fmr(F7_ARG7, floatSlot);
    __ b(loop_start);
    __ fmr(F8_ARG8, floatSlot);
    __ b(loop_start);
    __ fmr(F9_ARG9, floatSlot);
    __ b(loop_start);
    __ fmr(F10_ARG10, floatSlot);
    __ b(loop_start);
    __ fmr(F11_ARG11, floatSlot);
    __ b(loop_start);
    __ fmr(F12_ARG12, floatSlot);
    __ b(loop_start);
    __ fmr(F13_ARG13, floatSlot);
    __ b(loop_start);
...
address TemplateInterpreterGenerator::generate_ArrayIndexOutOfBounds_handler() {
    ...
    __ call_VM(noreg, CAST_FROM_FN_PTR(address, InterpreterRuntime::throw_ArrayIndexOutOfBoundsException), R4_ARG2,
               R5_ARG3);
    return entry;
}
...
address
TemplateInterpreterGenerator::generate_exception_handler_common(const char *name, const char *message, bool pass_oop) {
    address entry = __ pc();
...
address TemplateInterpreterGenerator::generate_return_entry_for(TosState state, int step, size_t index_size) {
    ...
    switch (state) {
        ...
        case dtos:
            ...
            break; // TOS cache -> GR_FRET
        case vtos:
            break;                           // Nothing to do, this was a void return.
        default  :
            ShouldNotReachHere();
    }
...
void TemplateInterpreterGenerator::generate_counter_incr(Label *overflow) {
    ...
    if (ProfileInterpreter) {
        ...
        const int mdo_ic_offs =
                in_bytes(MethodData::invocation_counter_offset()) + in_bytes(InvocationCounter::counter_offset());
    ...
    const int mo_ic_offs =
            in_bytes(MethodCounters::invocation_counter_offset()) + in_bytes(InvocationCounter::counter_offset());
    __ bind(no_mdo);
...
//
void TemplateInterpreterGenerator::generate_fixed_frame(bool native_call, Register Rsize_of_parameters,
                                                        Register Rsize_of_locals) {
    ...
    if (native_call) {
        ...
        assert(Rsize_of_locals == noreg,
               "Rsize_of_locals not initialized"); // Only relevant value is Rsize_of_parameters.
    } else {
...
address TemplateInterpreterGenerator::generate_math_entry(AbstractInterpreter::MethodKind kind) {
    ...
    switch (kind) {
        case Interpreter::java_lang_math_sqrt:
            use_instruction = VM_Version::has_fsqrt();
            break;
    ...
    switch (kind) {
        ...
        case Interpreter::java_lang_math_pow  :
            runtime_entry = CAST_FROM_FN_PTR(address, SharedRuntime::dpow);
            num_args = 2;
            break;
        case Interpreter::java_lang_math_exp  :
            runtime_entry = CAST_FROM_FN_PTR(address, SharedRuntime::dexp);
            break;
        case Interpreter::java_lang_math_fmaF : /* run interpreted */ num_args = 3;
            double_precision = false;
            break;
        case Interpreter::java_lang_math_fmaD : /* run interpreted */ num_args = 3;
            break;
        default:
            ShouldNotReachHere();
    }
    ...
    if (use_instruction) {
        switch (kind) {
            ...
            case Interpreter::java_lang_math_abs:
                ...
                break;
            case Interpreter::java_lang_math_fmaF:
                __ fmadds(F1_RET, F1, F2, F3);
                break;
            case Interpreter::java_lang_math_fmaD:
                __ fmadd(F1_RET, F1, F2, F3);
                break;
            default:
                ShouldNotReachHere();
        }
...
void TemplateInterpreterGenerator::generate_throw_exception() {
    ...
    Interpreter::_rethrow_exception_entry = __ pc();
    {
        ...
        // Compiled code destroys templateTableBase, reload.
        __ load_const_optimized(R25_templateTableBase, (address) Interpreter::dispatch_table((TosState) 0),
                                R11_scratch1);
    }
    ...
    Interpreter::_throw_exception_entry = __ pc();
    {
        ...
        __ call_VM(Rexception, CAST_FROM_FN_PTR(address, InterpreterRuntime::exception_handler_for_exception),
                   Rexception);
    ...
    Interpreter::_remove_activation_preserving_args_entry = __ pc();
    {
        ...
        __ empty_expression_stack();
        __ unlock_if_synchronized_method(vtos, /* throw_monitor_exception */ false, /* install_monitor_exception */
                                         false);
        ...
        __ call_VM_leaf(CAST_FROM_FN_PTR(address, Deoptimization::popframe_preserve_args), R16_thread, R4_ARG2,
                        R5_ARG3);
    ...
    Interpreter::_remove_activation_entry = __ pc();
    {
        ...
        __ ld(return_pc, 0, R1_SP);
        __ ld(return_pc, _abi0(lr), return_pc);
        __ call_VM_leaf(CAST_FROM_FN_PTR(address, SharedRuntime::exception_handler_for_return_address), R16_thread,
                        return_pc);
...
address TemplateInterpreterGenerator::generate_earlyret_entry_for(TosState state) {
    ...
    switch (state) {
        ...
        case stos:
        case itos:
            __ narrow(R17_tos); /* fall through */
        case ltos:
        case atos:
...
address TemplateInterpreterGenerator::generate_trace_code(TosState state) {
    ...
    __ mflr(R31);
    __ call_VM(noreg, CAST_FROM_FN_PTR(address, InterpreterRuntime::trace_bytecode), /* unused */ R4_ARG2, R5_ARG3,
               R6_ARG4, false);
    __ mtlr(R31);
    __ pop(state);
...
void TemplateInterpreterGenerator::histogram_bytecode(Template *t) {
    int offs = __ load_const_optimized(R11_scratch1, (address) & BytecodeHistogram::_counters[t->bytecode()],
                                       R12_scratch2, true);
    __ lwz(R12_scratch2, offs, R11_scratch1);
    __ addi(R12_scratch2, R12_scratch2, 1);
    __ stw(R12_scratch2, offs, R11_scratch1);
