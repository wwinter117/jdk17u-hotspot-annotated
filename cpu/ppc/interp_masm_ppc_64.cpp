...
void InterpreterMacroAssembler::load_earlyret_value(TosState state, Register Rscratch1) {
    ...
    switch (state) {
        case atos:
            ld(R17_tos, in_bytes(JvmtiThreadState::earlyret_oop_offset()), RjvmtiState);
            std(Rscratch2, in_bytes(JvmtiThreadState::earlyret_oop_offset()), RjvmtiState);
            break;
        case ltos:
            ld(R17_tos, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
            break;
        ...
        case itos:
            lwz(R17_tos, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
            break;
        case ftos:
            lfs(F15_ftos, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
            break;
        case dtos:
            lfd(F15_ftos, in_bytes(JvmtiThreadState::earlyret_value_offset()), RjvmtiState);
            break;
        case vtos:
            break;
        default  :
...
void InterpreterMacroAssembler::profile_arguments_type(Register callee,
    ...
    if (MethodData::profile_arguments() || MethodData::profile_return()) {
        ...
        int off_to_start = is_virtual ?
                           in_bytes(VirtualCallData::virtual_call_data_size()) : in_bytes(
        ...
        if (MethodData::profile_arguments()) {
            ...
            for (int i = 0; i < TypeProfileArgsLimit; i++) {
                ...
                profile_obj_type(tmp1, R28_mdx, in_bytes(TypeEntriesAtCall::argument_type_offset(i)) - off_to_args,
                                 tmp2, tmp1);
