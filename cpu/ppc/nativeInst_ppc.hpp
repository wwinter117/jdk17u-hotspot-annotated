...
public:
    ...
    bool is_pcrelative() {
        assert(MacroAssembler::is_bl64_patchable_at((address)
        this),
        "unexpected call type");
        return MacroAssembler::is_bl64_patchable_pcrelative_at((address)
        this);
    }
    ...
    address destination() const {
        ...
        this),
        "unexpected call type");
        return MacroAssembler::get_dest_of_bl64_patchable_at((address)
        this);
    }
    ...
    void set_destination(address dest) {
        ...
        this),
        "unexpected call type");
        MacroAssembler::set_dest_of_bl64_patchable_at((address)
        this, dest);
    }
...
public:
    ...
    bool is_pcrelative() {
        return MacroAssembler::is_b64_patchable_pcrelative_at((address)
        this);
    }
    ...
    address jump_destination() const {
        if (MacroAssembler::is_b64_patchable_at((address)this)) {
            return MacroAssembler::get_dest_of_b64_patchable_at((address)
            this);
        } else if (MacroAssembler::is_load_const_from_method_toc_at((address)this)
        && Assembler::is_mtctr(*(int *) ((address)
        this + 2 * 4))
        && Assembler::is_bctr(*(int *) ((address)
        this + 3 * 4))) {
    ...
    void set_jump_destination(address dest) {
        // Set new destination (implementation of call may change here).
        if (MacroAssembler::is_b64_patchable_at((address)this)) {
            MacroAssembler::set_dest_of_b64_patchable_at((address)
            this, dest);
        } else if (MacroAssembler::is_load_const_from_method_toc_at((address)this)
        && Assembler::is_mtctr(*(int *) ((address)
        this + 2 * 4))
...
public:
    ...
    address branch_destination() const {
        return MacroAssembler::get_dest_of_bc_far_at((address)
        this);
    }

    void set_branch_destination(address dest) {
        MacroAssembler::set_dest_of_bc_far_at((address)
        this, dest);
    }
};
...
inline bool is_NativeCallTrampolineStub_at(address address) {
    ...
    if (Assembler::is_addis(first_instr) &&
        (Register)(intptr_t)
        Assembler::inv_rt_field(first_instr) == R12_scratch2) return true;
    ...
    if (Assembler::is_ld(first_instr) &&
        (Register)(intptr_t)
        Assembler::inv_rt_field(first_instr) == R12_scratch2 &&
        Assembler::is_mtctr(second_instr) &&
        (Register)(intptr_t)
    Assembler::inv_rs_field(second_instr) == R12_scratch2) return true;
