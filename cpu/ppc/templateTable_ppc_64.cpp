...
void TemplateTable::patch_bytecode(Bytecodes::Code new_bc, Register Rnew_bc, Register Rtemp,
                                   bool load_bc_into_bc_reg /*=true*/, int byte_no) {
...
#else
            __ lbz(Rnew_bc, in_bytes(ConstantPoolCache::base_offset() + ConstantPoolCacheEntry::indices_offset()) + 7 -
                            (1 + byte_no), Rtemp);
#endif
    ...
    if (JvmtiExport::can_post_breakpoint()) {
        ...
        __ call_VM(noreg, CAST_FROM_FN_PTR(address, InterpreterRuntime::set_original_bytecode_at), R19_method, R14_bcp,
                   Rnew_bc);
        __ b(L_patch_done);
...
void TemplateTable::aastore() {
    ...
    __ gen_subtype_check(Rvalue_klass /*subklass*/, Rarray_element_klass /*superklass*/, Rscratch, Rscratch2, Rscratch3,
                         Lstore_ok);
...
void TemplateTable::iop2(Operation op) {
    ...
    switch (op) {
        case add:
            __ add(R17_tos, Rscratch, R17_tos);
            break;
        case sub:
            __ sub(R17_tos, Rscratch, R17_tos);
            break;
        case mul:
            __ mullw(R17_tos, Rscratch, R17_tos);
            break;
        case _and:
        ...
        case _xor:
            ...
            break;
        case shl:
            __ rldicl(R17_tos, R17_tos, 0, 64 - 5);
            __ slw(R17_tos, Rscratch, R17_tos);
            break;
        case shr:
            __ rldicl(R17_tos, R17_tos, 0, 64 - 5);
            __ sraw(R17_tos, Rscratch, R17_tos);
            break;
        case ushr:
            __ rldicl(R17_tos, R17_tos, 0, 64 - 5);
            __ srw(R17_tos, Rscratch, R17_tos);
            break;
        default:
            ShouldNotReachHere();
    }
...
void TemplateTable::fop2(Operation op) {
    ...
    switch (op) {
        case add:
            __ pop_f(F0_SCRATCH);
            __ fadds(F15_ftos, F0_SCRATCH, F15_ftos);
            break;
        case sub:
            __ pop_f(F0_SCRATCH);
            __ fsubs(F15_ftos, F0_SCRATCH, F15_ftos);
            break;
        case mul:
            __ pop_f(F0_SCRATCH);
            __ fmuls(F15_ftos, F0_SCRATCH, F15_ftos);
            break;
        case div:
            __ pop_f(F0_SCRATCH);
            __ fdivs(F15_ftos, F0_SCRATCH, F15_ftos);
            break;
        case rem:
            __ pop_f(F1_ARG1);
...
void TemplateTable::dop2(Operation op) {
    ...
    switch (op) {
        case add:
            __ pop_d(F0_SCRATCH);
            __ fadd(F15_ftos, F0_SCRATCH, F15_ftos);
            break;
        case sub:
            __ pop_d(F0_SCRATCH);
            __ fsub(F15_ftos, F0_SCRATCH, F15_ftos);
            break;
        case mul:
            __ pop_d(F0_SCRATCH);
            __ fmul(F15_ftos, F0_SCRATCH, F15_ftos);
            break;
        case div:
            __ pop_d(F0_SCRATCH);
            __ fdiv(F15_ftos, F0_SCRATCH, F15_ftos);
            break;
        case rem:
            __ pop_d(F1_ARG1);
            __ fmr(F2_ARG2, F15_ftos);
...
void TemplateTable::branch(bool is_jsr, bool is_wide) {
    ...
    if (increment_invocation_counter_for_backward_branches) {
        ...
        if (ProfileInterpreter) {
            ...
            const int mdo_bc_offs =
                    in_bytes(MethodData::backedge_counter_offset()) + in_bytes(InvocationCounter::counter_offset());
        ...
        const int mo_bc_offs =
                in_bytes(MethodCounters::backedge_counter_offset()) + in_bytes(InvocationCounter::counter_offset());
        __ bind(Lno_mdo);
...
void
TemplateTable::if_cmp_common(Register Rfirst, Register Rsecond, Register Rscratch1, Register Rscratch2, Condition cc,
                             bool is_jint, bool cmp0) {
    Label Lnot_taken;
...
#else
    __ lbz(Rscratch,
           in_bytes(ConstantPoolCache::base_offset() + ConstantPoolCacheEntry::indices_offset()) + 7 - (byte_no + 1),
           Rcache);
#endif
...
void TemplateTable::load_invoke_cp_cache_entry(int byte_no,
    ...
    const int method_offset = in_bytes(cp_base_offset + (is_invokevirtual ? ConstantPoolCacheEntry::f2_offset()
...
void TemplateTable::getfield_or_static(int byte_no, bool is_static, RewriteControl rc) {
    ...
    // Check field type.
    __ rldicl(Rflags, Rflags, 64 - ConstantPoolCacheEntry::tos_state_shift,
              64 - ConstantPoolCacheEntry::tos_state_bits);
    ...
    if (JvmtiExport::can_post_field_modification()) {
        ...
        if (is_static) {
            ...
            switch (bytecode()) {
                case Bytecodes::_fast_aputfield:
                    __ push_ptr();
                    offs += Interpreter::stackElementSize;
                    break;
                ...
                case Bytecodes::_fast_lputfield:
                    __ push_l();
                    offs += 2 * Interpreter::stackElementSize;
                    break;
                case Bytecodes::_fast_fputfield:
                    __ push_f();
                    offs += Interpreter::stackElementSize;
        ...
        __ call_VM(noreg, CAST_FROM_FN_PTR(address, InterpreterRuntime::post_field_modification), Robj, Rcache,
                   R6_ARG4);
        __ get_cache_and_index_at_bcp(Rcache, 1);
...
void TemplateTable::prepare_invoke(int byte_no,
    ...
    // Get return address.
    {
        ...
        __ rldicl(Rret_type, Rflags, 64 - ConstantPoolCacheEntry::tos_state_shift,
                  64 - ConstantPoolCacheEntry::tos_state_bits);
...
void TemplateTable::anewarray() {
    ...
    call_VM(R17_tos, CAST_FROM_FN_PTR(address, InterpreterRuntime::anewarray), R4 /* pool */, R5 /* index */,
            R6 /* size */);
...
void TemplateTable::checkcast() {
    ...
    __ gen_subtype_check(RobjKlass, RspecifiedKlass, /*3 temp regs*/ Roffset, Rcpool, Rtags, /*target if subtype*/
                         Ldone);
