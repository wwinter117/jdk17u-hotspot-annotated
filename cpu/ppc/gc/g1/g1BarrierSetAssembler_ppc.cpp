...
void G1BarrierSetAssembler::gen_write_ref_array_pre_barrier(MacroAssembler *masm, DecoratorSet decorators,
    ...
    if (!dest_uninitialized) {
        ...
        const int frame_size = align_up(frame::abi_reg_args_size + spill_slots * BytesPerWord,
        ...
        if (UseCompressedOops) {
            __ call_VM_leaf(CAST_FROM_FN_PTR(address, G1BarrierSetRuntime::write_ref_array_pre_narrow_oop_entry), to,
                            count);
        } else {
...
void G1BarrierSetAssembler::g1_write_barrier_pre(MacroAssembler *masm, DecoratorSet decorators,
    ...
    if (pre_val->is_volatile() && preloaded) {
        __ mr(nv_save, pre_val);
    } // Save pre_val across C call if it was preloaded.
