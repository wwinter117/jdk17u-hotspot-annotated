...
void LIR_Assembler::emit_opBranch(LIR_OpBranch *op) {
    ...
    if (op->cond() == lir_cond_always) {
        ...
        switch (op->cond()) {
            case lir_cond_equal:
                acond = Assembler::bcondEqual;
                break;
            case lir_cond_notEqual:
                acond = Assembler::bcondNotEqual;
                break;
            case lir_cond_less:
                acond = Assembler::bcondLow;
                break;
            case lir_cond_lessEqual:
                acond = Assembler::bcondNotHigh;
                break;
            case lir_cond_greaterEqual:
                acond = Assembler::bcondNotLow;
                break;
            case lir_cond_greater:
                acond = Assembler::bcondHigh;
                break;
...
void LIR_Assembler::call(LIR_OpJavaCall *op, relocInfo::relocType rtype) {
    assert((__ offset() + NativeCall::call_far_pcrelative_displacement_offset) %
           NativeCall::call_far_pcrelative_displacement_alignment == 0,
...
void LIR_Assembler::mem2reg(LIR_Opr src_opr, LIR_Opr dest, BasicType type, LIR_PatchCode patch_code,
    ...
    switch (type) {
        case T_BOOLEAN: // fall through
        case T_BYTE  :
            __ z_lb(dest->as_register(), disp_value, disp_reg, src);
            break;
        case T_CHAR  :
            __ z_llgh(dest->as_register(), disp_value, disp_reg, src);
            break;
        case T_SHORT :
            if (short_disp) {
        ...
        case T_DOUBLE:
            ...
            break;
        case T_LONG  :
            __ z_lg(dest->as_register_lo(), disp_value, disp_reg, src);
            break;
        default      :
            ShouldNotReachHere();
    }
...
void LIR_Assembler::reg2mem(LIR_Opr from, LIR_Opr dest_opr, BasicType type,
    ...
    switch (type) {
        ...
        case T_INT   :
            ...
            break;
        case T_LONG  :
            __ z_stg(from->as_register_lo(), disp_value, disp_reg, dest);
            break;
        case T_ADDRESS:
...
void LIR_Assembler::cmove(LIR_Condition condition, LIR_Opr opr1, LIR_Opr opr2, LIR_Opr result, BasicType type) {
    ...
    switch (condition) {
        case lir_cond_equal:
            acond = Assembler::bcondEqual;
            ncond = Assembler::bcondNotEqual;
            break;
        case lir_cond_notEqual:
            acond = Assembler::bcondNotEqual;
            ncond = Assembler::bcondEqual;
            break;
        case lir_cond_less:
            acond = Assembler::bcondLow;
            ncond = Assembler::bcondNotLow;
            break;
        case lir_cond_lessEqual:
            acond = Assembler::bcondNotHigh;
            ncond = Assembler::bcondHigh;
            break;
        case lir_cond_greaterEqual:
            acond = Assembler::bcondNotLow;
            ncond = Assembler::bcondLow;
            break;
        case lir_cond_greater:
            acond = Assembler::bcondHigh;
            ncond = Assembler::bcondNotHigh;
            break;
        case lir_cond_belowEqual:
            acond = Assembler::bcondNotHigh;
            ncond = Assembler::bcondHigh;
...
void LIR_Assembler::shift_op(LIR_Code code, LIR_Opr left, LIR_Opr count, LIR_Opr dest, LIR_Opr tmp) {
    if (dest->is_single_cpu()) {
        if (left->type() == T_OBJECT) {
            switch (code) {
                case lir_shl:
                    __ z_sllg(dest->as_register(), left->as_register(), 0, count->as_register());
                    break;
                case lir_shr:
                    __ z_srag(dest->as_register(), left->as_register(), 0, count->as_register());
                    break;
                case lir_ushr:
                    __ z_srlg(dest->as_register(), left->as_register(), 0, count->as_register());
                    break;
                default:
                    ShouldNotReachHere();
            }
            ...
            switch (code) {
                case lir_shl:
                    __ z_sllg(dest->as_register(), left->as_register(), 0, masked_count);
                    break;
                case lir_shr:
                    __ z_sra(dest->as_register(), 0, masked_count);
                    break;
                case lir_ushr:
                    __ z_srl(dest->as_register(), 0, masked_count);
                    break;
                default:
                    ShouldNotReachHere();
            }
        ...
        switch (code) {
            case lir_shl:
                __ z_sllg(dest->as_register_lo(), left->as_register_lo(), 0, count->as_register());
                break;
            case lir_shr:
                __ z_srag(dest->as_register_lo(), left->as_register_lo(), 0, count->as_register());
                break;
            case lir_ushr:
                __ z_srlg(dest->as_register_lo(), left->as_register_lo(), 0, count->as_register());
                break;
            default:
                ShouldNotReachHere();
        }
...
void LIR_Assembler::shift_op(LIR_Code code, LIR_Opr left, jint count, LIR_Opr dest) {
    ...
    if (dest->is_single_cpu()) {
        ...
        switch (code) {
            case lir_shl:
                __ z_sllg(dest->as_register(), left->as_register(), count);
                break;
            case lir_shr:
                __ z_sra(dest->as_register(), count);
                break;
            case lir_ushr:
                __ z_srl(dest->as_register(), count);
                break;
            default:
                ShouldNotReachHere();
        }
...
void LIR_Assembler::emit_alloc_obj(LIR_OpAllocObj *op) {
    if (op->init_check()) {
        ...
        __ branch_optimized(Assembler::bcondNotEqual,
                            *op->stub()->entry()); // Use long branch, because slow_case might be far.
    }
...
void LIR_Assembler::emit_typecheck_helper(LIR_OpTypeCheck *op, Label *success, Label *failure, Label *obj_is_null) {
    ...
    if (op->should_profile()) {
        ...
        metadata2reg(md->constant_encoding(), mdo);
        __ add2mem_64(Address(mdo, md->byte_offset_of_slot(data, CounterData::count_offset())),
                      -(int) DataLayout::counter_increment, Rtmp1);
