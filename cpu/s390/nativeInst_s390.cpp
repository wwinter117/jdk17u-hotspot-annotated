...
void NativeInstruction::verify() {
    ...
    if ((addr == 0) || (((unsigned long) addr & ~0x0fff) == 0) || ((intptr_t) addr & 1) != 0) {
        ...
        ": bad instruction address", p2i(addr));
...
void NativeInstruction::print(const char *msg) const {
    ...
    if (msg == NULL) { // Output line without trailing blanks.
        switch (len) {
            case 2:
                tty->print_cr(INTPTR_FORMAT
                "(len=%d): %4.4x", p2i(addr_at(0)), len, halfword_at(0));
                break;
            case 4:
                tty->print_cr(INTPTR_FORMAT
                "(len=%d): %4.4x %4.4x", p2i(addr_at(0)), len, halfword_at(0), halfword_at(2));
                break;
            case 6:
                tty->print_cr(INTPTR_FORMAT
                "(len=%d): %4.4x %4.4x %4.4x", p2i(addr_at(0)), len, halfword_at(0), halfword_at(2), halfword_at(4));
                break;
        ...
        switch (len) {
            case 2:
                tty->print_cr(INTPTR_FORMAT
                "(len=%d): %4.4x           %s", p2i(addr_at(0)), len, halfword_at(0), msg);
                break;
            case 4:
                tty->print_cr(INTPTR_FORMAT
                "(len=%d): %4.4x %4.4x      %s", p2i(addr_at(0)), len, halfword_at(0), halfword_at(2), msg);
                break;
            case 6:
                tty->print_cr(INTPTR_FORMAT
                "(len=%d): %4.4x %4.4x %4.4x %s", p2i(addr_at(0)), len, halfword_at(0), halfword_at(2), halfword_at(
                        4), msg);
                break;
            default: // Never reached. instr_len() always returns one of the above values. Keep the compiler happy.
                ShouldNotReachHere();
...
bool NativeInstruction::is_jump() {
    unsigned long inst;
    Assembler::get_instruction((address)
    this, &inst);
...
void NativeCall::set_destination_mt_safe(address dest) {
    if (MacroAssembler::is_call_far_pcrelative(instruction_address())) {
        ...
        assert(((long) addr_at(MacroAssembler::call_far_pcrelative_size()) &
                (call_far_pcrelative_displacement_alignment - 1)) == 0,
...
address NativeFarCall::destination() {
    assert(MacroAssembler::is_call_far_patchable_at((address)
    this), "unexpected call type");
    address ctable = NULL;
    return MacroAssembler::get_dest_of_call_far_patchable_at((address)
    this, ctable);
}
...
void NativeMovConstReg::verify() {
    ...
    if (!MacroAssembler::is_load_const_from_toc(loc) &&    // Load const from TOC.
        ...
        tty->cr();
        tty->print_cr("NativeMovConstReg::verify(): verifying addr %p(0x%x), %d leading nops", loc, *(uint *) loc,
                      nop_bytes / nop_sz);
        tty->cr();
...
address NativeMovConstReg::next_instruction_address(int offset) const {
    ...
    if (MacroAssembler::is_load_addr_pcrel(inst_addr)) {
        return addr_at(offset + MacroAssembler::load_addr_pcrel_size());
    }

    // Load constant from TOC.
    if (MacroAssembler::is_load_const_from_toc(inst_addr)) {
        return addr_at(offset + MacroAssembler::load_const_from_toc_size());
    }
    ...
    if (MacroAssembler::is_load_narrow_oop(inst_addr)) {
        return addr_at(offset + MacroAssembler::load_narrow_oop_size());
    }
    if (MacroAssembler::is_load_narrow_klass(inst_addr)) {
        return addr_at(offset + MacroAssembler::load_narrow_klass_size());
    }

    // Compare constant narrow inline.
    if (MacroAssembler::is_compare_immediate_narrow_oop(inst_addr)) {
        return addr_at(offset + MacroAssembler::compare_immediate_narrow_oop_size());
    }
    if (MacroAssembler::is_compare_immediate_narrow_klass(inst_addr)) {
        return addr_at(offset + MacroAssembler::compare_immediate_narrow_klass_size());
    }

    if (MacroAssembler::is_call_far_patchable_pcrelative_at(inst_addr)) {
        return addr_at(offset + MacroAssembler::call_far_patchable_size());
    }

    if (MacroAssembler::is_pcrelative_instruction(inst_addr)) {
        return addr_at(offset + Assembler::instr_len(inst_addr));
    }
...
intptr_t NativeMovConstReg::data() const {
    ...
    if (MacroAssembler::is_load_const(loc)) {
        ...
        ((NativeMovConstReg *) loc)->dump(32,
                                          "NativeMovConstReg::data(): cannot extract data from narrow ptr (oop or klass)");
#ifdef LUCY_DBG
        VM_Version::z_SIGSEGV();
#else
        ShouldNotReachHere();
...
void NativeMovConstReg::set_pcrel_data(intptr_t newData, CompiledMethod *passed_nm /* = NULL */) {
    ...
    if (MacroAssembler::is_load_const_from_toc(loc)) {  // Load constant from TOC.
        ...
        ((NativeMovConstReg *) loc)->dump(64,
                                          "NativeMovConstReg::set_pcrel_data() has a problem: setting data for a pc-relative call?");
#ifdef LUCY_DBG
        VM_Version::z_SIGSEGV();
#else
