...
void InterpreterMacroAssembler::load_earlyret_value(TosState state) {
    ...
    switch (state) {
        case atos:
            z_lg(Z_tos, oop_off, RjvmtiState);
            store_const(Address(RjvmtiState, oop_off), 0L, 8, 8, Z_R0_scratch);
            break;
        case ltos:
            z_lg(Z_tos, val_off, RjvmtiState);
            break;
        ...
        case itos:
            z_llgf(Z_tos, val_off, RjvmtiState);
            break;
        case ftos:
            z_le(Z_ftos, val_off, RjvmtiState);
            break;
        case dtos:
            z_ld(Z_ftos, val_off, RjvmtiState);
            break;
...
void InterpreterMacroAssembler::check_and_handle_earlyret(Register scratch_reg) {
    if (JvmtiExport::can_force_early_return()) {
        ...
        assert(JvmtiThreadState::earlyret_pending == 1,
               "must fix this check, when changing the values of the earlyret enum");
...
void InterpreterMacroAssembler::load_resolved_klass_at_offset(Register cpool, Register offset, Register iklass) {
    ...
    z_llgh(offset,
           Address(cpool, offset, sizeof(ConstantPool) + 2)); // offset = resolved_klass_index (s390 is big-endian)
...
void InterpreterMacroAssembler::unlock_if_synchronized_method(TosState state,
    ...
    if (throw_monitor_exception) {
        ...
        MacroAssembler::call_VM(noreg,
                                CAST_FROM_FN_PTR(address, InterpreterRuntime::throw_illegal_monitor_state_exception));
        ...
        if (install_monitor_exception) {
            MacroAssembler::call_VM(noreg,
                                    CAST_FROM_FN_PTR(address, InterpreterRuntime::new_illegal_monitor_state_exception));
        }
        z_bru(unlocked);
    }
...
void InterpreterMacroAssembler::remove_activation(TosState state,
    ...
    if (StackReservedPages > 0) {
        ...
        z_clg(Z_R0, Address(Z_thread,
                            JavaThread::reserved_stack_activation_offset())); // Compare with frame pointer in memory.
...
void InterpreterMacroAssembler::lock_object(Register monitor, Register object) {
    ...
    assert(current_header == displaced_header,
...
void InterpreterMacroAssembler::profile_parameters_type(Register mdp, Register tmp1, Register tmp2) {
    if (ProfileInterpreter && MethodData::profile_parameters()) {
        ...
        Address parm_di_addr(mdp, in_bytes(MethodData::parameters_type_data_di_offset()) -
...
void InterpreterMacroAssembler::increment_invocation_counter(Register Rcounters, Register RctrSum) {
    ...
    int inv_counter_offset = in_bytes(
            MethodCounters::invocation_counter_offset() + InvocationCounter::counter_offset());
