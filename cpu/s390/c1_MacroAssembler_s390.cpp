...
void C1_MacroAssembler::initialize_body(Register objectFields, Register len_in_bytes, Register Rzero) {
    ...
    assert(len_in_bytes->encoding() == (objectFields->encoding() + 1),
           "objectFields and len_in_bytes must be a register pair");
...
void C1_MacroAssembler::allocate_array(
    ...
    switch (elt_size) {
        case 1:
            lgr_if_needed(arr_size, len);
            break;
        case 2:
            z_sllg(arr_size, len, 1);
            break;
        case 4:
            z_sllg(arr_size, len, 2);
            break;
        case 8:
            z_sllg(arr_size, len, 3);
            break;
        default:
            ShouldNotReachHere();
    }
