...
unsigned long MacroAssembler::create_mask(int lBitPos, int rBitPos) {
    ...
    return (lBitPos == 0 ? (unsigned long) (-1L) : ((1UL << (63 - lBitPos + 1)) - 1)) &
           (~((1UL << (63 - rBitPos)) - 1));
}
...
void MacroAssembler::load_sized_value(Register dst, Address src, size_t size_in_bytes, bool is_signed) {
    switch (size_in_bytes) {
        case 8:
            ...
            break;
        case 4:
            is_signed ? z_lgf(dst, src) : z_llgf(dst, src);
            break;
        case 2:
            is_signed ? z_lgh(dst, src) : z_llgh(dst, src);
            break;
        case 1:
            is_signed ? z_lgb(dst, src) : z_llgc(dst, src);
            break;
        default:
            ShouldNotReachHere();
    }
...
void MacroAssembler::load_long_pcrelative(Register Rdst, address dataLocation) {
    ...
    if (!RelAddr::is_in_range_of_RelAddr32(total_distance)) {
        guarantee(RelAddr::is_in_range_of_RelAddr32(total_distance), "load_long_pcrelative can't handle distance "
        INTPTR_FORMAT, total_distance);
    }
...
void MacroAssembler::compare_and_branch_optimized(Register r1,
    ...
    if (VM_Version::has_CompareBranch() && RelAddr::is_in_range_of_RelAddr16(branch_addr, branch_origin)) {
        switch (casenum) {
            case 0:
                z_crj(r1, r2, cond, branch_addr);
                break;
            case 1:
                z_clrj(r1, r2, cond, branch_addr);
                break;
            case 2:
                z_cgrj(r1, r2, cond, branch_addr);
                break;
            case 3:
                z_clgrj(r1, r2, cond, branch_addr);
                break;
            default:
                ShouldNotReachHere();
                break;
...
void MacroAssembler::compare_and_branch_optimized(Register r1,
    ...
    if (VM_Version::has_CompareBranch() && is_RelAddr16 && x2_imm8) {
        switch (casenum) {
            case 0:
                z_cij(r1, x2, cond, branch_target);
                break;
            case 1:
                z_clij(r1, x2, cond, branch_target);
                break;
            case 2:
                z_cgij(r1, x2, cond, branch_target);
                break;
            case 3:
                z_clgij(r1, x2, cond, branch_target);
                break;
            default:
                ShouldNotReachHere();
                break;
    ...
    if (x2 == 0) {
        switch (casenum) {
            ...
            case 1:
                z_ltr(r1, r1);
                break; // Caution: unsigned test only provides zero/notZero indication!
            case 2:
                z_ltgr(r1, r1);
                break;
            case 3:
...
void MacroAssembler::compare_and_branch_optimized(Register r1,
    ...
    if (branch_target.is_bound()) {
        ...
        if (VM_Version::has_CompareBranch() && branch_target.is_near()) {
            switch (casenum) {
                case 0:
                    z_crj(r1, r2, cond, branch_target);
                    break;
                case 1:
                    z_clrj(r1, r2, cond, branch_target);
                    break;
                case 2:
                    z_cgrj(r1, r2, cond, branch_target);
                    break;
                case 3:
                    z_clgrj(r1, r2, cond, branch_target);
                    break;
                default:
                    ShouldNotReachHere();
                    break;
...
unsigned long MacroAssembler::patched_branch(address dest_pos, unsigned long inst, address inst_pos) {
    ...
    if (new_off != (dest_pos - inst_pos)) {
        tty->print_cr("case %d: dest_pos = %p, inst_pos = %p, disp = %ld(%12.12lx)", c, dest_pos, inst_pos, new_off,
                      new_off);
...
void MacroAssembler::patch_target_addr_pcrel(address pc, address con) {
    ...
    if (is_pcrelative_short(inst)) {
        *(short *) (pc + 2) = RelAddr::pcrel_off16(con,
                                                   pc);  // Instructions are at least 2-byte aligned, no test required.
...
bool MacroAssembler::is_call_far_patchable_variant2_at(address instruction_addr) {
    const address call_addr = (address)(
            (intptr_t) instruction_addr + call_far_patchable_size() - call_far_pcrelative_size());
...
bool MacroAssembler::is_call_far_pcrelative(address instruction_addr) {
    ...
    return is_z_nop(instruction_addr) &&
           is_z_brasl(instruction_addr + nop_size());  // Match at position after one nop required.
}
...
void
MacroAssembler::compiler_fast_lock_object(Register oop, Register box, Register temp1, Register temp2, bool try_bias) {
...
void
MacroAssembler::compiler_fast_unlock_object(Register oop, Register box, Register temp1, Register temp2, bool try_bias) {
    Register displacedHeader = temp1;
    ...
    if (UseCompressedClassPointers) {
      ...
      bool     used_R1 = true;
            ...
            if ((base_h != 0) && (base_l == 0) && VM_Version::has_HighWordInstr()) {
                ...
                int pow2_offset = get_oop_base_complement(Z_R1, ((uint64_t)(intptr_t)
                base));
...
void MacroAssembler::oop_encoder(Register Rdst, Register Rsrc, bool maybeNULL,
    ...
    if (maybeNULL) {
        ...
        if (pow2_offset == -1) {
            ...
            pow2_offset = get_oop_base_complement(Rbase, ((uint64_t)(intptr_t)
            oop_base) >> oop_shift);
        }
        ...
        if (oop_shift != 0) {  // Shift out alignment bits
            if (((intptr_t) oop_base & 0xc000000000000000L) ==
                0L) { // We are sure: no single address will have the leftmost bit set.
...
void MacroAssembler::oop_decoder(Register Rdst, Register Rsrc, bool maybeNULL, Register Rbase, int pow2_offset) {
    ...
    if (oop_base != NULL) {
        unsigned int oop_base_hl = ((unsigned int) ((uint64_t)(intptr_t)
        oop_base >> 32)) &0xffff;
        unsigned int oop_base_hh = ((unsigned int) ((uint64_t)(intptr_t)
        oop_base >> 48)) &0xffff;
        unsigned int oop_base_hf = ((unsigned int) ((uint64_t)(intptr_t)
        oop_base >> 32)) &0xFFFFffff;
        if (disjoint && (oop_base_hl == 0 || oop_base_hh == 0)) {
            ...
            if (Rdst == Rbase) {
                assert(!(((Rdst == Z_R0) && (Rsrc == Z_R1)) || ((Rdst == Z_R1) && (Rsrc == Z_R0))),
                       "need a scratch reg");
                if (Rdst != Rsrc) {
            ...
            // Get oop_base components.
            if (!base_preloaded) {
                pow2_offset = get_oop_base(Rbase_tmp, (uint64_t)(intptr_t)
                oop_base);
            }
...
void MacroAssembler::fold_byte_crc32(Register crc, Register val, Register table, Register tmp) {
    ...
    if (crc == val) {      // Must rotate first to use the unmodified value.
        rotate_then_insert(tmp, val, 56 - 2, 63 - 2, 2,
                           true);  // Insert byte 7 of val, shifted left by 2, into byte 6..7 of tmp, clear the rest.
...
void MacroAssembler::update_byteLoop_crc32(Register crc, Register buf, Register len, Register table, Register data) {
    ...
    bind(L_mainLoop);
    z_llgc(data, Address(buf,
                         (intptr_t) 0));// Current byte of input buffer (zero extended). Avoids garbage in upper half of register.
...
void MacroAssembler::save_volatile_regs(Register dst, int offset, bool include_fp, bool include_flags) {
    z_stmg(Z_R1, Z_R5, offset, dst);
    offset += 5 * BytesPerWord;
    if (include_fp) {
        z_std(Z_F0, Address(dst, offset));
        offset += BytesPerWord;
        z_std(Z_F1, Address(dst, offset));
        offset += BytesPerWord;
        z_std(Z_F2, Address(dst, offset));
        offset += BytesPerWord;
        z_std(Z_F3, Address(dst, offset));
        offset += BytesPerWord;
        z_std(Z_F4, Address(dst, offset));
        offset += BytesPerWord;
        z_std(Z_F5, Address(dst, offset));
        offset += BytesPerWord;
        z_std(Z_F6, Address(dst, offset));
        offset += BytesPerWord;
        z_std(Z_F7, Address(dst, offset));
        offset += BytesPerWord;
    }
    if (include_flags) {
        Label done;
...
void MacroAssembler::restore_volatile_regs(Register src, int offset, bool include_fp, bool include_flags) {
    z_lmg(Z_R1, Z_R5, offset, src);
    offset += 5 * BytesPerWord;
    if (include_fp) {
        z_ld(Z_F0, Address(src, offset));
        offset += BytesPerWord;
        z_ld(Z_F1, Address(src, offset));
        offset += BytesPerWord;
        z_ld(Z_F2, Address(src, offset));
        offset += BytesPerWord;
        z_ld(Z_F3, Address(src, offset));
        offset += BytesPerWord;
        z_ld(Z_F4, Address(src, offset));
        offset += BytesPerWord;
        z_ld(Z_F5, Address(src, offset));
        offset += BytesPerWord;
        z_ld(Z_F6, Address(src, offset));
        offset += BytesPerWord;
        z_ld(Z_F7, Address(src, offset));
        offset += BytesPerWord;
    }
    if (include_flags) {
...
address MacroAssembler::stop_chain(address reentry, int type, const char *msg, int id, bool allow_relocation) {
    BLOCK_COMMENT(err_msg("stop_chain(%s,%s): %s {", reentry == NULL ? "init" : "cont",
                          allow_relocation ? "reloc " : "static", msg));
