...
private:
    static bool
    is_call_far_patchable_variant0_at(address instruction_addr); // Dynamic TOC: load target addr from CP and call.
...
public:
    ...
    static bool is_call_far_pcrelative(
            address instruction_addr);         // Pure far pc-relative call, with one leading size adjustment nop.
...
public:
    ...
    void
    store_klass(Register klass, Register dst_oop, Register ck = noreg); // Klass will get compressed if ck not provided.
...
public:
    ...
    void asm_assert_static(bool check_equal, const char *msg, int id)
...
private:
    ...
    address stop_chain(address reentry, int type, const char *msg, int id,
                       bool allow_relocation); // Non-relocateable code only!!
    void stop_static(int type, const char *msg,
                     int id);                                        // Non-relocateable code only!!

public:

    // Prints msg and stops.
    address stop_chain(address reentry, const char *msg = "", int id = 0) {
        return stop_chain(reentry, stop_stop, msg, id, true);
    }

    address stop_chain_static(address reentry, const char *msg = "", int id = 0) {
        return stop_chain(reentry, stop_stop, msg, id, false);
    }
