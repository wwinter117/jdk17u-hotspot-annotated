...
int RegisterSaver::live_reg_save_size(RegisterSet reg_set) {
    ...
    switch (reg_set) {
        case all_registers:
            reg_space = sizeof(RegisterSaver_LiveRegs);
            break;
        case all_registers_except_r2:
            reg_space = sizeof(RegisterSaver_LiveRegsWithoutR2);
            break;
        case all_integer_registers:
            reg_space = sizeof(RegisterSaver_LiveIntRegs);
            break;
        case all_volatile_registers:
            reg_space = sizeof(RegisterSaver_LiveVolatileRegs);
            break;
        case arg_registers:
            reg_space = sizeof(RegisterSaver_LiveArgRegs);
            break;
        default:
            ShouldNotReachHere();
    }
...
nmethod *SharedRuntime::generate_native_wrapper(MacroAssembler *masm,
        ...
        switch (in_sig_bt[jix]) {
            ...
            case T_ARRAY:
                if (is_critical_native) {
                    ...
                    unpack_array_argument(masm, in_regs[jix], in_elem_bt[jix], out_regs[body_arg], out_regs[cix],
                                          stack_slots);
                    break;
                }
                // else fallthrough
            ...
            case T_DOUBLE:
                assert(jix + 1 < total_in_args && in_sig_bt[jix + 1] == T_VOID && out_sig_bt[cix + 1] == T_VOID,
                       "bad arg list");
    ...
    switch (ret_type) {
        case T_VOID:
            break;         // Nothing to do!
        case T_FLOAT:
            break;         // Got it where we want it (unless slow-path)
        case T_DOUBLE:
            break;         // Got it where we want it (unless slow-path)
        case T_LONG:
            break;         // Got it where we want it (unless slow-path)
        case T_OBJECT:
            break;         // Really a handle.
        ...
        case T_BOOLEAN:                // 0 -> false(0); !0 -> true(1)
            ...
            break;
        case T_BYTE:
            __ z_lgbr(Z_RET, Z_RET);
            break; // sign extension
        case T_CHAR:
            __ z_llghr(Z_RET, Z_RET);
            break; // unsigned result
        case T_SHORT:
            __ z_lghr(Z_RET, Z_RET);
            break; // sign extension
        case T_INT:
            __ z_lgfr(Z_RET, Z_RET);
            break; // sign-extend for beauty.
    ...
    nmethod *nm = nmethod::new_native_nmethod(method,
                                              ...
                                              stack_slots / VMRegImpl::slots_per_word,
                                              (method_is_static ? in_ByteSize(klass_offset) : in_ByteSize(
                                                      receiver_offset)),
...
void SharedRuntime::gen_i2c_adapter(MacroAssembler *masm,
    ...
    if (comp_args_on_stack) {
        ...
        int comp_words_on_stack =
                align_up(comp_args_on_stack * VMRegImpl::stack_slot_size, wordSize) >> LogBytesPerWord;
    ...
    for (int i = 0; i < total_args_passed; i++) {
        ...
        if (r_1->is_FloatRegister()) {
            ...
            if (r_1->is_stack()) {
                ...
                int st_off =
                        (r_1->reg2stack() + SharedRuntime::out_preserve_stack_slots()) * VMRegImpl::stack_slot_size;
...
AdapterHandlerEntry *SharedRuntime::generate_i2c2i_adapters(MacroAssembler *masm,
    ...
    Label skip_fixup;
    {
        ...
        if (UseCompressedClassPointers) {
            __ z_lg(Z_R11, holder_klass_offset,
                    Z_method);             // Z_R11 is overwritten a few instructions down anyway.
...
void SharedRuntime::generate_deopt_blob() {
...

    {
        ...
        __ compare64_and_branch(exec_mode_reg, Deoptimization::Unpack_exception, Assembler::bcondNotEqual,
                                skip_restore_excp);
    ...
    _deopt_blob = DeoptimizationBlob::create(&buffer, oop_maps, 0, exception_offset, reexecute_offset,
                                             RegisterSaver::live_reg_frame_size(RegisterSaver::all_registers) /
                                             wordSize);
...
SafepointBlob *SharedRuntime::generate_handler_blob(address call_ptr, int poll_type) {
    ...
    if (!cause_return) {
        ...
        const int offset_of_return_pc =
                _z_abi16(return_pc) + RegisterSaver::live_reg_frame_size(RegisterSaver::all_registers);
    ...
    return SafepointBlob::create(&buffer, oop_maps,
                                 RegisterSaver::live_reg_frame_size(RegisterSaver::all_registers) / wordSize);
}
