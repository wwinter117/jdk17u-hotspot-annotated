...
void TemplateInterpreterGenerator::generate_fixed_frame(bool native_call) {
    ...
    BLOCK_COMMENT("generate_fixed_frame {");
    {
        ...
        if (native_call) {
            ...
            __ compare64_and_branch(max_stack, argument_registers_in_bytes, Assembler::bcondNotLow,
                                    passing_args_on_stack);
  ...
  //           [operand stack]          = max_stack                          | = top_frame_size
        ...
        if (!native_call) {
            __ z_sllg(Z_R0_scratch, local_count,
                      LogBytesPerWord); // Params have already been subtracted from local_count.
...

    {
        ...
        const bool check_for_mdo = !native_call
        DEBUG_ONLY( || native_call);
        if (ProfileInterpreter && check_for_mdo) {
            ...
            DEBUG_ONLY(
            if (native_call) __ stop("native methods don't have a mdo"));
...
void TemplateInterpreterGenerator::generate_throw_exception() {
    ...
    __ remove_activation(vtos, noreg/*ret.pc already loaded*/, false/*throw exc*/, true/*install exc*/,
                         false/*notify jvmti*/);
    __ z_lg(Z_fp, _z_abi(callers_sp), Z_SP); // Restore frame pointer.
