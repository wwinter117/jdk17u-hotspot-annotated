...
#define DEF_XMM_OFFS(regnum) xmm ## regnum ## _off = xmm_off + (regnum)*16/BytesPerInt, xmm ## regnum ## H_off
    ...
    enum {
        FPU_regs_live = flags_off - fpu_state_end
    };
...
int SharedRuntime::java_calling_convention(const BasicType *sig_bt,
    ...
    enum {
        fltarg_dontuse = 9999 + 0, fltarg_float_only = 9999 + 1, fltarg_flt_dbl = 9999 + 2
    };
    ...
    for (int i = 0; i < total_args_passed; i++) {
        ...
        if (r_1->is_stack()) {
            ...
            if (!r_2->is_valid()) {
                ...
                sig_bt[i] == T_LONG || sig_bt[i] == T_DOUBLE)?
                next_off :
                ld_off;
...
nmethod *SharedRuntime::generate_native_wrapper(MacroAssembler *masm,
    ...
    if (method->is_synchronized()) {
        ...
        if (UseBiasedLocking) {
            ...
            __ biased_locking_enter(lock_reg, obj_reg, swap_reg, oop_handle_reg, noreg, false, lock_done,
                                    &slow_path_lock);
        }
...
void SharedRuntime::generate_deopt_blob() {
    ...
    if (UseSSE >= 1) __ movflt(xmm0, Address(rsp, (RegisterSaver::xmm0Offset() + additional_words + 1) * wordSize));
        ...
        __ fld_d(Address(rsp, (RegisterSaver::fpResultOffset() + additional_words + 1) * wordSize));
    __ jmp(results_done);
