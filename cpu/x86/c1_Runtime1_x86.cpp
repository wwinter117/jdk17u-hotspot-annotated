...
int StubAssembler::call_RT(Register oop_result1, Register metadata_result, address entry, int args_size) {
    // setup registers
    const Register thread = NOT_LP64(rdi)
    LP64_ONLY(r15_thread); // is callee-saved register (Visual C++ calling conventions)
...
OopMapSet *Runtime1::generate_handle_exception(StubID id, StubAssembler *sasm) {
    ...
    switch (id) {
        ...
        case handle_exception_from_callee_id: {
            ...
            WIN64_ONLY(+frame::arg_reg_save_area_bytes / BytesPerWord);
    ...
    switch (id) {
        ...
        case new_object_array_id: {
            ...
            if (!UseTLAB && Universe::heap()->supports_inline_contig_alloc()) {
                ...
                const Register thread = NOT_LP64(t2)
                LP64_ONLY(r15_thread);
                NOT_LP64(__ get_thread(thread));
                __ eden_allocate(thread, obj, arr_size, 0, t1, slow_path);  // preserves arr_size

                __ initialize_header(obj, klass, length, t1, t2);
                __ movb(t1, Address(klass, in_bytes(Klass::layout_helper_offset()) +
        ...
        case register_finalizer_id: {
            ...
            Register tmp_load_klass = LP64_ONLY(rscratch1)
            NOT_LP64(noreg);
            Register t = rsi;
            ...
            int call_offset = __ call_RT(noreg, noreg, CAST_FROM_FN_PTR(address, SharedRuntime::register_finalizer),
                                         rax);
            oop_maps = new OopMapSet();
            oop_maps->add_gc_map(call_offset, oop_map);
        ...
        case dtrace_object_alloc_id: { // rax,: object
            StubFrame f(sasm, "dtrace_object_alloc", dont_gc_arguments);
            // we can't gc here so skip the oopmap but make sure that all
            // the live registers get saved.
            save_live_registers(sasm, 1);

            __ NOT_LP64(push(rax))
            LP64_ONLY(mov(c_rarg0, rax));
