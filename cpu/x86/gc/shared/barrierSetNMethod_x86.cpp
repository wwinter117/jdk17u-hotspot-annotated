...
void NativeNMethodCmpBarrier::verify() const {
    ...
    if (inst != instruction_code) {
        tty->print_cr("Addr: "
        INTPTR_FORMAT
        " Code: 0x%x", p2i(instruction_address()),
                inst);
        fatal("not a cmp barrier");
    }
    ...
    if (modrm != instruction_modrm) {
        tty->print_cr("Addr: "
        INTPTR_FORMAT
        " mod/rm: 0x%x", p2i(instruction_address()),
                modrm);
        fatal("not a cmp barrier");
    }
}
...
void BarrierSetNMethod::deoptimize(nmethod *nm, address *return_address_ptr) {
    ...
    LogTarget(Trace, nmethod, barrier)
    out;
    if (out.is_enabled()) {
        JavaThread *jth = JavaThread::current();
        ResourceMark mark;
        log_trace(nmethod, barrier)(
                "deoptimize(nmethod: %p, return_addr: %p, osr: %d, thread: %p(%s), making rsp: %p) -> %p",
...
static const int entry_barrier_offset = LP64_ONLY(-19)
NOT_LP64(-18);
