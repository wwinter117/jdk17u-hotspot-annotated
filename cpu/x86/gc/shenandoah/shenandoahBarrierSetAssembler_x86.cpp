...
static void save_machine_state(MacroAssembler *masm, bool handle_gpr, bool handle_fp) {
    ...
    if (handle_fp) {
        ...
        if (UseSSE >= 2) {
            const int xmm_size = wordSize * LP64_ONLY(2)
            NOT_LP64(4);
            ...
            const int xmm_size = wordSize * LP64_ONLY(1)
            NOT_LP64(2);
            __ subptr(rsp, xmm_size * 8);
...
void ShenandoahBarrierSetAssembler::satb_write_barrier_pre(MacroAssembler *masm,
    ...
    if (expand_call) {
        ...
        __ push(pre_val);
#endif
        __ MacroAssembler::call_VM_leaf_base(CAST_FROM_FN_PTR(address, ShenandoahRuntime::write_ref_field_pre_entry),
                                             2);
    } else {
        __ call_VM_leaf(CAST_FROM_FN_PTR(address, ShenandoahRuntime::write_ref_field_pre_entry), LP64_ONLY(c_rarg0)
        NOT_LP64(pre_val), thread);
    }
...
void ShenandoahBarrierSetAssembler::load_reference_barrier(MacroAssembler *masm, Register dst, Address src,
    ...
    if (is_strong) {
        if (is_narrow) {
            __ call_VM_leaf(CAST_FROM_FN_PTR(address, ShenandoahRuntime::load_reference_barrier_strong_narrow), arg0,
                            arg1);
        } else {
        ...
        if (is_narrow) {
            __ super_call_VM_leaf(CAST_FROM_FN_PTR(address, ShenandoahRuntime::load_reference_barrier_weak_narrow),
                                  arg0, arg1);
        } else {
            __ super_call_VM_leaf(CAST_FROM_FN_PTR(address, ShenandoahRuntime::load_reference_barrier_weak), arg0,
                                  arg1);
        }
    } else {
