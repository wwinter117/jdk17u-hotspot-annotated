...
void InterpreterMacroAssembler::profile_arguments_type(Register mdp, Register callee, Register tmp, bool is_virtual) {
    ...
    if (MethodData::profile_arguments() || MethodData::profile_return()) {
        ...
        int off_to_start = is_virtual ? in_bytes(VirtualCallData::virtual_call_data_size()) : in_bytes(
                CounterData::counter_data_size());
...
void InterpreterMacroAssembler::profile_parameters_type(Register mdp, Register tmp1, Register tmp2) {
    if (ProfileInterpreter && MethodData::profile_parameters()) {
        ...
        movl(tmp1, Address(mdp, in_bytes(MethodData::parameters_type_data_di_offset()) -
...
void InterpreterMacroAssembler::check_and_handle_earlyret(Register java_thread) {
    if (JvmtiExport::can_force_early_return()) {
        Label L;
        Register
        tmp = LP64_ONLY(c_rarg0)
        NOT_LP64(java_thread);
        Register
...
void InterpreterMacroAssembler::get_cache_and_index_and_bytecode_at_bcp(Register cache,
    ...
    movl(bytecode, Address(cache, index, Address::times_ptr,
                           ConstantPoolCache::base_offset() + ConstantPoolCacheEntry::indices_offset()));
...
void InterpreterMacroAssembler::dispatch_base(TosState state,
    ...
    if (table != safepoint_table && generate_poll) {
        ...
        jccb(Assembler::zero, no_safepoint);
        ArrayAddress
        dispatch_addr(ExternalAddress((address) safepoint_table), index);
        jump(dispatch_addr);
        bind(no_safepoint);
    }
    ...
    {
        ArrayAddress
        dispatch_addr(ExternalAddress((address) table), index);
        jump(dispatch_addr);
    }
#endif // _LP64
}
...
void InterpreterMacroAssembler::remove_activation(
    ...
    const Register rthread = LP64_ONLY(r15_thread)
    NOT_LP64(rcx);
    const Register robj = LP64_ONLY(c_rarg1)
    NOT_LP64(rdx);
    const Register rmon = LP64_ONLY(c_rarg1)
    NOT_LP64(rcx);
...
void InterpreterMacroAssembler::lock_object(Register lock_reg) {
    assert(lock_reg == LP64_ONLY(c_rarg1)
    NOT_LP64(rdx),
    ...
    if (UseHeavyMonitors) {
        ...
        const Register obj_reg = LP64_ONLY(c_rarg3)
        NOT_LP64(rcx); // Will contain the oop
        ...
        const int zero_bits = LP64_ONLY(7)
        NOT_LP64(3);
...
void InterpreterMacroAssembler::unlock_object(Register lock_reg) {
    ...
    if (UseHeavyMonitors) {
        ...
        const Register swap_reg = rax;  // Must use rax for cmpxchg instruction
        const Register header_reg = LP64_ONLY(c_rarg2)
        NOT_LP64(rbx);  // Will contain the old oopMark
        const Register obj_reg = LP64_ONLY(c_rarg3)
        NOT_LP64(rcx);  // Will contain the oop
