  ...
  inc_counter_np_(counter);
    ...
    { Label L;
      ...
      __ bind(L);
        ...
        __ call_VM_leaf(CAST_FROM_FN_PTR(address, SharedRuntime::exception_handler_for_return_address), thread,
                        exception_pc);
    ...
    address generate_d2i_wrapper(BasicType t, address fcn) {
        ...
        if (t == T_INT) { BLOCK_COMMENT("SharedRuntime::d2i"); }
        else if (t == T_LONG) { BLOCK_COMMENT("SharedRuntime::d2l"); }
...
#define COUNT  Address(rsp, 12+12)   // Only for oop arraycopy
    ...
    // can optionally specify that the shuffle mask is already in an xmmregister
    void
    aes_enc_key(XMMRegister xmmdst, XMMRegister xmmtmp, Register key, int offset, XMMRegister xmm_shuf_mask = NULL) {
    ...
    // can optionally specify that the shuffle mask is already in an xmmregister
    void
    aes_dec_key(XMMRegister xmmdst, XMMRegister xmmtmp, Register key, int offset, XMMRegister xmm_shuf_mask = NULL) {
        load_key(xmmtmp, key, offset, xmm_shuf_mask);
...
#define CTR_DoFour(opc, src_reg)               \
  ...
  */
    ...
    address generate_method_entry_barrier() {
        ...
        __ lea(rbx,
               Address(rsp, wordSize * 3)); // 1 for cookie, 1 for rbp, 1 for rbx - this should be the return address
        ...
        __ call_VM_leaf(CAST_FROM_FN_PTR(address, static_cast<int (*)(
                address *)>(BarrierSetNMethod::nmethod_stub_entry_barrier)), rbx);
...
#define __ masm->
    ...
    void generate_all() {
        ...
        StubRoutines::x86::_vector_double_sign_mask = generate_vector_mask_long_double("vector_double_sign_mask",
                                                                                       0x7FFFFFFF, 0xFFFFFFFF);
        StubRoutines::x86::_vector_double_sign_flip = generate_vector_mask_long_double("vector_double_sign_flip",
        ...
        StubRoutines::x86::_vector_long_shuffle_mask = generate_vector_mask_long_double("vector_long_shuffle_mask",
                                                                                        0x00000001, 0x0);
        StubRoutines::x86::_vector_byte_perm_mask = generate_vector_byte_perm_mask("vector_byte_perm_mask");
        StubRoutines::x86::_vector_long_sign_mask = generate_vector_mask_long_double("vector_long_sign_mask",
                                                                                     0x80000000, 0x00000000);
