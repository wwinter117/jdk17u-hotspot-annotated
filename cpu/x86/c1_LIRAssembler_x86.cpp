...
static jlong *float_signmask_pool = double_quadword(&fp_signmask_pool[1 * 2], CONST64(0x7FFFFFFF7FFFFFFF),
                                                    CONST64(0x7FFFFFFF7FFFFFFF));
static jlong *double_signmask_pool = double_quadword(&fp_signmask_pool[2 * 2], CONST64(0x7FFFFFFFFFFFFFFF),
                                                     CONST64(0x7FFFFFFFFFFFFFFF));
...
const Register
IC_Klass = rax;   // where the IC klass is cached
...
static void select_different_registers(Register
preserve,
Register extra,
        Register
&tmp1,
...
int LIR_Assembler::check_icache() {
    ...
    const int ic_cmp_size = LP64_ONLY(10)
    NOT_LP64(9);
...
void LIR_Assembler::jobject2reg_with_patching(Register
reg,
CodeEmitInfo *info
) {
jobject o = NULL;
...
void LIR_Assembler::klass2reg_with_patching(Register
reg,
CodeEmitInfo *info
) {
Metadata *o = NULL;
...
void LIR_Assembler::return_op(LIR_Opr result, C1SafepointPollStub *code_stub) {
    assert(result->is_illegal() || !result->is_single_cpu() || result->as_register() == rax,
           "word returns are in rax,");
...
int LIR_Assembler::safepoint_poll(LIR_Opr tmp, CodeEmitInfo *info) {
  ...
  __ movptr(poll_addr, Address(r15_thread, JavaThread::polling_page_offset()));
    ...
    guarantee(pointer_delta(post_pc, pre_pc, 1) == 2
    LP64_ONLY(+1), "must be exact length");
    return offset;
}


void LIR_Assembler::move_regs(Register
from_reg,
Register to_reg
) {
...
void LIR_Assembler::swap_reg(Register
a,
Register b
) {
__
...
mdo,
ciMethodData *md, ciProfileData
*data,
Register recv, Label
* update_done) {
for (
uint i = 0;
i<ReceiverTypeData::row_limit();
i++) {
Label next_test;
// See if the receiver is receiver[n].
__
cmpptr(recv, Address(mdo, md->byte_offset_of_slot(data, ReceiverTypeData::receiver_offset(i)))
);
__
jccb(Assembler::notEqual, next_test
);
Address data_addr(mdo, md->byte_offset_of_slot(data, ReceiverTypeData::receiver_count_offset(i)));
__
addptr(data_addr, DataLayout::counter_increment
);
__
jmp(*update_done);
__
...
__
cmpptr(recv_addr, (intptr_t)
NULL_WORD);
__
jccb(Assembler::notEqual, next_test
);
__
movptr(recv_addr, recv
);
__
movptr(Address(mdo,
               md->byte_offset_of_slot(data, ReceiverTypeData::receiver_count_offset(i))), DataLayout::counter_increment
);
__
jmp(*update_done);
__
...
void LIR_Assembler::emit_typecheck_helper(LIR_OpTypeCheck *op, Label *success, Label *failure, Label *obj_is_null) {
    ...
    Register
    tmp_load_klass = LP64_ONLY(rscratch1)
    NOT_LP64(noreg);
...
void LIR_Assembler::cmove(LIR_Condition condition, LIR_Opr opr1, LIR_Opr opr2, LIR_Opr result, BasicType type) {
    ...
    if (VM_Version::supports_cmov() && !opr2->is_constant()) {
        ...
        if (opr2->is_single_cpu()) {
            ...
            assert(opr2->cpu_regnrLo() != result->cpu_regnrLo() && opr2->cpu_regnrLo() != result->cpu_regnrHi(),
                   "opr2 already overwritten by previous move");
            assert(opr2->cpu_regnrHi() != result->cpu_regnrLo() && opr2->cpu_regnrHi() != result->cpu_regnrHi(),
                   "opr2 already overwritten by previous move");
            ...
            __ cmovptr(ncond, result->as_register_lo(),
                       frame_map()->address_for_slot(opr2->double_stack_ix(), lo_word_offset_in_bytes));
            NOT_LP64(__ cmovptr(ncond, result->as_register_hi(),
                                frame_map()->address_for_slot(opr2->double_stack_ix(), hi_word_offset_in_bytes));)
        } else {
            ShouldNotReachHere();
        }
    ...
    if (left->is_single_cpu()) {
      ...
      switch (code) {
        ...
        if (right->is_double_fpu()) {
            arith_fpu_implementation(code, left->fpu_regnrLo(), right->fpu_regnrLo(), dest->fpu_regnrLo(),
                                     pop_fpu_stack);
...
void LIR_Assembler::arith_fpu_implementation(LIR_Code code, int left_index, int right_index, int dest_index,
                                             bool pop_fpu_stack) {
...
void LIR_Assembler::intrinsic_op(LIR_Code code, LIR_Opr value, LIR_Opr tmp, LIR_Opr dest, LIR_Op *op) {
    if (value->is_double_xmm()) {
        switch (code) {
            ...
            case lir_sqrt:
                __ sqrtsd(dest->as_xmm_double_reg(), value->as_xmm_double_reg());
                break;
...
void LIR_Assembler::logic_op(LIR_Code code, LIR_Opr left, LIR_Opr right, LIR_Opr dst) {
    ...
    if (left->is_single_cpu()) {
        ...
        if (right->is_constant()) {
            ...
            switch (code) {
                case lir_logic_and:
                    __ andptr(reg, rright);
                    break;
                case lir_logic_or :
                    __ orptr(reg, rright);
                    break;
                case lir_logic_xor:
                    __ xorptr(reg, rright);
                    break;
                default:
                    ShouldNotReachHere();
            }
...
void LIR_Assembler::comp_op(LIR_Condition condition, LIR_Opr opr1, LIR_Opr opr2, LIR_Op2 *op) {
    if (opr1->is_single_cpu()) {
        ...
        assert(opr1->is_fpu_register() && opr1->fpu() == 0,
               "currently left-hand side must be on TOS (relax this restriction)");
...
void LIR_Assembler::comp_fl2i(LIR_Code code, LIR_Opr left, LIR_Opr right, LIR_Opr dst, LIR_Op2 *op) {
    if (code == lir_cmp_fd2i || code == lir_ucmp_fd2i) {
        if (left->is_single_xmm()) {
            ...
            __ cmpsd2int(left->as_xmm_double_reg(), right->as_xmm_double_reg(), dst->as_register(),
                         code == lir_ucmp_fd2i);
...
void LIR_Assembler::store_parameter(Register
r,
int offset_from_rsp_in_words
) {
assert(offset_from_rsp_in_words
>= 0, "invalid offset from rsp");
int offset_from_rsp_in_bytes = offset_from_rsp_in_words * BytesPerWord;
assert(offset_from_rsp_in_bytes<frame_map()->reserved_argument_area_size(),
"invalid offset");
__
movptr (Address(rsp, offset_from_rsp_in_bytes), r
);
}
