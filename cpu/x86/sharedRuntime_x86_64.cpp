...
static void unpack_array_argument(MacroAssembler *masm, VMRegPair reg, BasicType in_elem_type, VMRegPair body_arg,
...
class ComputeMoveOrder : public StackObj {
    ...
    public:
        MoveOperation(int src_index, VMRegPair src, int dst_index, VMRegPair dst) :
                _src(src), _dst(dst), _src_index(src_index), _dst_index(dst_index), _processed(false), _next(NULL),
                _prev(NULL) {
        }
...
nmethod *SharedRuntime::generate_native_wrapper(MacroAssembler *masm,
    ...
    if (method->is_synchronized()) {
        ...
        if (UseBiasedLocking) {
            __ biased_locking_enter(lock_reg, obj_reg, swap_reg, rscratch1, rscratch2, false, lock_done,
                                    &slow_path_lock);
        }
...
assert(inv
* n[0] == ULLONG_MAX, "broken inverse in Montgomery multiply");
...
assert(inv
* n[0] == ULLONG_MAX, "broken inverse in Montgomery square");
...
static void reverse_words(julong * s, julong * d, int
len) {
d +=
len;
while(len-- > 0) {
...
void SharedRuntime::montgomery_multiply(jint *a_ints, jint *b_ints, jint *n_ints,
    ...
    julong * scratch = (julong * )
    alloca(total_allocation);
