...
void allocate_object(Register obj, Register t1, Register t2, int header_size, int object_size, Register klass,
                     Label &slow_case);
...
// slow_case  : exit to slow case implementation if fast allocation fails
void allocate_array(Register obj, Register len, Register t, Register t2, int header_size, Address::ScaleFactor f,
...
void push_oop(jobject o) {
    _rsp_offset++;
    pushoop(o);
}

// Seems to always be in wordSize
void push_addr(Address a) {
    _rsp_offset++;
    pushptr(a);
}

void push_reg(Register r) {
    _rsp_offset++;
    push(r);
}

void pop_reg(Register r) {
    _rsp_offset--;
    pop(r);
