...
static const Register rbcp = LP64_ONLY(r13)
NOT_LP64(rsi);
static const Register rlocals = LP64_ONLY(r14)
NOT_LP64(rdi);
...
void TemplateTable::patch_bytecode(Bytecodes::Code bc, Register bc_reg,
    ...
    if (JvmtiExport::can_post_breakpoint()) {
        ...
        __ call_VM(noreg, CAST_FROM_FN_PTR(address, InterpreterRuntime::set_original_bytecode_at), temp_reg, rbcp,
                   bc_reg);
#ifndef ASSERT
...
void TemplateTable::fconst(int value) {
    ...
    if (UseSSE >= 1) {
        ...
        if (value == 0) {
            ...
            __ fld1();
            __ fld1();
            __ faddp(); // should do a better solution here
        } else {
            ShouldNotReachHere();
        }
#endif // _LP64
...
void TemplateTable::condy_helper(Label &Done) {
    ...
    const Register rarg = NOT_LP64(rcx)
    LP64_ONLY(c_rarg1);
    __ movl(rarg, (int) bytecode());
...
void TemplateTable::iload_internal(RewriteControl rc) {
    ...
    if (RewriteFrequentPairs && rc == may_rewrite) {
        Label rewrite, done;
        const Register bc = LP64_ONLY(c_rarg3)
        NOT_LP64(rcx);
...
void TemplateTable::fneg() {
    transition(ftos, ftos);
    if (UseSSE >= 1) {
        static jlong *float_signflip = double_quadword(&float_signflip_pool[1], CONST64(0x8000000080000000),
                                                       CONST64(0x8000000080000000));
...
void TemplateTable::branch(bool is_jsr, bool is_wide) {
    ...
    if (UseLoopCounter) {
        if (UseOnStackReplacement) {
            ...
            const Register retaddr = LP64_ONLY(j_rarg2)
            NOT_LP64(rdi);
            const Register sender_sp = LP64_ONLY(j_rarg1)
            NOT_LP64(rdx);
...
void TemplateTable::getfield_or_static(int byte_no, bool is_static, RewriteControl rc) {
    ...
    const Register cache = rcx;
    const Register index = rdx;
    const Register obj = LP64_ONLY(c_rarg3)
    NOT_LP64(rcx);
    const Register off = rbx;
    const Register flags = rax;
    const Register bc = LP64_ONLY(c_rarg3)
    NOT_LP64(rcx); // uses same reg as obj, so don't mix them
...
void TemplateTable::jvmti_post_field_mod(Register cache, Register index, bool is_static) {
    ...
    const Register robj = LP64_ONLY(c_rarg2)
    NOT_LP64(rax);
    const Register RBX = LP64_ONLY(c_rarg1)
    NOT_LP64(rbx);
    const Register RCX = LP64_ONLY(c_rarg3)
    NOT_LP64(rcx);
    const Register RDX = LP64_ONLY(rscratch1)
    NOT_LP64(rdx);
...
void TemplateTable::jvmti_post_fast_field_mod() {
    ...
    const Register scratch = LP64_ONLY(c_rarg3)
    NOT_LP64(rcx);
    ...
    if (JvmtiExport::can_post_field_modification()) {
        ...
        // c_rarg3: jvalue object on the stack
        LP64_ONLY(
                __ call_VM(noreg, CAST_FROM_FN_PTR(address, InterpreterRuntime::post_field_modification), rbx, c_rarg2,
                           c_rarg3));
...
void TemplateTable::_new() {
...
#endif
        Register
        tmp_store_klass = LP64_ONLY(rscratch1)
        NOT_LP64(noreg);
    ...
    Register
    rarg1 = LP64_ONLY(c_rarg1)
    NOT_LP64(rax);
    Register
    rarg2 = LP64_ONLY(c_rarg2)
    NOT_LP64(rdx);
...
void TemplateTable::monitorenter() {
    ...
    Register
    rtop = LP64_ONLY(c_rarg3)
    NOT_LP64(rcx);
    Register
    rbot = LP64_ONLY(c_rarg2)
    NOT_LP64(rbx);
    Register
    rmon = LP64_ONLY(c_rarg1)
    NOT_LP64(rdx);
