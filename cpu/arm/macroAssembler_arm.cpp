...
void MacroAssembler::call_VM(Register oop_result, Register last_java_sp, address entry_point, int number_of_arguments,
                             bool check_exceptions) {
    // Not used on ARM
    Unimplemented();
}
...
void
MacroAssembler::call_VM(Register oop_result, Register last_java_sp, address entry_point, Register arg_1, Register arg_2,
                        bool check_exceptions) {
// Not used on ARM
    Unimplemented();
}
...
void MacroAssembler::stop(const char *msg) {
    ...
    InlinedAddress
    Ldebug(CAST_FROM_FN_PTR(address, MacroAssembler::debug));
    ...
    if (ShowMessageBoxOnError) {
        ...
        if (os::message_box(msg, "Execution stopped, print registers?")) {
            ...
            for (int i = 0; i < nregs; i++) {
                tty->print_cr("%s = "
                INTPTR_FORMAT, regs[i]->name(), registers[i]);
            }

            // derive original SP value from the address of register save area
            tty->print_cr("%s = "
            INTPTR_FORMAT, SP->name(), p2i(&registers[nregs]));
        }
        BREAKPOINT;
    } else {
...
void MacroAssembler::biased_locking_enter(Register obj_reg, Register swap_reg, Register tmp_reg,
    ...
    assert((markWord::biased_lock_mask_in_place | markWord::age_mask_in_place | markWord::epoch_mask_in_place) == 0x1ff,
           "biased bitmasks changed");
...
void MacroAssembler::load_sized_value(Register dst, Address src,
    ...
    switch (size_in_bytes) {
        case 4:
            ...
            break;
        case 2:
            is_signed ? ldrsh(dst, src, cond) : ldrh(dst, src, cond);
            break;
        case 1:
            is_signed ? ldrsb(dst, src, cond) : ldrb(dst, src, cond);
            break;
        default:
            ShouldNotReachHere();
    }
...
void MacroAssembler::load_heap_oop(Register dst, Address src, Register tmp1, Register tmp2, Register tmp3,
                                   DecoratorSet decorators) {
    access_load_at(T_OBJECT, IN_HEAP | decorators, src, dst, tmp1, tmp2, tmp3);
}

// Blows src and flags.
void MacroAssembler::store_heap_oop(Address obj, Register new_val, Register tmp1, Register tmp2, Register tmp3,
                                    DecoratorSet decorators) {
    access_store_at(T_OBJECT, IN_HEAP | decorators, obj, new_val, tmp1, tmp2, tmp3, false);
}

void MacroAssembler::store_heap_oop_null(Address obj, Register new_val, Register tmp1, Register tmp2, Register tmp3,
                                         DecoratorSet decorators) {
