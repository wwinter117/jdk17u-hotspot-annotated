...
#undef F
    ...
    void vpaddl(FloatRegister Dd, FloatRegister Dm, int size, bool s) {
        ...
        emit_int32(0xf3b00200 | Dd->hi_bit() << 22 | (size >> 4) << 18 | Dd->hi_bits() << 12 | (s ? 0 : 1) << 7 |
                   Dm->hi_bit() << 5 | Dm->hi_bits());
    }
    ...
    void vld1(FloatRegister Dd, Address addr, VElem_Size size, int bits) {
        ...
        emit_int32(0xf4200000 | Dd->hi_bit() << 22 | Dd->hi_bits() << 12 | type << 8 | size << 6 | align << 4 |
                   addr.encoding_simd());
    }
    ...
    void vst1(FloatRegister Dd, Address addr, VElem_Size size, int bits) {
        ...
        assert(bits == 128, "code assumption");
        VLD_Type type = VLD1_TYPE_2_REGS; // 2x64
        emit_int32(0xf4000000 | Dd->hi_bit() << 22 | Dd->hi_bits() << 12 | type << 8 | size << 6 | align << 4 |
                   addr.encoding_simd());
    }
    ...
    void vrev(FloatRegister Dd, FloatRegister Dm, int quad, int region_size, VElem_Size size) {
        ...
        switch (region_size) {
            ...
            case 64:
                op = 0x0; /*0b00*/ break;
            default:
                assert(false, "encoding constraint");
        }
