...
void LIR_Assembler::reg2stack(LIR_Opr src, LIR_Opr dest, BasicType type, bool pop_fpu_stack) {
    ...
    if (src->is_single_cpu()) {
        switch (type) {
            case T_OBJECT:
            case T_ARRAY:
                __ verify_oop(src->as_register());   // fall through
            case T_ADDRESS:
            case T_METADATA:
            ...
            case T_INT:
                __ str_32(src->as_register(), addr);
                break;
            default:
                ShouldNotReachHere();
        }
...
void LIR_Assembler::stack2reg(LIR_Opr src, LIR_Opr dest, BasicType type) {
    ...
    if (dest->is_single_cpu()) {
        switch (type) {
            ...
            case T_INT:
                __ ldr_u32(dest->as_register(), addr);
                break;
            default:
                ShouldNotReachHere();
        }
...
void LIR_Assembler::emit_opTypeCheck(LIR_OpTypeCheck *op) {
    ...
    switch (op->code()) {
        case lir_store_check: {
            ...
            if (op->should_profile()) {
                __ cmp(value, 0);
                typecheck_profile_helper1(op->profiled_method(), op->profiled_bci(), md, data, mdo_offset_bias, value,
                                          k_RInfo, Rtemp, &done);
            } else {
                __ cbz(value, done);
            }
        ...
        case lir_checkcast: {
            ...
            __ movs(res, obj);
            if (op->should_profile()) {
                typecheck_profile_helper1(op->profiled_method(), op->profiled_bci(), md, data, mdo_offset_bias, res,
                                          klass_RInfo, Rtemp, &done);
            } else {
                __ b(done, eq);
            }
            if (k->is_loaded()) {
...
void LIR_Assembler::emit_compare_and_swap(LIR_OpCompareAndSwap *op) {
    ...
    assert(op->addr()->is_register() || op->addr()->as_address_ptr()->index() == LIR_OprDesc::illegalOpr(),
           "unexpected index");
...
void LIR_Assembler::arith_op(LIR_Code code, LIR_Opr left, LIR_Opr right, LIR_Opr dest, CodeEmitInfo *info,
    ...
    if (right->is_address()) {
        ...
        assert(addr->base()->as_pointer_register() == lreg && addr->index()->is_register() && addr->disp() == 0,
               "must be");
        ...
        switch (code) {
            case lir_add:
                __ add(res, lreg, operand);
                break;
            case lir_sub:
                __ sub(res, lreg, operand);
                break;
            default:
                ShouldNotReachHere();
        }
        ...
        if (right->is_single_cpu()) {
            ...
            switch (code) {
                case lir_add:
                    __ add_32(res, lreg, rreg);
                    break;
                case lir_sub:
                    __ sub_32(res, lreg, rreg);
                    break;
                case lir_mul:
                    __ mul_32(res, lreg, rreg);
                    break;
                default:
                    ShouldNotReachHere();
            }
            ...
            switch (code) {
                case lir_add:
                    __ add_32(res, lreg, c);
                    break;
                case lir_sub:
                    __ sub_32(res, lreg, c);
                    break;
                default:
                    ShouldNotReachHere();
            }
        ...
        switch (code) {
            case lir_add:
                __ add_float(res, lreg, rreg);
                break;
            case lir_sub:
                __ sub_float(res, lreg, rreg);
                break;
            case lir_mul:
                __ mul_float(res, lreg, rreg);
                break;
            case lir_div:
                __ div_float(res, lreg, rreg);
                break;
            default:
                ShouldNotReachHere();
        }
        ...
        switch (code) {
            case lir_add:
                __ add_double(res, lreg, rreg);
                break;
            case lir_sub:
                __ sub_double(res, lreg, rreg);
                break;
            case lir_mul:
                __ mul_double(res, lreg, rreg);
                break;
            case lir_div:
                __ div_double(res, lreg, rreg);
                break;
            default:
                ShouldNotReachHere();
        }
...
void LIR_Assembler::logic_op(LIR_Code code, LIR_Opr left, LIR_Opr right, LIR_Opr dest) {
    ...
    if (dest->is_single_cpu()) {
        ...
        if (right->is_single_cpu()) {
            ...
            switch (code) {
                case lir_logic_and:
                    __ and_32(res, lreg, rreg);
                    break;
                case lir_logic_or:
                    __ orr_32(res, lreg, rreg);
                    break;
                case lir_logic_xor:
                    __ eor_32(res, lreg, rreg);
                    break;
                default:
                    ShouldNotReachHere();
            }
            ...
            switch (code) {
                case lir_logic_and:
                    __ and_32(res, lreg, c);
                    break;
                case lir_logic_or:
                    __ orr_32(res, lreg, c);
                    break;
                case lir_logic_xor:
                    __ eor_32(res, lreg, c);
                    break;
                default:
                    ShouldNotReachHere();
            }
    ...
    if (opr1->is_single_cpu()) {
        if (opr2->is_constant()) {
            ...
            if (opr1->type() == T_OBJECT || opr1->type() == T_ARRAY) {
                ...
                assert(opr2->type() != T_OBJECT && opr2->type() != T_ARRAY && opr2->type() != T_METADATA &&
                       opr2->type() != T_ADDRESS, "incompatibe type");
...
void LIR_Assembler::emit_arraycopy(LIR_OpArrayCopy *op) {
    ...
    if (flags & LIR_OpArrayCopy::type_check) {
        ...
        if (basic_type != T_OBJECT) {
            ...
            __ check_klass_subtype_fast_path(tmp, tmp2, R6, altFP_7_11, &cont,
                                             copyfunc_addr == NULL ? stub->entry() : &slow, NULL);

            __ mov(R6, R0);
            __ mov(altFP_7_11, R1);
            __ mov(R0, tmp);
            __ mov(R1, tmp2);
            __ call(Runtime1::entry_for(Runtime1::slow_subtype_check_id),
                    relocInfo::runtime_call_type); // does not blow any registers except R0, LR and Rtemp
            __ cmp_32(R0, 0);
            __ mov(R0, R6);
