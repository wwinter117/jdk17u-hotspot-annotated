...
LIR_Opr LIRGenerator::result_register_for(ValueType *type, bool callee) {
    ...
    switch (type->tag()) {
        case intTag:
            opr = FrameMap::Int_result_opr;
            break;
        case objectTag:
            opr = FrameMap::Object_result_opr;
            break;
        case longTag:
            opr = FrameMap::Long_result_opr;
            break;
        case floatTag:
            opr = FrameMap::Float_result_opr;
            break;
        case doubleTag:
            opr = FrameMap::Double_result_opr;
            break;
        case addressTag:
        default:
            ShouldNotReachHere();
...
static LIR_Opr make_constant(BasicType type, jlong c) {
    switch (type) {
        case T_ADDRESS:
        case T_OBJECT:
            return LIR_OprFact::intptrConst(c);
        case T_LONG:
            return LIR_OprFact::longConst(c);
        case T_INT:
            return LIR_OprFact::intConst(c);
        default:
            ShouldNotReachHere();
...
LIR_Address *LIRGenerator::generate_address(LIR_Opr base, LIR_Opr index,
    ...
    assert(index->is_illegal() || (index->type() == T_INT && !index->is_constant()),
...
void LIRGenerator::do_NewObjectArray(NewObjectArray *x) {
    ...
    length.load_item_force(
            FrameMap::R2_opr);           // R2 is required by runtime call in NewObjectArrayStub::emit_code
    LIR_Opr len = length.result();
    ...
    LIR_Opr reg = result_register_for(
            x->type());       // R0 is required by runtime call in NewObjectArrayStub::emit_code
