...
void InterpreterMacroAssembler::call_VM_helper(Register oop_result, address entry_point, int number_of_arguments,
...
void InterpreterMacroAssembler::load_earlyret_value(TosState state) {
    ...
    switch (state) {
        ...
        case ltos:
            ldr(R1_tos_hi, val_addr_hi);        // fall through
        case btos:                                     // fall through
        case ztos:                                     // fall through
        case ctos:                                     // fall through
        case stos:                                     // fall through
        case itos:
            ldr_s32(R0_tos, val_addr);
            break;
#ifdef __SOFTFP__
...
#else
        case ftos:
            ldr_float(S0_tos, val_addr);
            break;
        case dtos:
            ldr_double(D0_tos, val_addr);
            break;
#endif // __SOFTFP__
...
void
InterpreterMacroAssembler::get_cache_and_index_and_bytecode_at_bcp(Register cache, Register index, Register bytecode,
                                                                   int byte_no, int bcp_offset, size_t index_size) {
    get_cache_and_index_at_bcp(cache, index, bcp_offset, index_size);
    // caution index and bytecode can be the same
    add(bytecode, cache, AsmOperand(index, lsl, LogBytesPerWord));
    ldrb(bytecode, Address(bytecode, (1 + byte_no) + in_bytes(
...
void InterpreterMacroAssembler::get_cache_entry_pointer_at_bcp(Register cache, Register reg_tmp, int bcp_offset,
                                                               size_t index_size) {
    assert(bcp_offset > 0, "bcp is still pointing to start of bytecode");
...
void
InterpreterMacroAssembler::profile_switch_case(Register mdp, Register index, Register reg_tmp1, Register reg_tmp2) {
...
void InterpreterMacroAssembler::inc_global_counter(address address_of_counter, int offset, Register tmp1, Register tmp2,
                                                   bool avoid_overflow) {
