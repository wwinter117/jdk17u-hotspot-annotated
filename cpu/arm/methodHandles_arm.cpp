...
void MethodHandles::jump_to_lambda_form(MacroAssembler *_masm,
    ...
    __ access_load_at(T_ADDRESS, IN_HEAP,
                      Address(Rmethod, NONZERO(java_lang_invoke_ResolvedMethodName::vmtarget_offset())), Rmethod, noreg,
                      noreg, noreg);
...
void trace_method_handle_stub(const char *adaptername,
    ...
    PTR_FORMAT,
            ...
            (intptr_t) entry_sp, (intptr_t) saved_sp - (intptr_t) entry_sp, (intptr_t)(
            base_sp - last_sp), (intptr_t) saved_bp);

    if (last_sp != saved_sp && last_sp != NULL) {
        log_info(methodhandles)("*** last_sp="
        INTPTR_FORMAT, p2i(last_sp));
    }
    LogTarget(Trace, methodhandles)
    lt;
    if (lt.is_enabled()) {
        ResourceMark rm;
        LogStream ls(lt);
        ...
        for (i = 0; i < trace_mh_nregs; i++) {
            ...
            ls.print(" %s: "
            INTPTR_FORMAT, reg_name, p2i((void *) saved_regs[i]));
        }
        ls.cr();
