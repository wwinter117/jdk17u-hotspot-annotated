...
void TemplateInterpreterGenerator::generate_stack_overflow_check(void) {
    ...
    const int overhead_size =
            (frame::sender_sp_offset - frame::interpreter_frame_initial_sp_offset) * wordSize + entry_size;
...
void TemplateInterpreterGenerator::lock_method() {
    ...
    // get synchronization object
    {
        ...
        __ ldr(R0, Address(Rlocals, Interpreter::local_offset_in_bytes(0)),
               eq); // get receiver (assume this is frequent case)
        __ b(done, eq);
...
address
TemplateInterpreterGenerator::generate_CRC32_updateBytes_entry(AbstractInterpreter::MethodKind kind) { return NULL; }

address
TemplateInterpreterGenerator::generate_CRC32C_updateBytes_entry(AbstractInterpreter::MethodKind kind) { return NULL; }
...
address TemplateInterpreterGenerator::generate_native_entry(bool synchronized) {
    ...
    if (synchronized) {
        ...
        __ sub(R0, FP,
               -(frame::interpreter_frame_monitor_block_bottom_offset - frame::interpreter_frame_monitor_size()) *
               wordSize);
        __ unlock_object(R0);
    }
    ...
    __ notify_method_exit(vtos, InterpreterMacroAssembler::NotifyJVMTI, true, Rsaved_result_lo, Rsaved_result_hi,
                          saved_result_fp);
...
void
TemplateInterpreterGenerator::set_vtos_entry_points(Template *t, address &bep, address &cep, address &sep, address &aep,
...
void TemplateInterpreterGenerator::histogram_bytecode(Template *t) {
    __ inc_global_counter((address) & BytecodeHistogram::_counters[0],
                          sizeof(BytecodeHistogram::_counters[0]) * t->bytecode(), Rtemp, R2_tmp, true);
}
