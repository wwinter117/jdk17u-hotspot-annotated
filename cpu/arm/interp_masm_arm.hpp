...
protected:
    ...
    typedef enum {
        DispatchDefault, DispatchNormal
    } DispatchTableMode;
...
public:
    ...
    void get_constant_pool(Register reg) {
        get_const(reg);
        ldr(reg, Address(reg, ConstMethod::constants_offset()));
    }
    ...
    void get_cache_and_index_and_bytecode_at_bcp(Register cache, Register index, Register bytecode, int byte_no,
                                                 int bcp_offset, size_t index_size = sizeof(u2));

    // Sets cache. Blows reg_tmp.
    void
    get_cache_entry_pointer_at_bcp(Register cache, Register reg_tmp, int bcp_offset, size_t index_size = sizeof(u2));
    ...
    void dispatch_next(TosState state, int step = 0,
                       bool generate_poll = false); // load R3_bytecode from [Rbcp + step] and dispatch by R3_bytecode
    ...
    // Debugging
    void interp_verify_oop(Register reg, TosState state, const char *file,
                           int line);    // only if +VerifyOops && state == atos
    ...
    void inc_global_counter(address address_of_counter, int offset_in_bytes, Register tmp1, Register tmp2,
                            bool avoid_overflow);
    ...
    void notify_method_exit(TosState state, NotifyMethodExitMode mode,
                            bool native = false, Register result_lo = noreg, Register result_hi = noreg,
                            FloatRegister result_fp = fnoreg);

    void trace_state(const char *msg)
