...
ZPhysicalMemoryBacking::ZPhysicalMemoryBacking(size_t max_capacity) :
    ...
    log_info_p(gc, init)("Heap Backing Filesystem: %s (0x"
    UINT64_FORMAT_X
    ")",
    ...
    if (ZLargePages::is_explicit() && os::large_page_size() != ZGranuleSize) {
        log_error_p(gc)("Incompatible large page size configured "
        SIZE_FORMAT
        " (expected "
        SIZE_FORMAT
        ")",
    ...
    if (ZGranuleSize % _block_size != 0) {
        log_error_p(gc)("Filesystem backing the heap has incompatible block size ("
        SIZE_FORMAT
        ")",
                _block_size);
        return;
    ...
    if (is_hugetlbfs() && _block_size != ZGranuleSize) {
        log_error_p(gc)("%s filesystem has unexpected block size "
        SIZE_FORMAT
        " (expected "
        SIZE_FORMAT
        ")",
...
int ZPhysicalMemoryBacking::create_mem_fd(const char *name) const {
    ...
    if (fd == -1) {
        ...
        log_debug_p(gc, init)("Failed to create memfd file (%s)",
                              ((ZLargePages::is_explicit() && err == EINVAL) ? "Hugepages not supported"
                                                                             : err.to_string()));
        return -1;
    }
...
int ZPhysicalMemoryBacking::create_file_fd(const char *name) const {
    ...
    if (fd_anon == -1) {
        ...
        log_info_p(gc, init)("Heap Backing File: %s/#"
        UINT64_FORMAT, mountpoint.get(), (uint64_t) stat_buf.st_ino);
...
void ZPhysicalMemoryBacking::warn_available_space(size_t max_capacity) const {
    ...
    log_info_p(gc, init)("Available space on backing filesystem: "
    SIZE_FORMAT
    "M", _available / M);
    ...
    if (_available < max_capacity) {
        ...
        log_warning_p(gc)("size ("
        SIZE_FORMAT
        "M). Please adjust the size of the backing filesystem accordingly "
        "(available", max_capacity / M);
        log_warning_p(gc)("space is currently "
        SIZE_FORMAT
        "M). Continuing execution with the current filesystem "
...
void ZPhysicalMemoryBacking::warn_max_map_count(size_t max_capacity) const {
    ...
    if (actual_max_map_count < required_max_map_count) {
        ...
        log_warning_p(gc)("max Java heap size ("
        SIZE_FORMAT
        "M). Please adjust %s to allow for at",
                max_capacity / M, filename);
        log_warning_p(gc)("least "
        SIZE_FORMAT
        " mappings (current limit is "
        SIZE_FORMAT
...
bool ZPhysicalMemoryBacking::commit_inner(size_t offset, size_t length) const {
    log_trace(gc, heap)("Committing memory: "
    SIZE_FORMAT
    "M-"
    SIZE_FORMAT
    "M ("
    ...
    if (err) {
        if (err == ENOSPC && !is_init_completed() && ZLargePages::is_explicit() &&
            z_fallocate_hugetlbfs_attempts-- > 0) {
...
size_t ZPhysicalMemoryBacking::uncommit(size_t offset, size_t length) const {
    log_trace(gc, heap)("Uncommitting memory: "
    SIZE_FORMAT
    "M-"
    SIZE_FORMAT
    "M ("
...
void ZPhysicalMemoryBacking::unmap(uintptr_t addr, size_t size) const {
    ...
    const void *const res = mmap((void *) addr, size, PROT_NONE,
