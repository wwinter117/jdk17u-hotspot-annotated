...
julong os::Linux::available_memory() {
    ...
    if (OSContainer::is_containerized()) {
        ...
        if (mem_limit > 0 && mem_usage > 0) {
            ...
            log_trace(os)("available container memory: "
...
julong os::physical_memory() {
    ...
    if (OSContainer::is_containerized()) {
        ...
        if ((mem_limit = OSContainer::memory_limit_in_bytes()) > 0) {
            log_trace(os)("total container memory: "
            JLONG_FORMAT, mem_limit);
            return mem_limit;
        }
    }

    phys_mem = Linux::physical_memory();
    log_trace(os)("total system memory: "
    JLONG_FORMAT, phys_mem);
    return phys_mem;
}
...
!=
NULL &&p
<= (volatile char *)bottom, "alloca problem?");
p[0] = '\0';
}
}

void os::Linux::expand_stack_to(address
bottom) {
...
bool os::create_thread(Thread *thread, ThreadType thr_type,
    ...
    if (status != 0) {
        ...
        log_warning(os, thread)("The %sthread stack size specified is invalid: "
        SIZE_FORMAT
        "k",
...

    {
        ...
        char buf[64];
        if (ret == 0) {
            log_info(os, thread)("Thread \"%s\" started (pthread id: "
            UINTX_FORMAT
            ", attributes: %s). ",
            ...
            if (TimerSlack >= 0) {
                ...
                if (slack >= 0) {
                    log_info(os, thread)("Thread \"%s\" (pthread id: "
                    UINTX_FORMAT
                    ") timer slack: %dns",
...
static bool find_vma(address
addr,
address *vma_low, address
* vma_high) {
...
void os::Linux::capture_initial_stack(size_t max_size) {
    ...
    dlsym(RTLD_DEFAULT, "__libc_stack_end");
    if (p && *p) {
        stack_start = *p;
    } else {
...
#undef _DFM
    ...
    _initial_thread_stack_bottom = (address)
    stack_top - _initial_thread_stack_size;

    assert(_initial_thread_stack_bottom < (address)
    stack_top, "overflow!");
    ...
    if (log_is_enabled(Info, os, thread)) {
        ...
        log_info(os, thread)("Capturing initial stack in %s thread: req. size: "
        SIZE_FORMAT
        "K, actual size: "
        SIZE_FORMAT
        "K, top="
...
bool os::address_is_in_vm(address
addr) {
...
libjvm_base_addr = (address)
dlinfo.
dli_fbase;
}
assert(libjvm_base_addr
!=NULL, "Cannot obtain base address for libjvm");
}
...
bool os::dll_address_to_function_name(address
addr,
char *buf,
int buflen,
int *offset,
...
.dli_sname != NULL) {
if (!(
demangle &&Decoder::demangle(dlinfo.dli_sname, buf, buflen)
)) {
...
.dli_fbase != NULL) {
if (
Decoder::decode((address)(addr - (address)
dlinfo.dli_fbase),
...
bool os::dll_address_to_library_name(address
addr,
char *buf,
int buflen,
int *offset
...
void os::Linux::print_proc_sys_info(outputStream *st) {
    ...
    _print_ascii_file_h(
            "/proc/sys/vm/swappiness (control to define how aggressively the kernel swaps out anonymous memory)",
...
bool os::Linux::query_process_memory_info(os::Linux::meminfo_t *info) {
    ...
    if (f != NULL) {
        while (::fgets(buf, sizeof(buf), f) != NULL && num_found < num_values) {
            if ((info->vmsize == -1 && sscanf(buf, "VmSize: " SSIZE_FORMAT
            " kB", &info->vmsize) == 1) ||
            (info->vmpeak == -1 && sscanf(buf, "VmPeak: "
            SSIZE_FORMAT
            " kB", &info->vmpeak) == 1) ||
            (info->vmswap == -1 && sscanf(buf, "VmSwap: "
            SSIZE_FORMAT
            " kB", &info->vmswap) == 1) ||
            (info->vmhwm == -1 && sscanf(buf, "VmHWM: "
            SSIZE_FORMAT
            " kB", &info->vmhwm) == 1) ||
            (info->vmrss == -1 && sscanf(buf, "VmRSS: "
            SSIZE_FORMAT
            " kB", &info->vmrss) == 1) ||
            (info->rssanon == -1 && sscanf(buf, "RssAnon: "
            SSIZE_FORMAT
            " kB", &info->rssanon) == 1) || // Needs Linux 4.5
            (info->rssfile == -1 && sscanf(buf, "RssFile: "
            SSIZE_FORMAT
            " kB", &info->rssfile) == 1) || // Needs Linux 4.5
            (info->rssshmem == -1 && sscanf(buf, "RssShmem: "
            SSIZE_FORMAT
            " kB", &info->rssshmem) == 1)  // Needs Linux 4.5
            )
            {
                num_found++;
            }
...
void os::Linux::print_process_memory_info(outputStream *st) {
    ...
    if (query_process_memory_info(&info)) {
        st->print_cr("Virtual Size: "
        SSIZE_FORMAT
        "K (peak: "
        SSIZE_FORMAT
        "K)", info.vmsize, info.vmpeak);
        st->print("Resident Set Size: "
        SSIZE_FORMAT
        "K (peak: "
        SSIZE_FORMAT
        "K)", info.vmrss, info.vmhwm);
...
bool os::Linux::print_container_info(outputStream *st) {
    ...
    OSContainer::print_container_helper(st, OSContainer::memory_and_swap_limit_in_bytes(),
...
void os::Linux::print_steal_info(outputStream *st) {
    if (has_initial_tick_info) {
        ...
        if (res && pticks.has_steal_ticks) {
            ...
            st->print_cr("Steal ticks since vm start: "
            UINT64_FORMAT, steal_ticks_difference);
...
static void warn_fail_commit_memory(char *addr, size_t size,
    ...
    ", "
    SIZE_FORMAT
    ", %d) failed; error='%s' (errno=%d)", p2i(addr), size,
...
bool os::pd_uncommit_memory(char *addr, size_t size, bool exec) {
    ...
    return res != (uintptr_t)
    MAP_FAILED;
}

static address get_stack_commited_bottom(address
bottom,
size_t size
) {
address nbot = bottom;
...
if (errno != EAGAIN) {
assert(errno
== ENOMEM, "Unexpected mincore errno");
imax = imid;
}
} else {
...
bool os::committed_in_range(address
start,
size_t size, address
& committed_start,
...
assert(is_aligned(start, page_sz),
...
// Get stable read
while ((
mincore_return_value = mincore(loop_base, pages_to_query * page_sz, vec)
) == -1 && errno == EAGAIN);
...
== 0, "Range must be valid");
// Process this stripe
for (
int vecIdx = 0;
vecIdx<pages_to_query;
vecIdx ++) {
...
loop_base +=
pages_to_query *page_sz;
}

if (committed_start != NULL) {
assert(committed_pages
> 0, "Must have committed region");
assert(committed_pages
<=
int(size
/ page_sz), "Can not commit more than it has");
assert(committed_start
>=
start &&committed_start<start + size,
"Out of range");
committed_size = page_sz * committed_pages;
return true;
} else {
assert(committed_pages
== 0, "Should not have committed region");
return false;
}
}
...
bool os::pd_create_stack_guard_pages(char *addr, size_t size) {
    if (os::is_primordial_thread()) {
        ...
        if (mincore((address)stack_extent, os::vm_page_size(), vec) == -1) {
            ...
            assert((uintptr_t)
            addr >= stack_extent,
                    "Sanity: addr should be larger than extent, "
            PTR_FORMAT
            " >= "
            PTR_FORMAT,
...
#endif
    Events::log_memprotect(NULL, "Protecting memory ["
    INTPTR_FORMAT
    ","
    INTPTR_FORMAT
...
bool os::Linux::hugetlbfs_sanity_check(bool warn, size_t page_size) {
    ...
    if (p != MAP_FAILED) {
        ...
        log_info(pagesize)("Large page size ("
        SIZE_FORMAT
        "%s) failed sanity check, "
    ...
    ~LargePageInitializationLoggerMark() {
        ...
        if (lt.is_enabled()) {
            ...
            if (UseLargePages) {
                ...
                ls.print_cr(". Default large page size: "
                EXACTFMT
...
void os::large_page_init() {
    ...
    if (UseTransparentHugePages) {
        ...
        if (FLAG_IS_DEFAULT(LargePageSizeInBytes) ||
            ...
            log_info(pagesize)("Using the default large page size: "
            SIZE_FORMAT
            "%s",
            ...
            if (all_large_pages.contains(LargePageSizeInBytes)) {
                ...
                _large_page_size = default_large_page_size;
                log_info(pagesize)("LargePageSizeInBytes is not a valid large page size ("
                SIZE_FORMAT
                "%s) "
                "using the default large page size: "
                SIZE_FORMAT
                "%s",
...
bool os::Linux::commit_memory_special(size_t bytes,
    ...
    log_debug(pagesize)("Commit special mapping: "
    PTR_FORMAT
    ", size="
    SIZE_FORMAT
    "%s, page size="
...
bool os::start_debugging(char *buf, int buflen) {
    ...
    jio_snprintf(p, buflen - len,
                 ...
                 "To debug, run 'gdb /proc/%d/exe %d'; then switch to thread "
    UINTX_FORMAT
    " ("
    INTPTR_FORMAT
    ")\n"
