...
void StaticHugePageSupport::print_on(outputStream *os) {
    if (_initialized) {
        ...
        for (size_t s = _pagesizes.smallest(); s != 0; s = _pagesizes.next_larger(s)) {
            os->print_cr("  hugepage size: "
            EXACTFMT, EXACTFMTARGS(s));
        }
        os->print_cr("  default hugepage size: "
        EXACTFMT, EXACTFMTARGS(_default_hugepage_size));
    } else {
        os->print_cr("  unknown.");
    }
    if (_inconsistent) {
...
void StaticHugePageSupport::scan_os() {
    ...
    if (_default_hugepage_size > 0) {
        ...
        if (!_pagesizes.contains(_default_hugepage_size)) {
            log_info(pagesize)("Unexpected configuration: default pagesize ("
            SIZE_FORMAT
            ") "
...
void THPSupport::print_on(outputStream *os) {
    if (_initialized) {
        ...
        os->print_cr("  THP pagesize: "
        EXACTFMT, EXACTFMTARGS(_pagesize));
    } else {
        os->print_cr("  unknown.");
    }
}
