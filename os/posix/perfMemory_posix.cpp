...
static char *get_sharedmem_filename(const char *dirname, int vmid, int nspid) {
    ...
    int pid = LINUX_ONLY((nspid == -1) ? vmid : nspid)
    NOT_LINUX(vmid);
...
//
static void
mmap_attach_shared(const char *user, int vmid, PerfMemory::PerfMemoryMode mode, char **addr, size_t *sizep, TRAPS) {
    ...
    int nspid = LINUX_ONLY(os::Linux::get_namespace_pid(vmid))
    NOT_LINUX(-1);
