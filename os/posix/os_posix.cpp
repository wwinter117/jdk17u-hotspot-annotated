    ...
    if (n <= 0) {
        ...
        switch (rlim.rlim_cur) {
            ...
            case 0:
                ...
                break;
            default:
                jio_snprintf(buffer, bufferSize, "%s (max size "
                UINT64_FORMAT
                " kB). To ensure a full core dump, try \"ulimit -c unlimited\" before starting Java again", core_path,
                        uint64_t(rlim.rlim_cur) / 1024);
                success = true;
                break;
        }
    }
...
int os::get_native_stack(address *stack, int frames, int toSkip) {
    ...
    while (fr.pc() && frame_idx < frames) {
        ...
        if (fr.fp() == NULL || fr.cb() != NULL ||
            fr.sender_pc() == NULL || os::is_first_C_frame(&fr))
            break;
...
static char *reserve_mmapped_memory(size_t bytes, char *requested_addr) {
    ...
    NOT_AIX( | MAP_NORESERVE ) | MAP_ANONYMOUS;
    if (requested_addr != NULL) {
        assert((uintptr_t) requested_addr % os::vm_page_size() == 0,
               "Requested address should be aligned to OS page size");
        flags |= MAP_FIXED;
    }
...
char *os::map_memory_to_file(char *base, size_t size, int fd) {
    ...
    if (ret != 0) {
        vm_exit_during_initialization(
                err_msg("Error in mapping Java heap at the given filesystem directory. error(%d)", ret));
        return NULL;
    }
...
static void print_rlimit(outputStream *st, const char *msg,
    ...
    if (res == -1) {
        ...
        if (rlim.rlim_cur == RLIM_INFINITY) { st->print("infinity"); }
            ...
            if (output_k) {
                st->print(UINT64_FORMAT
                "k", uint64_t(rlim.rlim_cur) / 1024);
...
void os::dll_unload(void *lib) {
    ...
    if (res == 0) {
        Events::log_dll_message(NULL, "Unloaded shared library \"%s\" ["
        INTPTR_FORMAT
        "]",
                l_path, p2i(lib));
        log_info(os)("Unloaded shared library \"%s\" ["
        INTPTR_FORMAT
        "]", l_path, p2i(lib));
    } else {
        ...
        Events::log_dll_message(NULL, "Attempt to unload shared library \"%s\" ["
        INTPTR_FORMAT
        "] failed, %s",
                l_path, p2i(lib), error_report);
        log_info(os)("Attempt to unload shared library \"%s\" ["
        INTPTR_FORMAT
        "] failed, %s",
                l_path, p2i(lib), error_report);
    }
...
jlong os::lseek(int fd, jlong offset, int whence) {
    return (jlong) AIX_ONLY(::lseek64)
    NOT_AIX(::lseek)(fd, offset, whence);
}
...
int os::ftruncate(int fd, jlong length) {
    return AIX_ONLY(::ftruncate64)
    NOT_AIX(::ftruncate)(fd, length);
}
...
char *os::Posix::describe_pthread_attr(char *buf, size_t buflen, const pthread_attr_t *attr) {
    ...
    LINUX_ONLY(
    if (os::Linux::adjustStackSizeForGuardPages()) stack_size -= guard_size;)
    pthread_attr_getdetachstate(attr, &detachstate);
    jio_snprintf(buf, buflen, "stacksize: "
    SIZE_FORMAT
    "k, guardsize: "
    SIZE_FORMAT
    "k, %s",
...
void os::abort(bool dump_core, void *siginfo, const void *context) {
    ...
    if (dump_core) {
        LINUX_ONLY(
        if (DumpPrivateMappingsInCore) ClassLoader::close_jrt_image();)
        ::abort(); // dump core
    }
    ::_exit(1);
}
