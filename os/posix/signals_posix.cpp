...
public:
    ...
    void set(int sig, const struct sigaction *act) {
        if (check_signal_number(sig)) {
            ...
            _sa[sig] = NEW_C_HEAP_OBJ(
            struct sigaction, mtInternal);
            *_sa[sig] = *act;
        }
    }
...
int JVM_HANDLE_XXX_SIGNAL(int sig, siginfo_t *info,
    ...
    if (!signal_was_handled && abort_if_unrecognized) {
        ...
        VMError::report_and_die(t, sig, pc, info, NOT_ZERO(ucVoid)
        ZERO_ONLY(NULL));
...
bool os::signal_thread(Thread *thread, int sig, const char *reason) {
    ...
    if (osthread) {
        ...
        if (status == 0) {
            Events::log(Thread::current(), "sent signal %d to Thread "
            INTPTR_FORMAT
            " because %s.",
