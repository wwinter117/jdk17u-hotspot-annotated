...
bool os::create_thread(Thread *thread, ThreadType thr_type,
    ...
    if (ret == 0) {
        ...
        log_warning(os, thread)("Failed to start thread \"%s\" - pthread_create failed (%d=%s) for attributes: %s.",
                                thread->name(), ret, os::errno_name(ret),
...
void os::print_memory_info(outputStream *st) {
    ...
    if (os::Aix::get_meminfo(&mi)) {
        if (os::Aix::on_aix()) {
            st->print_cr("physical total : "
            SIZE_FORMAT, mi.real_total);
            st->print_cr("physical free  : "
            SIZE_FORMAT, mi.real_free);
            st->print_cr("swap total     : "
            SIZE_FORMAT, mi.pgsp_total);
            st->print_cr("swap free      : "
            SIZE_FORMAT, mi.pgsp_free);
        } else {
    ...
    // Print program break.
    st->print_cr("Program break at VM startup: "
    PTR_FORMAT
    ".", p2i(g_brk_at_startup));
    address
            brk_now = (address)
    ::sbrk(0);
    if (brk_now != (address) - 1) {
        st->print_cr("Program break now          : "
        PTR_FORMAT
        " (distance: "
        SIZE_FORMAT
        "k).",
    ...
    SIZE_FORMAT
    "k.", MaxExpectedDataSegmentSize / K);
    st->cr();
    ...
    void assert_is_valid_subrange(char *p, size_t s) const {
        ...
        if (!is_aligned_to(p, pagesize) || !is_aligned_to(p + s, pagesize)) {
            trcVerbose("range [" PTR_FORMAT " - " PTR_FORMAT "] is not"
                                                             " aligned to pagesize (%lu)", p2i(p), p2i(p + s),
...
static char *reserve_shmated_memory(size_t bytes, char *requested_addr) {
    ...
    if (addr) {
        ...
        if (requested_addr != NULL) {
            trcVerbose("failed to shm-allocate " UINTX_FORMAT " bytes at with address " PTR_FORMAT ".", size,
                       p2i(requested_addr));
        } else {
...
static char *reserve_mmaped_memory(size_t bytes, char *requested_addr) {
    ...
    if (addr == MAP_FAILED) {
        ...
        trcVerbose(
                "mmap(" PTR_FORMAT ", " UINTX_FORMAT ", ..) succeeded, but at a different address than requested (" PTR_FORMAT "), will unmap",
...
static bool checked_mprotect(char *addr, size_t size, int prot) {
    ...
    ","
    INTPTR_FORMAT
    "] with protection modes %x", p2i(addr), p2i(addr + size), prot);
    ...
    if (!rc) {
        ...
        "-"
        PTR_FORMAT
        ", 0x%X) failed (%s).", addr, addr + size, prot, s_errno);
        return false;
    }
...
jlong os::current_file_offset(int fd) {
    return (jlong)
    ::lseek64(fd, (off64_t) 0, SEEK_CUR);
}

// move file pointer to the specified offset
jlong os::seek_to_file_offset(int fd, jlong offset) {
    return (jlong)
    ::lseek64(fd, (off64_t) offset, SEEK_SET);
}
...
bool os::start_debugging(char *buf, int buflen) {
    ...
    jio_snprintf(p, buflen - len,
                 ...
                 "To debug, run 'dbx -a %d'; then switch to thread tid "
    INTX_FORMAT
    ", k-tid "
    INTX_FORMAT
    "\n"
