  ...
  if (!os::is_readable_pointer(p)) { \
    ...
    if (p_name && namelen > 0) {
        if (tb->tb.name_present) {
            ...
            // Be very careful.
            int i = 0;
            char *const p = (char *) pc2 + sizeof(short);
...
#define MAX_CALLSTACK_DEPTH 50
    ...
    while (frame < MAX_CALLSTACK_DEPTH) {
        ...
        if (sp == NULL) {
            ...
            st->print_cr("*** stack pointer invalid - backchain corrupted ("
            PTR_FORMAT
            ") ***", p2i(sp));
            retry = true;
        } else if (sp < sp_last) {
            st->print_cr("invalid stack pointer: "
            PTR_FORMAT
            " (not monotone raising)", p2i(sp));
            retry = true;
        }
        ...
        if (retry) {
            ...
            if (sp) {
                st->print_cr("found something which looks like a backchain at "
                PTR64_FORMAT
                ", after 0x%x bytes... ",
