...
// access control list.
//
static void free_security_desc(PSECURITY_DESCRIPTOR
pSD) {
...
static bool add_allow_aces(PSECURITY_DESCRIPTOR
pSD,
ace_data_t aces[],
int ace_count
) {
...
if (!
GetSecurityDescriptorDacl(pSD, &exists, &oldACL, &isdefault
)) {
if (
...
if (exists) {
if (!
GetAclInformation(oldACL, &aclinfo,
...
if (((ACCESS_ALLOWED_ACE *)ace)->Header.
...
if (matches == 0) {
if (!
AddAce(newACL, ACL_REVISION, MAXDWORD, ace,
((PACE_HEADER)
ace)->AceSize)) {
if (
PrintMiscellaneous &&Verbose
...
i++) {
if (!
AddAccessAllowedAce(newACL, ACL_REVISION,
        aces[i]
.mask, aces[i].pSid)) {
if (
PrintMiscellaneous &&Verbose
...
if (!
_SetSecurityDescriptorControl(pSD, SE_DACL_PROTECTED,
        SE_DACL_PROTECTED
)) {
if (
PrintMiscellaneous &&Verbose
...
//
static HANDLE
create_sharedmem_resources(const char *dirname, const char *filename, const char *objectname, size_t size) {
...
static size_t sharedmem_filesize(const char *filename, TRAPS) {
    ...
    if ((statbuf.st_size == 0) || (statbuf.st_size % os::vm_page_size() != 0)) {
        if (PrintMiscellaneous && Verbose) {
            warning("unexpected file size: size = "
            SIZE_FORMAT
            "\n",
                    statbuf.st_size);
        }
