...
static LPVOID virtualAlloc(LPVOID
lpAddress,
SIZE_T dwSize, DWORD
flAllocationType,
DWORD flProtect
...
if (result != NULL) {
log_trace(os)("VirtualAlloc("
PTR_FORMAT ", " SIZE_FORMAT ", %x, %x) returned " PTR_FORMAT "%s.",
p2i(lpAddress), dwSize, flAllocationType, flProtect, p2i(result),
((lpAddress
!=
NULL &&result
!= lpAddress) ? " <different base!>" : ""));
} else {
PreserveLastError ple;
log_info(os)("VirtualAlloc("
PTR_FORMAT ", " SIZE_FORMAT ", %x, %x) failed (%u).",
p2i(lpAddress), dwSize, flAllocationType, flProtect, ple
.v);
}
return
result;
...
static BOOL virtualFree(LPVOID
lpAddress,
SIZE_T dwSize, DWORD
dwFreeType) {
BOOL result = ::VirtualFree(lpAddress, dwSize, dwFreeType);
if (result != FALSE) {
log_trace(os)("VirtualFree("
PTR_FORMAT ", " SIZE_FORMAT ", %x) succeeded",
p2i(lpAddress), dwSize, dwFreeType
);
} else {
PreserveLastError ple;
log_info(os)("VirtualFree("
PTR_FORMAT ", " SIZE_FORMAT ", %x) failed (%u).",
p2i(lpAddress), dwSize, dwFreeType, ple
.v);
}
return
result;
...
static LPVOID virtualAllocExNuma(HANDLE hProcess, LPVOID lpAddress, SIZE_T dwSize, DWORD flAllocationType,
    ...
    if (result != NULL) {
        log_trace(os)("VirtualAllocExNuma("
        PTR_FORMAT
        ", "
        SIZE_FORMAT
        ", %x, %x, %x) returned "
        ...
        PreserveLastError ple;
        log_info(os)("VirtualAllocExNuma("
        PTR_FORMAT
        ", "
        SIZE_FORMAT
        ", %x, %x, %x) failed (%u).",
...
static BOOL unmapViewOfFile(LPCVOID lpBaseAddress) {
    ...
    if (result != FALSE) {
        log_trace(os)("UnmapViewOfFile("
        PTR_FORMAT
        ") succeeded", p2i(lpBaseAddress));
    } else {
        PreserveLastError ple;
        log_info(os)("UnmapViewOfFile("
        PTR_FORMAT
        ") failed (%u).", p2i(lpBaseAddress), ple.v);
    }
    return result;
}
...
Handle_FLT_Exception(struct _EXCEPTION_POINTERS *exceptionInfo);
...
WINAPI topLevelExceptionFilter(struct _EXCEPTION_POINTERS *exceptionInfo);
...
static unsigned __stdcall thread_native_entry(Thread *thread) {
  ...
  thread->call_run();
    ...
    log_info(os, thread)("Thread finished (tid: "
    UINTX_FORMAT
    ").", os::current_thread_id());
...
bool os::create_attached_thread(JavaThread *thread) {
    ...
    log_info(os, thread)("Thread attached (tid: "
    UINTX_FORMAT
    ", stack: "
    PTR_FORMAT
    " - "
...
bool os::create_thread(Thread *thread, ThreadType thr_type,
    ...
    if (thread_handle != NULL) {
        ...
        log_warning(os, thread)("Failed to start thread \"%s\" - _beginthreadex failed (%s) for attributes: %s.",
                                thread->name(), os::errno_name(errno),
                                describe_beginthreadex_attributes(buf, sizeof(buf), stack_size, initflag));
...
*SetThreadDescriptionFnPtr)(HANDLE, PCWSTR);
typedef HRESULT (WINAPI
*GetThreadDescriptionFnPtr)(HANDLE, PWSTR*);
...
void os::set_native_thread_name(const char *name) {
    ...
    if (_SetThreadDescription != NULL) {
        ...
        if (err == ERROR_SUCCESS) {
            ...
            if (FAILED(hr)) {
                log_debug(os, thread)(
                        "set_native_thread_name: SetThreadDescription failed - falling back to debugger method");
        ...
        if (FAILED(hr2)) {
          ...
          LocalFree(thread_name);
            ...
            log_debug(os, thread)(
                    "set_native_thread_name: convert_to_unicode failed - falling back to debugger method");
        }
    }
...
void os::abort(bool dump_core, void *siginfo, const void *context) {
    ...
    if (!WindowsDbgHelp::miniDumpWriteDump(hProcess, processId, dumpFile, dumpType, pmei, NULL, NULL) &&
        !WindowsDbgHelp::miniDumpWriteDump(hProcess, processId, dumpFile, (MINIDUMP_TYPE) MiniDumpWithFullMemory, pmei,
                                           NULL, NULL)) {
...
void os::dll_unload(void *lib) {
    ...
    if (::FreeLibrary((HMODULE) lib)) {
        Events::log_dll_message(NULL, "Unloaded dll \"%s\" ["
        INTPTR_FORMAT
        "]", name, p2i(lib));
        log_info(os)("Unloaded dll \"%s\" ["
        INTPTR_FORMAT
        "]", name, p2i(lib));
    } else {
        const DWORD errcode = ::GetLastError();
        Events::log_dll_message(NULL, "Attempt to unload dll \"%s\" ["
        INTPTR_FORMAT
        "] failed (error code %d)", name, p2i(lib), errcode);
        log_info(os)("Attempt to unload dll \"%s\" ["
        INTPTR_FORMAT
        "] failed (error code %d)", name, p2i(lib), errcode);
    }
}
...
void os::print_memory_info(outputStream *st) {
    ...
    if (r1 != 0) {
        ...
        st->print("TotalPageFile size "
        INT64_FORMAT
        "M ",
                (int64_t) ms.ullTotalPageFile >> 20);
        st->print("(AvailPageFile size "
        INT64_FORMAT
        "M)",
    ...
    if (r2 != 0) {
        ...
        st->print("current process commit charge (\"private bytes\"): "
        INT64_FORMAT
        "M, ",
                (int64_t) pmex.PrivateUsage >> 20);
        st->print("peak: "
        INT64_FORMAT
        "M", (int64_t) pmex.PeakPagefileUsage >> 20);
    } else {
...
void os::print_siginfo(outputStream *st, const void *siginfo) {
    ...
    if ((er->ExceptionCode == EXCEPTION_ACCESS_VIOLATION ||
        ...
        switch (er->ExceptionInformation[0]) {
            case 0:
                st->print(", reading address");
                break;
            case 1:
                st->print(", writing address");
                break;
            case 8:
                st->print(", data execution prevention violation at address");
                break;
            default:
        ...
        st->print(" "
        INTPTR_FORMAT, er->ExceptionInformation[1]);
    } else {
        int num = er->NumberParameters;
        if (num > 0) {
            st->print(", ExceptionInformation=");
            for (int i = 0; i < num; i++) {
                st->print(INTPTR_FORMAT
...
#error unknown architecture
    ...
    if (thread != nullptr && thread->is_Java_thread()) {
        thread->as_Java_thread()->set_saved_exception_pc((address)(DWORD_PTR)
        exceptionInfo->ContextRecord->PC_NAME);
    }
...
fastJNIAccessorExceptionFilter(struct _EXCEPTION_POINTERS *exceptionInfo) {
    DWORD exception_code = exceptionInfo->ExceptionRecord->ExceptionCode;
    if (exception_code == EXCEPTION_ACCESS_VIOLATION) {
        address pc = (address) exceptionInfo->ContextRecord->Eip;
...
address os::win32::fast_jni_accessor_wrapper(BasicType type) {
    switch (type) {
        ...
        case T_BYTE:
            return (address) jni_fast_GetByteField_wrapper;
        case T_CHAR:
            return (address) jni_fast_GetCharField_wrapper;
        case T_SHORT:
            return (address) jni_fast_GetShortField_wrapper;
        case T_INT:
            return (address) jni_fast_GetIntField_wrapper;
        case T_LONG:
            return (address) jni_fast_GetLongField_wrapper;
        case T_FLOAT:
            return (address) jni_fast_GetFloatField_wrapper;
        case T_DOUBLE:
            return (address) jni_fast_GetDoubleField_wrapper;
        default:
            ShouldNotReachHere();
    }
    return (address) - 1;
...
int os::create_file_for_heap(const char *dir) {
    ...
    if (fullname == NULL) {
        vm_exit_during_initialization(
                err_msg("Malloc failed during creation of backing file for heap (%s)", os::strerror(errno)));
        return -1;
    }
...
char *os::pd_attempt_reserve_memory_at(char *addr, size_t bytes, bool exec) {
    ...
    if (!use_individual) {
        ...
        if (Verbose && PrintMiscellaneous) {
            reserveTimer.stop();
            tty->print_cr("reserve_memory of %Ix bytes took "
            JLONG_FORMAT
            " ms ("
            JLONG_FORMAT
            " ticks)", bytes,
...
static char *reserve_large_pages_aligned(size_t size, size_t alignment, bool exec) {
    log_debug(pagesize)("Reserving large pages at an aligned address, alignment="
    SIZE_FORMAT
    "%s",
...
init_crit_sect_call(PINIT_ONCE, PVOID
pcrit_sect, PVOID*) {
    ...
    if (os::win32::has_exit_bug()) {
        ...
        if (!InitOnceExecuteOnce(&init_once_crit_sect, init_crit_sect_call, &crit_sect, NULL)) {
            ...
            if (what != EPT_THREAD) {
                ...
                Atomic::cmpxchg(&process_exiting, (DWORD)
...
jint os::init_2(void) {
  ...
  __asm { fldcw fp_control_word }
    ...
    NOT_PRODUCT(
    if (PrintMiscellaneous && Verbose) nx_check_protection());
#endif
...
static ULONGLONG make_double_word(DWORD
high_word,
DWORD low_word
) {
...
jlong os::current_file_offset(int fd) {
    return (jlong)
    ::_lseeki64(fd, (__int64) 0L, SEEK_CUR);
}

// move file pointer to the specified offset
jlong os::seek_to_file_offset(int fd, jlong offset) {
    return (jlong)
    ::_lseeki64(fd, (__int64) offset, SEEK_SET);
}
...
char *os::pd_map_memory(int fd, const char *file_name, size_t file_offset,
    ...
    if (allow_exec) {
        ...
        base = (char *) mapViewOfFileEx(hMap, access, 0, (DWORD)
        file_offset,
                (DWORD)
        bytes, addr);
        if (base == NULL) {
...
static void print_snippet(const void *p, outputStream *st) {
    static const int num_words = LP64_ONLY(3)
    NOT_LP64(6);
...
#define IS_IN(p) (p >= start && p < end)
    ...
    for (;;) {
        ...
        "-"
        PTR_FORMAT
        "), state=", p2i(region_start), p2i(region_end));
        switch (minfo->State) {
            case MEM_COMMIT:
                st->print_raw("MEM_COMMIT ");
                break;
            case MEM_FREE:
                st->print_raw("MEM_FREE   ");
                break;
            case MEM_RESERVE:
                st->print_raw("MEM_RESERVE");
                break;
            default:
                st->print("%x?", (unsigned) minfo->State);
        }
        st->print(", prot=%3x, type=", (unsigned) minfo->Protect);
        switch (minfo->Type) {
            case MEM_IMAGE:
                st->print_raw("MEM_IMAGE  ");
                break;
            case MEM_MAPPED:
                st->print_raw("MEM_MAPPED ");
                break;
            case MEM_PRIVATE:
                st->print_raw("MEM_PRIVATE");
                break;
            default:
                st->print("%x?", (unsigned) minfo->State);
        }
...
void os::print_memory_mappings(char *addr, size_t bytes, outputStream *st) {
    ...
    while (p < end && p >= p2) {
        ...
        if (checkedVirtualQuery(p, &minfo)) {
            ...
            static const address end_virt = (address)(LP64_ONLY(0x7ffffffffffULL)
            NOT_LP64(3 * G));
            if (p >= end_virt) {
                break;
            } else {
