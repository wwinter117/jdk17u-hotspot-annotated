...
public:
    ...
    size_t commit(size_t offset, size_t size) {
        ...
        if (!res) {
            fatal("Failed to allocate physical memory "
            SIZE_FORMAT
            "M @ "
            PTR_FORMAT
            " (%d)",
            ...
            log_debug(gc)("Allocated physical memory: "
            SIZE_FORMAT
            "M @ "
            PTR_FORMAT, size / M, offset);
        }
    ...
    size_t uncommit(size_t offset, size_t size) {
        ...
        if (!res) {
            fatal("Failed to uncommit physical memory "
            SIZE_FORMAT
            "M @ "
            PTR_FORMAT
            " (%d)",
