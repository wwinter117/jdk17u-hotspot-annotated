...
static OSReturn
add_counter(UpdateQueryP query, HCOUNTER *counter, const char *counter_path, bool first_sample_on_init) {
    assert(query != NULL, "invariant");
...
static OSReturn
add_counter(MultiCounterQueryP query, int counter_idx, const char *counter_path, bool first_sample_on_init) {
    assert(query != NULL, "invariant");
...
static OSReturn allocate_pdh_constants() {
    ...
    pdh_process_instance_wildcard_IDProcess_counter = NEW_C_HEAP_ARRAY(char, pdh_IDProcess_wildcard_fmt_len + 1,
                                                                       mtInternal);
...
CPUPerformanceInterface::CPUPerformance::CPUPerformance() : _context_switches(NULL), _process_cpu_load(NULL),
