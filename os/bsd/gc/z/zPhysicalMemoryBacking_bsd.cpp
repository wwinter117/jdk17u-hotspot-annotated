...
bool ZPhysicalMemoryBacking::commit_inner(size_t offset, size_t length) const {
    ...
    const void *const res = mmap((void *) addr, length, PROT_READ | PROT_WRITE, MAP_FIXED | MAP_ANONYMOUS | MAP_PRIVATE,
                                 -1, 0);
    if (res == MAP_FAILED) {
        ZErrno err;
...
size_t ZPhysicalMemoryBacking::uncommit(size_t offset, size_t length) const {
    ...
    const void *const res = mmap((void *) start, length, PROT_NONE,
